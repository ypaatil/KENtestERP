...
<h4 class="mb-sm-0 font-size-18">Purchase Return Master</h4>

<div class="page-title-right">
<ol class="breadcrumb m-0">
<li class="breadcrumb-item"><a href="javascript: void(0);">Forms</a></li>
<li class="breadcrumb-item active">Purchase Return Master</li>
</ol>
</div>
...
<form action="{{route('GeneralPurchaseReturn.update',$purchasereturnfetch)}}" method="POST">   
...
{{ $row->firm_id == $purchasereturnfetch->firm_id ? 'selected="selected"' : '' }}
...
<input type="text" name="pur_code" class="form-control" id="pur_code" value="{{ $purchasereturnfetch->pur_code }}" readonly="readonly">

<input type="hidden" name="c_code" id="c_code" value="{{ $purchasereturnfetch->c_code }}" />
...
<input type="date" name="pur_date" class="form-control" id="formrow-email-input" value="{{ $purchasereturnfetch->pur_date }}">
</div>
</div>
<div class="col-md-2">
<div class="mb-3">
<label for="formrow-email-input" class="form-label">Purchase Bill No</label>
<input type="text" name="pur_bill_no" class="form-control" id="formrow-email-input" value="{{ $purchasereturnfetch->pur_bill_no }}">
</div>
</div>
<div class="col-md-2">
<div class="mb-3">
<label for="formrow-email-input" class="form-label">Purchase Bill Date</label>
<input type="date" name="pur_bill_date" class="form-control" id="formrow-email-input" value="{{ $purchasereturnfetch->pur_bill_date }}">
</div>
</div>
...
{{ $rowledger->ac_code == $purchasereturnfetch->Ac_code ? 'selected="selected"' : '' }}
...
{{ $rowgst->tax_type_id == $purchasereturnfetch->tax_type_id ? 'selected="selected"' : '' }}
...
{{ $rowledger2->ac_code == $purchasereturnfetch->Purchase_Ac_code ? 'selected="selected"' : '' }}
...
<tbody>
@php  if($detailpurchasereturn->isEmpty()) { @endphp
...
<td><input type="button" class="btn btn-danger pull-left" onclick="deleteRowcone(this);" value="X" ></td>
       ...
       @php } else { @endphp
  @php $no=1; @endphp
@foreach($detailpurchasereturn as $row)
...
<td><input readOnly style="width:80px;"  type="number" step="0.01"  class="" name="pur_cgsts[]" id="pur_cgst" value="{{ $row->pur_cgst }}"></td>
<td><input  style="width:80px;"  type="number" step="0.01" class="GSTAMT" name="camts[]" id="camt" value="{{ $row->camt }}"></td>
<td><input readOnly style="width:80px;"  type="number" step="0.01" class="" name="pur_sgsts[]" value="{{ $row->pur_sgst }}" id="pur_sgst"></td>
<td><input style="width:80px;"  type="number" step="0.01" class="GSTAMT" name="samts[]" value="{{ $row->samt }}"  id="samt"></td>
<td><input readOnly style="width:80px;"  type="number" step="0.01" class="" name="pur_igsts[]" id="pur_igst" value="{{ $row->pur_igst }}"></td>
<td><input  style="width:80px;"  type="number" step="0.01" class="GSTAMT" name="iamts[]" id="iamt" value="{{ $row->iamt }}"></td>
<td><input  style="width:80px;"  type="number" step="0.01" class="GROSS" name="amounts[]" id="amount" value="{{ $row->amount }}"></td>
<td><input  style="width:80px;"  type="number" step="0.01" class="TOTAMT" name="total_amounts[]"  id="total_amount" value="{{ $row->total_amount }}"></td>
...
<br/>
<input type="hidden"   name="cnt" id="cnt" value="{{ count($detailpurchasereturn) }}">
<div class="row">
<div class="col-md-2">
<div class="mb-3">
<label for="formrow-email-input" class="form-label">Gross Amount</label>
<input type="text" name="Gross_amount" class="form-control" id="Gross_amount" onChange="mycalc();" value="{{ $purchasereturnfetch->Gross_amount }}">
</div>
</div>
<div class="col-md-2">
<div class="mb-3">
<label for="formrow-email-input" class="form-label">GST Amount</label>
<input type="text" name="Gst_amount" class="form-control" id="Gst_amount" value="{{ $purchasereturnfetch->Gst_amount }}">
</div>
</div>
...
<input type="text" name="Net_amount" class="form-control" id="Net_amount" value="{{ $purchasereturnfetch->Net_amount }}">
</div>
</div>
<div class="col-md-6">  
<div class="mb-3">
<label for="formrow-email-input" class="form-label">Narration</label>
<input type="text" name="narration" class="form-control" id="narration" value="{{ $purchasereturnfetch->narration }}">
</div>
</div>
...
t5.type="number";
t5.step="0.01";
t5.id = "pur_cgsts"+index;
t5.name= "pur_cgsts[]";
t5.value=document.getElementById("pur_cgst").value;
...
t6.id = "camts"+index;
t6.name= "camts[]";
t6.value=document.getElementById("camt").value;
...
t7.type="number";
t7.step="0.01";
t7.id = "pur_sgsts"+index;
t7.name= "pur_sgsts[]";
t7.value=document.getElementById("pur_sgst").value;
...
t6.id = "samts"+index;
t6.name= "samts[]";
t6.value=document.getElementById("samt").value;
cell6.appendChild(t6);
...
t9.type="number";
t9.step="0.01";
t9.id = "pur_igsts"+index;
t9.name= "pur_igsts[]";
t9.value=document.getElementById("pur_igst").value;
...
t10.id = "iamts"+index;
t10.name= "iamts[]";
t10.value=document.getElementById("iamt").value;
...
t11.step="0.01";
t11.className="GROSS";
t11.id = "amounts"+index;
t11.name= "amounts[]";
t11.value=document.getElementById("amount").value;
...
t12.type="number";
t12.step="0.01";
t12.className='TOTAMT';
t12.id = "total_amounts"+index;
t12.name= "total_amounts[]";
t12.value=document.getElementById("total_amount").value;
