...
<form action="{{route('Receipt_Transaction.update',$Receiptfetch)}}" method="POST">
...
<div class="col-md-2">
<div class="mb-3">
<label for="formrow-email-input" class="form-label">Receipt No</label>
...
{{ $rowledger->ac_code == $Receiptfetch->DrCode ? 'selected="selected"' : '' }}
...
{{ $rowledger11->ac_code == $Receiptfetch->CrCode ? 'selected="selected"' : '' }}
...
<form action="{{route('Receipt_Transaction.store')}}" method="POST">
<input type="hidden" name="type" id="type" class="form-control" value="<?php echo  'PURCHASE' ?>" /> 
@csrf 
<div class="row">
<div class="col-md-4">
