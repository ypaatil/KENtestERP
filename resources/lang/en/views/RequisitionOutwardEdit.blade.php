...
<h4 class="mb-sm-0 font-size-18">Requisition Outward</h4>

<div class="page-title-right">
<ol class="breadcrumb m-0">
<li class="breadcrumb-item"><a href="javascript: void(0);">Forms</a></li>
<li class="breadcrumb-item active">Requisition Outward</li>
</ol>
</div>
...
<div class="card">
<div class="card-body">
<h4 class="card-title mb-4">Requisition Outward</h4>
@if ($errors->any())
...
<form action="{{route('RequisitionOutward.update',$requisitionoutwardfetch)}}" method="POST">
...
{{ $row->firm_id == $requisitionoutwardfetch->firm_id ? 'selected="selected"' : '' }}
...
<input type="text" name="requisition_outward_no" class="form-control" id="requisition_outward_no" value="{{ $requisitionoutwardfetch->requisition_outward_no }}" readonly="readonly">
<input type="hidden" name="c_code" id="c_code" value="{{ $requisitionoutwardfetch->c_code }}" />
...
<input type="date" name="requisition_outward_date" class="form-control" id="formrow-email-input" value="{{ $requisitionoutwardfetch->requisition_outward_date }}">
</div>
</div>
...
<input type="text" name="requisitionno" class="form-control" id="requisitionno" value="{{ $requisitionoutwardfetch->requisitionNo }}" onchange="gettable(this.value);getDetails(this.value);">
</div>
</div>
...
{{ $rowdepartment->dept_id == $requisitionoutwardfetch->dept_id ? 'selected="selected"' : '' }}
...
{{ $rowmachinelist->machineId == $requisitionoutwardfetch->machineId ? 'selected="selected"' : '' }}
...
@php  if($detailrequisitionoutward->isEmpty()) { @endphp
...
<td><input type="text"   name="balanceQty[]"   value="0" id="balanceQty" style="width:80px;" required/></td>
<td><input type="text"   name="approvedQty[]"  value="0" class="RATE"  id="approvedQty" style="width:80px;" required/></td>
<td><input type="text"   name="issuedQty[]"  value="0" class="pur_cgsts"  id="issuedQty" style="width:80px;" required/></td>
...
<td><input type="button" class="btn btn-danger pull-left" onclick="deleteRowcone(this);" value="X" ></td>
  ...
  @php } else { @endphp
  @php $no=1; @endphp
@foreach($detailrequisitionoutward as $rowdetail)
...
<td><input type="text"   name="balanceQty[]"    id="balanceQty" style="width:80px;" value="{{ $rowdetail->balanceQty }}" required/></td>


<td><input type="text"   name="approvedQty[]"   class="RATE"  id="approvedQty" style="width:80px;" value="{{ $rowdetail->approvedQty }}" required/></td>
<td><input type="text"   name="issuedQty[]"  value="{{ $rowdetail->issuedQty }}" class="pur_cgsts"  id="issuedQty" style="width:80px;" required/></td>
...
<input type="hidden"   name="cnt" id="cnt" value="{{ count($detailrequisitionoutward) }}">  
<br/>
...
<input type="text" name="issueTo" class="form-control" id="issueTo" value="{{ $requisitionoutwardfetch->issueTo }}">
</div>
</div>
<div class="col-md-4">  
<div class="mb-3">
...
@foreach($reasonlist as  $rowreasonlist)
{
<option value="{{ $rowreasonlist->reasonId  }}"

{{ $rowreasonlist->reasonId == $requisitionoutwardfetch->reasonId ? 'selected="selected"' : '' }}



  >{{ $rowreasonlist->reason }}</option>
...
function gettable(requisitionNo){

  //alert(requisitionNo);

$.ajax({
type:"GET",
url:"{{ route('getRequitionDetails') }}",
//dataType:"json",
...
function getDetails(requisitionNo){

$.ajax({
type:"GET",
url:"{{ route('getMasterDetails') }}",
//dataType:"json",
data:{requisitionNo:requisitionNo},
success:function(response){
console.log(response);
...
$("#machineId").val(response[0].machineId);
$("#issueTo").val(response[0].issueTo);
$("#reasonId").val(response[0].reasonId);
