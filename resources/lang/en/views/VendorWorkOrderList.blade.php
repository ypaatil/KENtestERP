...
<div class="col-12">
    <div class="page-title-box d-sm-flex align-items-center justify-content-between">
        <h4 class="mb-sm-0 font-size-18">Vendor Work Order Model</h4>

        <div class="page-title-right">
            <ol class="breadcrumb m-0">
                <li class="breadcrumb-item"><a href="javascript: void(0);">Transaction</a></li>
                <li class="breadcrumb-item active">Vendor Work Order Model</li>
            </ol>
        </div>
...
<!--    </div>  -->
                                ...
                                <div class="card">
                                    ...
                                    <table data-order='[[ 0, "desc" ]]' data-page-length='25' id="datatable-buttons" class="table table-bordered dt-responsive nowrap w-100">
                                          <thead>
                                            ...
                                            <tr>
                                                <td > {{ substr($row->vw_code,3,15)  }} </td>
                                                <td style="text-align:center; white-space:nowrap;"> {{ $row->sales_order_no  }} </td>
                                                <td style="text-align:center; white-space:nowrap;"> {{ $row->vw_date  }} </td>
                                                <td style="text-align:center;"> {{ $row->Ac_name  }} </td>
                                                <td style="text-align:center;"> {{ $row->season_name  }} </td>
                                                
                                                <td style="text-align:right;"> {{ $row->vendorRate  }} </td>
                                               
                                                <td> {{ $row->username  }} </td>
                                                @if($chekform->edit_access==1)
                                                 <td>
                                                <a class="btn btn-outline-secondary btn-sm print" href="VWPrint/{{ $row->vw_code }}" title="print">
                                                    <i class="fas fa-print"></i>
                                                </a>
                                                </td>
                                                
                                                <td>
                                                <a class="btn btn-outline-secondary btn-sm edit" href="{{route('VendorWorkOrder.edit', $row->vw_code)}}" title="Edit">
        ...
        <button  class="btn   btn-sm delete"  data-placement="top" id="DeleteRecord" data-token="{{ csrf_token() }}" data-id="{{ $row->vw_code }}"  data-route="{{route('VendorWorkOrder.destroy', $row->vw_code )}}" title="Delete">
