...
@php 
    $formData = DB::select("select report_viewer.*, form_master.form_label,form_master.form_name from report_viewer
           INNER JOIN form_master ON form_master.form_code = report_viewer.form_code WHERE report_viewer.moduleId =".$report->moduleId);
    ...
    @foreach($formData as $form) 
        <div class="col-xl-3">
                <div class="card rptShadow">
                    <div class="card-body">
                        <a href="{{ url('/'.$form->form_name) }}"> {{$form->form_label}} </a>
