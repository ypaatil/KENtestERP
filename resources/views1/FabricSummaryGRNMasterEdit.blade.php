   ...
   <div class="col-xl-12">
      <div class="card">
         <div class="card-body">
            ...
            <form action="{{ route('FabricSummaryGRN.update',$FabricSummaryGRNMasterList->sr_no) }}" method="POST" enctype="multipart/form-data">
               @method('put')
               @csrf 
               <div class="row">
                  <div class="col-md-2">
                   ...
                   @php
                        ...
                        where tr_code ='".$FabricSummaryGRNMasterList->fsg_code."'"); 
                  ...
                  <div class="col-md-2">
                     <div class="mb-3">
                        <label for="formrow-challan_no-input" class="form-label">Challan No</label>
                        <input type="text" name="challan_no" id="challan_no" class="form-control" id="formrow-challan_no-input" value="{{$FabricSummaryGRNMasterList->challan_no}}" required>
                     </div>
                  </div>
                  <div class="col-md-2">
                     <div class="mb-3">
                        <label for="formrow-challan_date-input" class="form-label">Challan Date</label>
                        <input type="date" name="challan_date" id="challan_date" class="form-control" id="formrow-challan_date-input" value="{{$FabricSummaryGRNMasterList->challan_date}}">
                     </div>
                  </div>
                  <div class="col-md-2">
                     <div class="mb-3">
                        <label for="formrow-invoice_no-input" class="form-label">Invoice No</label>
                        <input type="text" name="invoice_no" id="invoice_no" class="form-control" value="{{ $FabricSummaryGRNMasterList->invoice_no }}" id="formrow-invoice_no-input" required>
                     </div>
                  </div>
                  <div class="col-md-2">
                     <div class="mb-3">
                        <label for="formrow-invoice_date-input" class="form-label">Invoice Date</label>
                        <input type="date" name="invoice_date" id="invoice_date" class="form-control" id="formrow-invoice_date-input" value="{{ $FabricSummaryGRNMasterList->invoice_date }}">
                     </div>
                  </div>
                  <div class="col-md-2">
                     <div class="mb-3">
                     ...
                     <table id="footable_2" class="table  table-bordered table-striped m-b-0  footable_2">
                        ...
                        <tbody>
                           ...
                           <tr>
                              ...
                              <td><input   type="text"  value="{{ $row->item_description}}" style="width:250px;height:30px;"> </td>
                              <td>
                                 <select name="unit_ids[]"   id="unit_ids"   style="width:150px;height:30px;">
                              ...
                              <td><input type="number" step="any" readOnly  name="amounts[]" class="AMT"  value="{{$total_amount}}" id="amounts" style="width:80px;height:30px;" required/>
                              <td>
                                 <select name="rack_id[]"  id="rack_id" class="select2" style="width:100px;height:30px;" >
                                    <option value="">--Racks--</option>
                                    @foreach($RackList as  $rowrack)
                                    {
                                    <option value="{{ $rowrack->rack_id }}">{{ $rowrack->rack_name }}</option>
                                    }
                                    @endforeach
                                 </select>
                              </td>
                              <td>
                                  ...
                                  <button type="button" name="allocate[]"  onclick="stockAllocate(this);" item_code="{{$row->item_code}}" isClick="{{$isclick}}" qty="{{$row->item_qty}}" bom_code="{{$row->bom_code}}" cat_id="{{$row->cat_id}}" class_id="{{$row->class_id}}"  class="btn btn-success pull-left">Allocate</button> 
                              </td>
                           </tr>
                            @endforeach
                           @endif
         ...
         <div class="col-md-2">
         <div class="mb-3">
         <label for="formrow-inputState" class="form-label">Transporter</label>
         <select name="transport_id" class="form-select" id="transport_id">
         <option value="">--Transporter--</option>
         @foreach($TransportList as  $rowTp)
         {
         <option value="{{ $rowTp->transport_id }}"
         {{ $rowTp->transport_id == $FabricSummaryGRNMasterList->transport_id ? 'selected="selected"' : '' }}       
         >{{ $rowTp->transport_name }}</option>
         }
         @endforeach
         </select>
         </div>
         ...
         <div class="col-md-2">
         <div class="mb-3">
         <label for="freight_paid" class="form-label">Freight Paid</label>
         <input type="number"   step="0.01"  name="freight_paid" class="form-control" id="freight_paid" value="{{ $FabricSummaryGRNMasterList->freight_paid }}" required>
         </div>
         </div>
         <div class="col-sm-8">
         <div class="mb-3">
    ...
    function stockAllocate(obj)
    {
      ...
      if(isClick == 0)
      {
          $.ajax({
                  ...
                  data:{'bom_code':row3,'item_code' : row1,'item_qty': row2, 'cat_id':row4, 'class_id':row5},
