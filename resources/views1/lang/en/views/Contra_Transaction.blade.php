...
<h4 class="mb-sm-0 font-size-18">Contra Voucher</h4>
...
<div class="card">
<div class="card-body">
<h4 class="card-title mb-4">Contra Voucher</h4>
@if ($errors->any())
...
<form action="{{route('Contra_Transaction.update',$Cvfetch)}}" method="POST">
...
{{ $row->firm_id == $Cvfetch->firm_id ? 'selected="selected"' : '' }}
...
<div class="col-md-2">
<div class="mb-3">
<label for="formrow-email-input" class="form-label">Contra No</label>
 <input type="hidden" name="type" id="type" class="form-control" value="<?php echo  'PAYMENT' ?>" /> 
    <input type="text" id="TrNo" name="TrNo" class="form-control" placeholder="Transaction No" value="{{ $Cvfetch->TrNo }}" readonly />
<input type="hidden" name="userId" value="{{ Session::get('userId')}}" class="form-control" id="formrow-email-input">

<input type="hidden" name="TrType" value="85" />
<input type="hidden" name="c_code" value="{{ $Cvfetch->c_code }}" />

</div>
</div>
<div class="col-md-2">
<div class="mb-3">
<label for="formrow-email-input" class="form-label">Contra Date</label>
<input type="date" name="Date" class="form-control" value="<?php echo date('Y-m-d'); ?>" value="{{ $Cvfetch->Date }}"/>
</div>
</div>
...
<input type="text" id="TrNo" name="ref_no" class="form-control" placeholder="Transaction No" value="{{ $Cvfetch->ref_no }}"  />
</div>
</div>
...
<input type="date" name="ref_date" class="form-control" value="<?php echo date('Y-m-d'); ?>" value="{{ $Cvfetch->ref_date }}"/>
</div>
</div>
</div>
...
<input type="radio" name="Pay_mode" value="1" @if($Cvfetch->Pay_mode==1) checked="Checked" @endif>BY CASH  
<input type="radio" name="Pay_mode" value="2" @if($Cvfetch->Pay_mode==2) checked="Checked" @endif>BY CHEQUE  
<input type="radio" name="Pay_mode" value="3" @if($Cvfetch->Pay_mode==3) checked="Checked" @endif>BY NEFT/RTGS  
<input type="radio" name="Pay_mode" value="4" @if($Cvfetch->Pay_mode==4) checked="Checked" @endif>OTHER
</div>
</div>

<div class="col-md-4">
<div class="mb-3">
<label for="formrow-inputState" class="form-label">Cash/Bank AC  (Dr)</label>
<select name="DrCode" class="form-select" id="DrCode">
<option value="">--Select Cash/Bank AC  (Dr)--</option>
@foreach($cashbank as  $rowledger)
{
<option value="{{ $rowledger->ac_code  }}"

{{ $rowledger->ac_code == $Cvfetch->DrCode ? 'selected="selected"' : '' }}
...
<div class="mb-3">
<label for="formrow-inputState" class="form-label">Cash/Bank AC  (Cr)</label>
<select name="CrCode" class="form-select" id="CrCode" >
<option value="">--- Select Cash/Bank AC  (Cr) ---</option>
@foreach($ledgerlist as  $rowledger11)
{
<option value="{{ $rowledger11->ac_code  }}"

{{ $rowledger11->ac_code == $Cvfetch->CrCode ? 'selected="selected"' : '' }}
...
<input type="text" name="Amount" class="form-control" id="BillAmount" onkeyup="disc_calculatess();" value="{{ $Cvfetch->Amount }}">
</div>
</div>
<div class="col-md-4">
<div class="mb-3">
<label for="formrow-email-input" class="form-label">Narration</label>
<input type="text" name="Naration" class="form-control" id="Narration" onkeyup="disc_calculatess();" value="{{ $Cvfetch->Naration }}">
</div>
</div>
</div>
...
<form action="{{route('Contra_Transaction.store')}}" method="POST">
<input type="hidden" name="type" id="type" class="form-control" value="<?php echo  'PURCHASE' ?>" /> 
@csrf 
<div class="row">
<div class="col-md-4">
