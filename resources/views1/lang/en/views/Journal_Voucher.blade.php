...
<div class="card">
<div class="card-body">
<h4 class="card-title mb-4">Journal Voucher</h4>
@if ($errors->any())
...
<form action="{{route('Journal_Voucher.update',$Jvfetch)}}" method="POST">
...
{{ $row->firm_id == $Jvfetch->firm_id ? 'selected="selected"' : '' }}
...
<div class="col-md-2">
<div class="mb-3">
<label for="formrow-email-input" class="form-label">JV No</label>
 <input type="hidden" name="type" id="type" class="form-control" value="<?php echo  'PAYMENT' ?>" /> 
    <input type="text" id="TrNo" name="TrNo" class="form-control" placeholder="Transaction No" value="{{ $Jvfetch->TrNo }}" readonly />
<input type="hidden" name="userId" value="{{ Session::get('userId')}}" class="form-control" id="formrow-email-input">

<input type="hidden" name="TrType" value="84" />
<input type="hidden" name="c_code" value="{{ $Jvfetch->c_code }}" />

</div>
</div>
<div class="col-md-2">
<div class="mb-3">
<label for="formrow-email-input" class="form-label">JV Date</label>
<input type="date" name="Date" class="form-control" value="<?php echo date('Y-m-d'); ?>" value="{{ $Jvfetch->Date }}"/>
</div>
</div>
...
<input type="text" id="TrNo" name="ref_no" class="form-control" placeholder="Transaction No" value="{{ $Jvfetch->ref_no }}"  />
</div>
</div>
...
<input type="date" name="ref_date" class="form-control" value="<?php echo date('Y-m-d'); ?>" value="{{ $Jvfetch->ref_date }}"/>
</div>
</div>
</div>
...
<input type="radio" name="Pay_mode" value="1" @if($Jvfetch->Pay_mode==1) checked="Checked" @endif>BY CASH  
<input type="radio" name="Pay_mode" value="2" @if($Jvfetch->Pay_mode==2) checked="Checked" @endif>BY CHEQUE  
<input type="radio" name="Pay_mode" value="3" @if($Jvfetch->Pay_mode==3) checked="Checked" @endif>BY NEFT/RTGS  
<input type="radio" name="Pay_mode" value="4" @if($Jvfetch->Pay_mode==4) checked="Checked" @endif>OTHER
</div>
</div>

<div class="col-md-4">
<div class="mb-3">
<label for="formrow-inputState" class="form-label">Dr</label>
<select name="DrCode" class="form-select" id="DrCode">
<option value="">--Select Dr AC--</option>
@foreach($cashbank as  $rowledger)
{
<option value="{{ $rowledger->ac_code  }}"

{{ $rowledger->ac_code == $Jvfetch->DrCode ? 'selected="selected"' : '' }}
...
<div class="mb-3">
<label for="formrow-inputState" class="form-label">Cr</label>
<select name="CrCode" class="form-select" id="CrCode" >
<option value="">--- Select Cr AC ---</option>
@foreach($ledgerlist as  $rowledger11)
{
<option value="{{ $rowledger11->ac_code  }}"

{{ $rowledger11->ac_code == $Jvfetch->CrCode ? 'selected="selected"' : '' }}
...
<input type="text" name="Amount" class="form-control" id="BillAmount" onkeyup="disc_calculatess();" value="{{ $Jvfetch->Amount }}">
</div>
</div>
<div class="col-md-4">
<div class="mb-3">
<label for="formrow-email-input" class="form-label">Narration</label>
<input type="text" name="Naration" class="form-control" id="Narration" onkeyup="disc_calculatess();" value="{{ $Jvfetch->Naration }}">
</div>
</div>
</div>
...
<form action="{{route('Journal_Voucher.store')}}" method="POST">
<input type="hidden" name="type" id="type" class="form-control" value="<?php echo  'PURCHASE' ?>" /> 
@csrf 
<div class="row">
<div class="col-md-4">
<div class="mb-3">
<label for="formrow-inputState" class="form-label">Firm11</label>
<select name="firm_id" class="form-select" id="firm_id">
<option value="">--- Select Firm ---</option>
@foreach($firmlist as  $row)
{
...
<label for="formrow-inputState" class="form-label">Dr</label>
<select name="DrCode" class="form-select" id="DrCode">
<option value="">--Select Dr A/C--</option>
@foreach($cashbank as  $rowledger)
{
