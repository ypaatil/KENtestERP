...
<div class="col-md-2">
    <div class="mb-3">
        ...
        <input type="number" step="0.01" name="order_rate" class="form-control" id="order_rate" value="{{$BuyerPurchaseOrderMasterList->order_rate}}" required>
    </div>
</div>
...
<div class="col-md-2">
 ...
 <div class="col-md-2">
    <div class="mb-3">
        ...
        <input type="number" step="0.01"  name="total_qty" class="form-control" id="total_qty" value="{{ $BuyerPurchaseOrderMasterList->total_qty }}" required readOnly>
    </div>
</div>
...
<tbody>
@if(count($BuyerPurchaseOrderDetaillist)>0)

@php $nos=1;    @endphp
@foreach($BuyerPurchaseOrderDetaillist as $List) 
 
<tr>
<td><input type="text" name="id" value="@php echo $nos; @endphp" id="id0" style="width:50px;"/></td>
<td><input type="text"  value="{{$List->item_code}}" id="icode0" style="width:50px;"/></td>
<td> <select name="item_code[]" class="Item select2"  id="item_code0" style="width:250px; height:30px;"  required @php if($is_approved==1){ echo 'disabled'; } @endphp>
...
<td> <select name="color_id[]" class="color select2"  id="color_id0" style="width:250px; height:30px;"required @php if($is_approved==1){ echo 'disabled'; } @endphp>
...
<td><input type="text"   name="s@php echo $no; @endphp[]"  class="size_id"  value="{{ $size_id }}" id="size_id0" style="width:80px;" required  @php if($is_approved==1){ echo 'readOnly'; } @endphp/></td>
@php $no=$no+1; @endphp
@endforeach
  <td class="track">
        <input type="text" name="size_qty_total[]" class="QTY" value="{{$List->size_qty_total}}" id="size_qty_total0" style="width:80px;"  @php if($is_approved==1){ echo 'readOnly'; } @endphp/>
        <input type="hidden" name="size_qty_array[]"  value="{{$List->size_qty_array}}" id="size_qty_array0" style="width:80px;"  />
        <input type="hidden" name="size_array[]"  value="{{$List->size_array}}" id="size_array" style="width:80px;"  /></td>


<td> <select name="unit_id[]" class="unit select2" id="unit_id" style="width:150px; height:30px;" required @php if($is_approved==1){ echo 'disabled'; } @endphp>
...
@endforeach
 ...
 <td><input type="number" step="0.01" name="shipment_allowance[]"  value="{{$List->shipment_allowance}}" id="shipment_allowance" style="width:80px;" required @php if($is_approved==1){ echo 'readOnly'; } @endphp/></td>
 <td><input type="number" step="0.01" name="garment_rejection_allowance[]"   value="{{$List->garment_rejection_allowance}}" id="garment_rejection_allowance" style="width:80px;" required @php if($is_approved==1){ echo 'disabled'; } @endphp/></td>
@php   if($is_approved!=1){  @endphp
<td><input type="button" style="width:40px;" id="Abutton0"  name="button[]"    value="+" class="Abutton btn btn-warning pull-left">  <input type="button" id="Bbutton0" class="btn btn-danger pull-left" onclick="deleteRowcone(this);" value="X" ></td>
@php }  @endphp
</tr>

@php $nos=$nos+1;  @endphp
@endforeach
...
<div class="col-md-2">
    <div class="mb-3">
        ...
        <input type="number" name="shipped_qty" class="form-control" id="shipped_qty" value="{{ $BuyerPurchaseOrderMasterList->shipped_qty }}" required onkeyup="calculate();">
    </div>
</div>
...
window.onload = function() {
  mycalc();
  @php     if($BOMCheck>0 || $is_approved==1) {   @endphp
