...
<form action="{{route('MaterialInward.update',$MaterialOutwardMasterList)}}" method="POST" enctype="multipart/form-data">
@method('put')    
@csrf 
<div class="row">
<div class="col-md-2">
...
<input type="text" name="job_code" class="form-control" id="job_code" value="{{ $MaterialOutwardMasterList->job_code  }}">
<input type="hidden" name="userId" value="{{ Session::get('userId') }}" class="form-control" id="formrow-email-input">
<input type="hidden" name="InCode" value="{{ $MaterialOutwardMasterList->InCode  }}" class="form-control" id="formrow-email-input">
</div>
</div>
<div class="col-md-2">
<div class="mb-3">
...
{
    <option value="{{ $Trtyperow->transactionId  }}"

{{ $Trtyperow->transactionId == $MaterialOutwardMasterList->transactionId ? 'selected="selected"' : '' }}    



        >{{ $Trtyperow->transactionType }}</option>
}
@endforeach
</select>
</div>
...
<input type="text" name="lot_no" class="form-control" id="lot_no" value="{{ $MaterialOutwardMasterList->lot_no  }}">
<input type="hidden" name="userId" value="{{ Session::get('userId') }}" class="form-control" id="formrow-email-input">
</div>
</div>
<div class="col-md-2">
    <div class="mb-3">
        <label for="po_date" class="form-label">Inward Date</label>
        <input type="date" name="inward_date" class="form-control" id="inward_date" value="{{ $MaterialOutwardMasterList->inward_date  }}">
    </div>
</div>
...
{{ $row->ac_code == $MaterialOutwardMasterList->ac_code ? 'selected="selected"' : '' }}    
...
@php  if($MaterialOutwarddetailslist->isEmpty()) { @endphp
...
<td><input type="button" class="btn btn-danger pull-left" onclick="deleteRowcone(this);" value="X" ></td>
    ...
    @php } else { @endphp
  @php $no=1; @endphp
@foreach($MaterialOutwarddetailslist as $rowdetail)
...
{{ $row->item_code == $rowdetail->item_code ? 'selected="selected"' : '' }}

        >{{ $row->item_name }}</option>
}
@endforeach
</select></td>
<td><input type="text" class="StyleNo" id="style_code" onkeyup="mycalc();" name="style_code[]" value="{{ $rowdetail->style_no }}"  style="width:100px;" /></td>
...
{{ $rowcolor->color_id == $rowdetail->color_id ? 'selected="selected"' : '' }}
...
<td> <select name="sz_code[]"  id="sz_code" style="width:100px;">
<option value="">--Size--</option>
@foreach($SizeList as  $rowsize)
{
    <option value="{{ $rowsize->sz_code }}"

{{ $rowsize->sz_code == $rowdetail->sz_code ? 'selected="selected"' : '' }}


        >{{ $rowsize->sz_name }}</option>
}
@endforeach
</select></td>
<td><input type="text" class="PRODQTY"  name="production_qty[]" value="{{ $rowdetail->qty }}" id="prodqty1" style="width:80px;" /></td>
...
<input type="number" value="{{ count($MaterialOutwarddetailslist) }}" name="cntrr" id="cntrr" readonly="" hidden="true"  />
...
@php  if($fabricdetails->isEmpty()) { @endphp
...
@php } else { @endphp
  @php $no=1; @endphp
@foreach($fabricdetails as $rowdetail)
<tr>
<td><input type="text" name="id" value="{{ $no }}" id="id" style="width:50px;"/></td>
<td> <select name="item_codefabric[]" class="item" id="item_codefabric" style="width:100px;">
<option value="">--Item--</option>
...
<td><input type="text" class="StyleNo" onkeyup="mycalc();" name="style_codefabric[]" value="{{ $rowdetail->style_no }}"  style="width:80px;" id="style_codefabric" required /></td>

<td> <select name="color_idfabric[]"  id="color_idfabric" style="width:100px;">
<option value="">--Color--</option>
@foreach($ColorList as  $rowcolors)
{
    <option value="{{ $rowcolors->color_id }}"

{{ $rowcolors->color_id == $rowdetail->color_id ? 'selected="selected"' : '' }}

        >{{ $rowcolors->color_name }}</option>
}
@endforeach
</select></td>

<td><input type="text" class="METER"  name="meter[]" value="{{ $rowdetail->meter }}" id="meter" style="width:80px;" /></td>
...
<input type="number" value="{{ count($fabricdetails) }}" name="cntrfabric" id="cntrfabric" readonly="" hidden="true"  />

<div class="col-md-2">
    <div class="mb-3">
        <label for="total_qty" class="form-label">Total Qty</label>
<input type="number"   name="total_qty" class="form-control" id="total_qty" value="{{ $MaterialOutwardMasterList->totalQty  }}" onkeyup="mycalc();">
    </div>
</div>
<div class="col-md-2">
    <div class="mb-3">
<label for="rate_per_piece" class="form-label">Total Meter</label>
<input type="number" step="0.01"  name="total_meter" class="form-control" id="total_meter" value="{{ $MaterialOutwardMasterList->totalMeter  }}" onkeyup="mycalc();">
    </div>
</div>
</div>
</div>
...
<div class="mb-3">
<input type="text" name="narration" class="form-control" id="narration"  value="{{ $MaterialOutwardMasterList->narration  }}" />
</div>
</div>
