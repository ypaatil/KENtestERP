...
<!--    </div>  -->
                                ...
                                <div class="card">
                                    ...
                                    <table data-order='[[ 0, "desc" ]]' data-page-length='25' id="datatable-buttons" class="table table-bordered dt-responsive nowrap w-100">
                                          <thead>
                                            ...
                                            @foreach($SalesOrderCostingList as $row)    
                                            <tr>
                                                <td > {{ substr($row->soc_code,4,15)  }} </td>
                                                <td style="text-align:center; white-space:nowrap;"> {{ $row->sales_order_no  }} </td>
                                                <td style="text-align:center; white-space:nowrap;"> {{ $row->soc_date  }} </td>
                                                
                                               
                                                <td style="text-align:center;"> {{ $row->Ac_name  }} </td>
                                                <td style="text-align:center;"> {{ $row->mainstyle_name  }} </td>
                                                <td style="text-align:center;"> {{ $row->fg_name  }} </td>
                                                ...
                                                @if($chekform->edit_access==1)
                                                   <td>
                                                <a class="btn btn-outline-secondary btn-sm print" target="_blank" href="GetCostingData/{{ $row->soc_code }}" title="print">
                                                    <i class="fas fa-print"></i>
                                                </a>
                                                </td>
                                                
                                                <td>
                                                <a class="btn btn-outline-secondary btn-sm edit" href="{{route('SalesOrderCosting.edit', $row->soc_code)}}" title="Edit">
        ...
        <button  class="btn   btn-sm delete"  data-placement="top" id="DeleteRecord" data-token="{{ csrf_token() }}" data-id="{{ $row->soc_code }}"  data-route="{{route('SalesOrderCosting.destroy', $row->soc_code )}}" title="Delete">
