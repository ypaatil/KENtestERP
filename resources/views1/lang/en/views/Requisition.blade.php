...
<input type="text" name="requisitionNo" class="form-control" id="requisitionNo" value="" readonly="readonly">
...
<select name="requisitionTypeId" class="form-select" id="requisitionTypeId" required="required">
<option value="">--- Select Requisition Type---</option>
@foreach($requisitiontypelist as  $rowrequisitionlist)
{
<option value="{{ $rowrequisitionlist->requisitionId  }}">{{ $rowrequisitionlist->requisitiontype }}</option>
...
<select name="dept_id" class="form-select" id="dept_id" required="required">
...
<select name="machineId" class="form-select" id="machineId" required="required">
<option value="">--- Select Machine---</option>
@foreach($machinelist as  $rowmachine)
{
<option value="{{ $rowmachine->machineId  }}">{{ $rowmachine->machineName }}</option>
...
<td><input type="text"   name="requestedQty[]" class="requestedQty"   value="0" id="requestedQty" style="width:80px;" required/></td>
...
{
<option value="{{ $rowreason->reasonId  }}">{{ $rowreason->reason }}</option>
...
$("#pur_igst").val(response[0].igst_per);
 ...
 $('#footable_2').on('blur', '.requestedQty', function() 
 {
 
    var requestedQty = $(this).val();
    var row = $(this).closest('tr'); // get the row

  var stockQty=row.find('input[name^="stockQty[]"]').val();

if(parseFloat(requestedQty) > parseFloat(stockQty))
{

  alert('Request quantity is greater than stock quantity');

  row.find('input[name^="requestedQty[]"]').val(0);
}
