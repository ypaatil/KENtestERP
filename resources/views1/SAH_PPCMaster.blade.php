      ...
      <div class="card">
         <div class="card-body">
            ...
            @if(isset($SAHPPCEditData))
              <form action="{{ route('SAH_PPCMasterUpdate',["id" => $SAHPPCEditData->sah_ppc_master_id] ) }}" method="POST" enctype="multipart/form-data">
                  @method('put')
                   @csrf 
            ...
            @else
            <form action="{{route('SAHPPC')}}" method="POST" enctype="multipart/form-data">
               @csrf 
               <input type="hidden" name="userId" value="{{ Session::get('userId') }}" class="form-control" id="formrow-brand_name-input">
               <div class="row">
                  <input type="number" value="1" name="cntrr" id="cntrr" readonly="" hidden="true"  />
