...
<td><input type="text"    name="roll_no[]"  value="{{ $List->roll_no }}" id="roll_no" style="width:80px;" required /></td>
...
<td><input type="text"   class="METER" name="meter[]" onkeyup="mycalc();" value="{{ $List->meter }}" id="meter1" style="width:80px;height:30px;" required/>
<td><input type="text"   name="width[]"  value="{{ $List->width }}" id="width" style="width:80px;" required /></td>

 <input type="hidden"   class="KG" name="kg[]" onkeyup="mycalc();" value="{{ $List->kg }}" id="kg" style="width:80px;height:30px;" required/> 
  <input type="hidden"   name="item_rate[]"   value="{{ $List->item_rate }}" id="item_rate" style="width:80px;height:30px;" required/> 
</td>
...
<td> <select name="defect_id[]"  id="defect_id" style="width:100px;" required>
...
<td><input type="text" name="extra_meter[]"  value="{{ $List->extra_meter }}" id="extra_meter" style="width:80px;" required /></td>
<td><input type="text" name="track_code[]"  value="{{ $List->track_code }}" id="track_code" style="width:80px;height:30px;" readOnly /></td>

 <td> <select name="rack_id[]"  id="rack_id" style="width:100px;height:30px;"  >
...
<td><input type="text" name="width[]"  value="" id="width" style="width:80px;" required /></td>
 <input type="hidden" step="0.01" class="KG" name="kg[]" onkeyup="mycalc();" value="0" id="kg" style="width:80px;" required/>
...
<td><input type="text" name="extra_meter[]"  value="0" id="extra_meter" style="width:80px;" required /></td>
<td><input type="text" name="track_code[]"  value="" id="track_code" style="width:80px;" readOnly /></td>
<td> <select name="rack_id[]"  id="rack_id" class="select2" style="width:100px;" required>
...
$(document).on("change", 'input[name^="meter[]"]', function (event) {
	...
	{ 
        ...
        var reject_short_meter=+row.find('input[name^="reject_short_meter[]"]').val();
         if(old_meter<meter)
         {   
             var em=parseFloat(meter - old_meter - reject_short_meter).toFixed(2);
             row.find('input[name^="extra_meter[]"]').val(em);
             row.find('input[name^="reject_short_meter[]"]').val(0);
         }
         else if(old_meter>meter)
         {
            var rm=parseFloat(old_meter - (meter+extra_meter)).toFixed(2);
            row.find('input[name^="reject_short_meter[]"]').val(rm);
...
$(document).on("change", 'input[name^="reject_short_meter[]"]', function (event) 
{CalculateRow2($(this).closest("tr"));});
	...
	{ 
        ...
        var extra_meter=+row.find('input[name^="extra_meter[]"]').val();
        var reject_short_meter=+row.find('input[name^="reject_short_meter[]"]').val();
        var meter=parseFloat(old_meter + extra_meter-reject_short_meter).toFixed(2);
        row.find('input[name^="meter[]"]').val(meter);
	   	mycalc();
}
                      ...
                      $.ajax({
                           ...
                           if((data['result'])=='success')
                          {
                            alert('Barcode For Roll: '+track_code+' is Ready');
                             window.open('https://ken.korbofx.com/barcode/index.php?id=2', '_blank').focus();
                          }
                          else
                          {
