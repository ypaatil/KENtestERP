                ...
                <div class="card">
                    <div class="card-body">
                        ...
                        <table id="datatable-buttons" class="table table-bordered dt-responsive nowrap w-100">
                            ...
                            <tr>
                                <td style="text-align:center; white-space:nowrap"><a target="_blank" href="http://ken.korbofx.com/PurchaseOrder/{{ $row->sr_no }}/edit" >{{ $row->pur_code }} </a></td>
                                ...
                                <td style="text-align:center; white-space:nowrap"><a target="_blank" href="http://ken.korbofx.com/PurchaseOrder/{{ $row->sr_no }}/edit" >{{ number_format($row->item_qty) }}</a></td>
                                <td style="text-align:center; white-space:nowrap"> <span onclick="openmodal({{ $row->sr_no }},{{ $row->item_code }});" style="color:#556ee6; cursor: pointer;">{{ number_format($row->received_item_qty) }}</span></td>
                                <td style="text-align:right;">{{   number_format((double)(($row->item_qty-$row->received_item_qty)), 2, '.', '') }}</td>
                                <td style="text-align:right;">{{  number_format((double)($row->received_item_qty), 2, '.', '') }}</td>
                                <td style="text-align:right;">0</td>
                                <td style="text-align:right;">{{   number_format((double)($row->issue_item_qty), 2, '.', '')}}</td>
                                <td style="text-align:right;">{{   number_format((double)(($row->received_item_qty - $row->issue_item_qty)), 2, '.', '')}}</td>
                                <td style="text-align:center; white-space:nowrap">{{ $row->job_status_name }}</td>
                                <td style="text-align:right;">{{   number_format((double)($row->t30_days_item_qty), 2, '.', '')}}</td>
                                <td style="text-align:right;">{{   number_format((double)($row->t60_days_item_qty), 2, '.', '')}}</td>
                                <td style="text-align:right;">{{    number_format((double)($row->t90_days_item_qty), 2, '.', '')}}</td>
                                </tr>
                                @endforeach
                            </tbody>
                        </table>
...
<h6 class="txt-dark capitalize-font"><i class="zmdi zmdi-calendar-note mr-10"></i>GRN (Trims Inward)</h6>
