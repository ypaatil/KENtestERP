...
<div class="row">
   <div class="col-xl-12">
      <div class="card">
         <div class="card-body">
            <h4 class="card-title mb-4">Return Packing Inhouse</h4>
            @if ($errors->any())
            <div class="col-md-6">
            ...
            @endif
            @if(isset($ReturnPackingInhouseMasterList))
            <form action="{{ route('ReturnPackingInhouseMaster.update',$ReturnPackingInhouseMasterList) }}" method="POST" enctype="multipart/form-data" id="frmData">
               @method('put')
               @csrf
               <div class="row">
                  <div class="col-md-2">
                     <div class="mb-3">
                        <label for="rpki_date" class="form-label">Entry Date</label>
                        <input type="date" name="rpki_date" class="form-control" id="rpki_date" value="{{$ReturnPackingInhouseMasterList->rpki_date}}" required  >
                        <input type="hidden" name="c_code" class="form-control" id="c_code" value="{{ $ReturnPackingInhouseMasterList->c_code }}">
                        <input type="hidden" name="rpki_code" class="form-control" id="rpki_code" value="{{$ReturnPackingInhouseMasterList->rpki_code}}" required readOnly>
                        <input type="hidden" name="userId" value="{{ Session::get('userId') }}" class="form-control" id="formrow-email-input">
                     </div>
                  </div>
                  <div class="col-md-2">
                     <div class="mb-3">
                        <label for="sale_code" class="form-label">Sale Invoice No.</label>
                        ...
                        <select name="sale_code" class="form-control select2" id="sale_code"    onchange="GetTaxType(this.value);GetInvoiceWiseSalesOrderList();" disabled>
                            @php 
                                $SaleInvoiceList = DB::select("SELECT  DISTINCT sale_code from sale_transaction_detail");
                            @endphp
                           <option value="">--Select--</option>
                           @foreach($SaleInvoiceList as  $row)
                           {
                           <option value="{{ $row->sale_code }}"  {{ $row->sale_code == $ReturnPackingInhouseMasterList->sale_code ? 'selected="selected"' : '' }}  >{{ $row->sale_code }}</option>
                           }
                           @endforeach
                        </select>
                     </div>
                  ...
                  <div class="col-md-2">
                     <div class="mb-3">
                        ...
                        <select name="sales_order_no" class="form-control select2" id="sales_order_no" required  onChange="getSalesOrderTableOpening(this.value);GetSaleInvoices(this.value);" disabled>
                           <option value="">--Sales Order No--</option>
                            @php 
                                $SalesOrderList = DB::select("SELECT DISTINCT sales_order_no from sale_transaction_detail where sale_code='".$ReturnPackingInhouseMasterList->sale_code."'");
                            @endphp
                           @foreach($SalesOrderList as  $row)
                           {
                           <option value="{{ $row->sales_order_no }}"  {{ $row->sales_order_no == $ReturnPackingInhouseMasterList->sales_order_no ? 'selected="selected"' : '' }}  >{{ $row->sales_order_no }}</option>
                           }
                           @endforeach
                        </select>
                     </div>
                  ...
                  <div class="col-md-4">
                     <div class="mb-3">
                        ...
                        <select name="Ac_code" class="form-control select2" id="Ac_code" disabled >
                           ...
                           {{ $row->ac_code == $ReturnPackingInhouseMasterList->Ac_code ? 'selected="selected"' : '' }} 
                           >{{ $row->ac_name }}</option>
                           }
                           @endforeach
                        </select>
                  ...
                  <input type="hidden" name="tax_type_id" class="form-control" id="tax_type_id" value="{{$ReturnPackingInhouseMasterList->tax_type_id}}"> 
                  <input type="hidden" name="vendor_rate" class="form-control" id="vendor_rate" value="{{$ReturnPackingInhouseMasterList->vendor_rate}}"> 
                  <div class="col-md-2">
                     <div class="mb-3">
                        ...
                        <select name="mainstyle_id" class="form-control" id="mainstyle_id"  onchange="getSubStyle(this.value)" required disabled>
                           ...
                           {{ $row->mainstyle_id == $ReturnPackingInhouseMasterList->mainstyle_id ? 'selected="selected"' : '' }}  
                  ...
                  <div class="col-md-2">
                     <div class="mb-3">
                        ...
                        <select name="substyle_id" class="form-control" id="substyle_id" onchange="getStyle(this.value)" required disabled>
                           ...
                           {{ $row->substyle_id == $ReturnPackingInhouseMasterList->substyle_id ? 'selected="selected"' : '' }}
                  ...
                  <div class="col-md-2">
                     <div class="mb-3">
                        ...
                        <select name="fg_id" class="form-control" id="fg_id" required disabled>
                           ...
                           {{ $row->fg_id == $ReturnPackingInhouseMasterList->fg_id ? 'selected="selected"' : '' }} 
                           >{{ $row->fg_name }}</option>
                           }
                           @endforeach
                        </select>
                  ...
                  <div class="col-md-2">
                     <div class="mb-3">
                        ...
                        <input type="text" name="style_no" class="form-control" id="style_no" value="{{$ReturnPackingInhouseMasterList->style_no}}" required readOnly>
                     </div>
                  </div>
                  <div class="col-md-4">
                     <div class="mb-3">
                        <label for="style_description" class="form-label">Style Description</label>
                        <input type="text" name="style_description" class="form-control" id="style_description" value="{{$ReturnPackingInhouseMasterList->style_description}}" required readOnly>
                     </div>
                  </div>  
               </div> 
              <input type="hidden" id="location_id" name="location_id"  value="{{$ReturnPackingInhouseMasterList->location_id}}" > 
              <input type="hidden" id="vendorId" name="vendorId"  value="{{$ReturnPackingInhouseMasterList->vendorId}}" > 
              <input type="hidden" id="is_opening" name="is_opening"  value="{{$ReturnPackingInhouseMasterList->is_opening}}" > 
               <div class="row"  >
                  <div class="  "  >
                                    ...
                                    <div class="table-wrap">
                                       <div class="table-responsive">
                                          <table id="footable_2" class="table  table-bordered table-striped m-b-0  footable_2">
                                             ...
                                             <tbody>
                                                @if(count($ReturnPackingInhouseDetailList)>0)
                                                @php $no=1;$n=1; @endphp
                                                @foreach($ReturnPackingInhouseDetailList as $List) 
                                                <tr>
                                                   ...
                                                   <td ><input style="width:80px; float:left;"  name="s@php echo $n; @endphp[]" class="size_id" type="number" id="s@php echo $n; @endphp" value="{{$szQty}}" prev_val="{{$szQty}}" required onchange="calculateTax(this);checkTotalQty(this);" />  </td>
                                                   @php $n=$n+1;  @endphp
                                                   @endforeach
                                                   ...
                                                   <td><input type="number" step="any" name="rates[]"  onchange="calculateTax(this);" value="{{$List->rate}}" id="rate" style="width:80px;  float:left;"  /></td>
                                                   <td><input type="number" step="any" name="cgst[]" class="readonly" readonly  value="{{$List->cgst}}" id="cgst" style="width:80px;  float:left;"  /></td>
                                                   <td><input type="number" step="any" name="camt[]" class="readonly" readonly  value="{{$List->camt}}" id="camt" style="width:80px;  float:left;"  /></td>
                                                   <td><input type="number" step="any" name="sgst[]" class="readonly" readonly  value="{{$List->sgst}}" id="sgst" style="width:80px;  float:left;"  /></td>
                                                   <td><input type="number" step="any" name="samt[]" class="readonly" readonly  value="{{$List->samt}}" id="samt" style="width:80px;  float:left;"  /></td>
                                                   <td><input type="number" step="any" name="igst[]" class="readonly" readonly  value="{{$List->igst}}" id="igst" style="width:80px;  float:left;"  /></td>
                                                   <td><input type="number" step="any" name="iamt[]" class="readonly" readonly  value="{{$List->iamt}}" id="iamt" style="width:80px;  float:left;"  /></td>
                                                   <td><input type="number" step="any" name="amount[]"  class="readonly" readonly  value="{{$List->amount}}" id="amount" style="width:80px;  float:left;"  /></td>
                                                   <td><input type="number" step="any" name="total_amount[]" class="readonly" readonly   value="{{$List->total_amount}}" id="total_amount" style="width:80px;  float:left;"  /></td>
                                                   <input type="hidden" name="is_transfered[]"   value="{{$List->is_transfered}}" id="is_transfered" style="width:80px; height:30px; float:left;" required readOnly  />
                                                   <input type="hidden" name="trans_sales_order_no[]"  value="{{ $List->trans_sales_order_no }}" id="trans_sales_order_no" style="width:80px; float:left;"  />
                                                   <input type="hidden" name="transfer_code[]"  value="{{ $List->transfer_code }}" id="transfer_code" style="width:80px;  float:left;"  />
                                                </tr>
                                                @php $no=$no+1;  @endphp
                                                @endforeach
                                                @endif
               ...
               <div class="row">
                  <div class="col-md-2">
                     <div class="mb-3">
                        ...
                        <input type="text" name="total_qty" class="form-control" id="total_qty" value="{{$ReturnPackingInhouseMasterList->total_qty}}" required readOnly>
                     </div>
                  </div>
                  <input type="hidden" step="any" name="rate" class="form-control" id="rate" value="{{$ReturnPackingInhouseMasterList->rate}}">
                  <div class="col-md-2">
                     <div class="mb-3">
                        <label for="vendor_amount" class="form-label">Total Amount</label>
                        <input type="text" name="vendor_amount" class="form-control" id="vendor_amount" value="{{$ReturnPackingInhouseMasterList->vendor_amount}}" required readOnly>
                     </div>
                  </div>
                  <div class="col-sm-6">
                     <label for="formrow-inputState" class="form-label">Narration</label>
                     <div class="mb-3">
                        <input type="text" name="narration" class="form-control" id="narration"  value="{{$ReturnPackingInhouseMasterList->narration}}" />
                     </div>
                  </div>
               </div>
               <div class="col-sm-6">
                  ...
                  <div class="form-group">
                     ...
                     <a href="{{ Route('ReturnPackingInhouseMaster.index') }}" class="btn btn-warning w-md">Cancel</a>
                  </div>
               </div>
         </div>
         </form>
...
<script>
   ...
   function checkTotalQty(row)
   {
      var cur_val = $(row).val();
      
      var total_val = $(row).attr('prev_val');
      console.log(total_val);
      if(cur_val > total_val)
      {
          alert("Quatity should be less than "+total_val);
          $(row).val(total_val);
      }
   }
   
   function GetInvoiceWiseSalesOrderList()
   {
       var sale_code = $('#sale_code').val();
        $.ajax({
               type: "GET",
               dataType:"json",
               url: "{{ route('GetInvoiceWiseSalesOrderList') }}",
   ...
   { 
        ...
        var size_qty_total = $(row).parent().parent('tr').find('td input[name="size_qty_total[]"]').val();
        var rate = $(row).parent().parent('tr').find('td input[name="rates[]"]').val();
        var sale_cgst = $(row).parent().parent('tr').find('td input[name="cgst[]"]').val();
        var sale_sgst = $(row).parent().parent('tr').find('td input[name="sgst[]"]').val();
        var sale_igst = $(row).parent().parent('tr').find('td input[name="igst[]"]').val();
        
        var amount =  size_qty_total * rate; 
        
        $(row).parent().parent('tr').find('td input[name="amount[]"]').val(amount).attr('readonly','readonly').addClass('readonly');
     
        if(tax_type_id == 2)
        {
            var iamt=  parseFloat(( amount*(sale_igst/100))).toFixed(2);
            $(row).parent().parent('tr').find('td input[name="iamt[]"]').val(iamt).attr('readonly','readonly').addClass('readonly');
            $(row).parent().parent('tr').find('td input[name="total_amount[]"]').val(parseFloat(amount) + parseFloat(iamt)).attr('readonly','readonly').addClass('readonly');
            
        }
        else
        {
            var camt=  parseFloat(( amount*(sale_cgst/100))).toFixed(2);
            $(row).parent().parent('tr').find('td input[name="camt[]"]').val(camt).attr('readonly','readonly').addClass('readonly');
            var samt= parseFloat(( amount*(sale_sgst/100))).toFixed(2);
            $(row).parent().parent('tr').find('td input[name="samt[]"]').val(samt).attr('readonly','readonly').addClass('readonly');
            $(row).parent().parent('tr').find('td input[name="total_amount[]"]').val(parseFloat(amount) + parseFloat(camt) + parseFloat(samt)).attr('readonly','readonly').addClass('readonly');
        }
   }
   
   function GetTaxType(sale_code)
   {
       $.ajax({
               type: "GET",
               dataType:"json",
               ...
               success: function(data)
               {
                    $('#tax_type_id option[value="'+data.html+'"]').attr("selected", "selected").trigger('change');
                    if(data.html === 1)
                    {
                        $('#footable_2 tbody tr').find('td input[name="igst[]"]').val(0).attr('readonly','readonly').addClass('readonly');
                        $('#footable_2 tbody tr').find('td input[name="iamt[]"]').val(0).attr('readonly','readonly').addClass('readonly');
                        $('#footable_2 tbody tr').find('td input[name="cgst[]"]').val(data.detail[0].sale_cgst).attr('readonly','readonly').addClass('readonly');
                        $('#footable_2 tbody tr').find('td input[name="camt[]"]').attr('readonly','readonly').addClass('readonly');
                        $('#footable_2 tbody tr').find('td input[name="sgst[]"]').val(data.detail[0].sale_sgst).attr('readonly','readonly').addClass('readonly');
                        $('#footable_2 tbody tr').find('td input[name="samt[]"]').attr('readonly','readonly').addClass('readonly');
                    }
                    else if(data.html === 2)
                    {
                        $('#footable_2 tbody tr').find('td input[name="cgst[]"]').val(0).attr('readonly','readonly').addClass('readonly');
                        $('#footable_2 tbody tr').find('td input[name="camt[]"]').val(0).attr('readonly','readonly').addClass('readonly');
                        $('#footable_2 tbody tr').find('td input[name="sgst[]"]').val(0).attr('readonly','readonly').addClass('readonly');
                        $('#footable_2 tbody tr').find('td input[name="samt[]"]').val(0).attr('readonly','readonly').addClass('readonly');
                        $('#footable_2 tbody tr').find('td input[name="igst[]"]').val(data.detail[0].sale_igst).attr('readonly','readonly').addClass('readonly');
                        $('#footable_2 tbody tr').find('td input[name="iamt[]"]').attr('readonly','readonly').addClass('readonly');
                    }
   ...
   function GetSaleInvoices(sales_order_no)
   {
        $.ajax({
               type: "GET",
               dataType:"json",
               url: "{{ route('GetSaleInvoices') }}",
               data:{'sales_order_no':sales_order_no},
               success: function(data)
               {
                   $('#sale_code').html(data.html);
               }
        });
   }
   function getSalesOrderTableOpening(sales_order_no)
   {
         ...
         $.ajax({
           ...
           $.ajax({
           dataType: "json",
           url: "{{ route('Op_ReturnGetOrderQty') }}",
       ...
       $("#footable_2 tr td  input[class='size_qty_total']").each(function() {
        ...
        // alert(color_id);
         ...
         if(size_qty_total!=0)
         {
           $.ajax({
           dataType: "json",
           url: "{{ route('GetFabricConsumption') }}",
           ...
           var size_qty_array=$(this).closest("tr").find('input[name="size_qty_array[]"]').val();
           var size_array=$(this).closest("tr").find('input[name="size_array[]"]').val();
           
           $.ajax({
           dataType: "json",
           url: "{{ route('GetSewingConsumption') }}",
           ...
           success: function(data){
            //$("#footable_2").html(data.html);
            $("#SewingData").append(data.html);
           }
           });
           
           $.ajax({
           dataType: "json",
           url: "{{ route('GetPackingConsumption') }}",
           data:{'color_id':color_id,'size_qty_total':size_qty_total,'sales_order_no':sales_order_no,'no':no,'size_qty_array':size_qty_array,'size_array':size_array},
           success: function(data){
            //$("#footable_2").html(data.html);
