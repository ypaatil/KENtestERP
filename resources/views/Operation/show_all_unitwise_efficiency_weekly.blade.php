...
<div class="row">
    <div class="left-buttons">
      ...
      <div class="centerDiv">
        <span>Unitwise Efficiency Report Weekly</span>
    </div>
...
$grandTotalUnitWise=[];$totalPresentUnitWise=[];$AllUnitTotal=0;$AllUnitPresent=0;$AllUnitHorizontalTotal=0;$AllUnitPresentHorizontalTotal=0;  @endphp
 ...
 <div class="table-responsive">
   ...
   <table border="1" id="rptsum">
    <thead>
        <tr>
            <th rowspan="3" class="sticky">Date</th>
   ...
   <th style="background-color:#91d14f;font-weight: 700;color:#000;" nowrap>Eff.</th>  
    ...
    <tbody>
     ...
     @php $totalProMin=0;$totalEFF=0;$totalPresent=0;  $totals = array_fill(0, count($deptlist), ['totalProMin' => 0, 'totalEFF' => 0]); @endphp
        ...
        ->select('daily_pr_date','dept_id','sub_company_id', 
         DB::raw("daily_pr_date as daily_pr_date"),
        DB::raw("
    ...
    $dataMap[$record->dept_id][$record->sub_company_id][$record->daily_pr_date][] = [
...
$day = (int) 1;     // Ensure it's an integer
...
@endphp
        ...
        <tr>
            ...
            <td style="font-weight:bold;" nowrap><a href="/show_all_unit_and_datewise_efficiency/{{ date('Y-m-d',strtotime($list[$x])) }}/{{ date('Y-m-t',strtotime($list[$x])) }}" >{{ date('d-m-Y', strtotime($list[$x])) }} -{{ date('t-m-Y',strtotime($list[$x])) }}</a></td>
              ...
              foreach($periodW as $rowDates)    
              {
             if (isset($dataMap[$rowFetch->dept_id][$rowSubcompany->sub_company_id][$rowDates])) {
        foreach($dataMap[$rowFetch->dept_id][$rowSubcompany->sub_company_id][$rowDates] as $record) {
   ...
   @endphp
           ...
           <td class="amount text-right present{{ $rowFetch->dept_id }}{{ $rowSubcompany->sub_company_id  }}-emp" style="text-align:right;display:none">{{ $total_present; }}</td>  
                   ...
                   @php 
                     ...
                     if(!isset($totalProduced[$rowSubcompany->sub_company_id][$rowDates])) {
                        $totalProduced[$rowSubcompany->sub_company_id][$rowDates] = 0;
                        $totalPresentss[$rowSubcompany->sub_company_id][$rowDates]= 0;
                     }
                    $totalProduced[$rowSubcompany->sub_company_id][$rowDates] += $producedMin;
                    $totalPresentss[$rowSubcompany->sub_company_id][$rowDates] += $total_present;
           ...
           @endforeach
             ...
             @if($totalProduced[$rowSubcompany->sub_company_id][$rowDates]!=0)
            <td class="amount text-right" style="text-align:right;">{{ $totalProduced[$rowSubcompany->sub_company_id][$rowDates] != 0 ? indian_number_format_wd($totalProduced[$rowSubcompany->sub_company_id][$rowDates]) : '-' }}</td> 
            @else
              <td class="amount text-right" style="text-align:right;">-</td> 
            @endif
            
            
          @if($totalProduced[$rowSubcompany->sub_company_id][$rowDates]!=0 && $totalPresentss[$rowSubcompany->sub_company_id][$rowDates]!=0)

            <td class="amount text-right" style="text-align:right;">{{   number_format((float)(($totalProduced[$rowSubcompany->sub_company_id][$rowDates] / ($totalPresentss[$rowSubcompany->sub_company_id][$rowDates] * 480)) * 100), 2, '.', '') }}</td>  
            ...
            @php
             ...
             $AllUnitHorizontalTotal+=$totalProduced[$rowSubcompany->sub_company_id][$rowDates];
             $AllUnitPresentHorizontalTotal+= $totalPresentss[$rowSubcompany->sub_company_id][$rowDates];
         ...
         @php $periodD = App\Http\Controllers\DailyProductionEntry::getBetweenDates(date('Y-m-d',strtotime($list[$x])),date('Y-m-d',strtotime('+7 day',strtotime($list[$x])))); @endphp
         
         
                    <td style="font-weight:bold;" nowrap><a href="/show_all_unit_and_datewise_efficiency/{{ date('Y-m-d',strtotime($list[$x])) }}/{{ date('Y-m-d',strtotime('+7 day',strtotime($list[$x]))) }}" >{{ date('d-m-Y', strtotime($list[$x])) }} -{{ date('d-m-Y',strtotime('+7 day',strtotime($list[$x]))) }}</a></td>
              ...
              @foreach($deptlist  as  $index =>  $rowFetch)
                    ...
                    $producedMin=0;$total_present=0; 
              foreach($periodD as $rowDatesD)    
              {
             if (isset($dataMap[$rowFetch->dept_id][$rowSubcompany->sub_company_id][$rowDatesD])) {
        foreach($dataMap[$rowFetch->dept_id][$rowSubcompany->sub_company_id][$rowDatesD] as $record) {
            $producedMin+= $record['producedMin'];
            $total_present+= $record['total_present']; 
   ...
   @endphp
           ...
           <td class="amount text-right present{{ $rowFetch->dept_id }}{{ $rowSubcompany->sub_company_id  }}-emp" style="text-align:right;display:none">{{ $total_present; }}</td>  
                   ...
                   @php 
                     ...
                     if(!isset($totalProduced[$rowSubcompany->sub_company_id][$rowDatesD])) {
                        $totalProduced[$rowSubcompany->sub_company_id][$rowDatesD] = 0;
                        $totalPresentss[$rowSubcompany->sub_company_id][$rowDatesD] = 0;
                     }
                    $totalProduced[$rowSubcompany->sub_company_id][$rowDatesD] += $producedMin;
                    $totalPresentss[$rowSubcompany->sub_company_id][$rowDatesD] += $total_present;
           ...
           @endforeach
             ...
             @if($totalProduced[$rowSubcompany->sub_company_id][$rowDatesD]!=0)
            <td class="amount text-right" style="text-align:right;">{{ $totalProduced[$rowSubcompany->sub_company_id][$rowDatesD] != 0 ? indian_number_format_wd($totalProduced[$rowSubcompany->sub_company_id][$rowDatesD]) : '-' }}</td> 
            @else
              <td class="amount text-right" style="text-align:right;">-</td> 
            @endif
            
            
          @if($totalProduced[$rowSubcompany->sub_company_id][$rowDatesD]!=0 && $totalPresentss[$rowSubcompany->sub_company_id][$rowDatesD]!=0)

            <td class="amount text-right" style="text-align:right;">{{   number_format((float)(($totalProduced[$rowSubcompany->sub_company_id][$rowDatesD] / ($totalPresentss[$rowSubcompany->sub_company_id][$rowDatesD] * 480)) * 100), 2, '.', '') }}</td>  
            ...
            @php
             ...
             $AllUnitHorizontalTotal+=$totalProduced[$rowSubcompany->sub_company_id][$rowDatesD];
             $AllUnitPresentHorizontalTotal+= $totalPresentss[$rowSubcompany->sub_company_id][$rowDatesD];
...
function html_table_to_excel(type) {
    ...
    XLSX.writeFile(file, 'Unit-wise Efficiency Weekly.' + type);
}
