...
<div class="page-header d-lg-flex d-block">
   <div class="page-leftheader">
      <h4 class="page-title">Line Plan Master List</h4>
   </div>
   ...
   <div class="col-12">
      ...
      <div class="card">
         ...
         <div class="card-header border-bottom-0">
            <a href="{{ Route('line_plan.create') }}"><input type='button' class="btn btn-info mt-2" value='Add New Record' ></a>
         </div>
       ...
       <div class="row">
                        ...
                        <select name="sub_company_id" id="sub_company_id"  class="form-control" autocomplete="off" data-placeholder="Select" tabindex="26">
                        <option label="Select" value="">Any</option>
                        @foreach($sub_company_list as $rowSC)
                        <option value="{{ $rowSC->sub_company_id }}">{{ $rowSC->sub_company_name }}</option>
                        @endforeach
                        </select>
                        </div>
                        </div>
                        ...
                        <select name="mainstyle_id" id="mainstyle_id"  class="form-control" autocomplete="off" data-placeholder="Select" tabindex="26">
                        <option label="Select" value="">Any</option>
...
<script type="text/javascript"> 
  ...
  $(function () {
   ...
   function load_data(sub_company_id,dept_id,mainstyle_id)
{    
    ...
    var url = window.location.href;  
       ...
       var table = $('#task-list').DataTable({
        ...
        serverSide: true,
            ...
            ajax: {
                  url:url,
         		  data:  {sub_company_id:sub_company_id,dept_id:dept_id,mainstyle_id:mainstyle_id},
         		  dataSrc: 'data'
               },   
    ...
    // }], 
        columns: [
            ...
            load_data(sub_company_id,dept_id,mainstyle_id);
   ...
   $(document).ready(function(){
     ...
     $.ajax({
   	...
   	swal({
   				title: "Success",
   				text: "Line plan has been deleted",
   				icon: "success",
   			});
   ...
   $('#exampleModal').modal('show');
    ...
    var line_plan_id = $(this).attr('data-id');
 

     $.ajax({

          type: "POST",
          url: "{{ route('get_line_table') }}",
          data: { line_plan_id :line_plan_id,_token:"{{ csrf_token() }}" }, 
           success: function(data)
           {
