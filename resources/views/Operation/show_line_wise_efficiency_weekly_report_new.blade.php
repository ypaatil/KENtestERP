...
@endphp
     ...
     <tr> 
        ...
        <td class="amount text-right">{{ date('Y-m-d', strtotime($list[$x])) }} -{{ date('Y-m-t',strtotime($list[$x])) }}</td>   
          ...
          foreach($deptlist as  $indexw => $rowFetchW)
         {
            ...
            foreach($periodW as $datesw)
            {
                        ...
                        if (isset($dataMap[$rowFetch->line_id][$sub_company_id][$datesw])) {
        foreach($dataMap[$rowFetch->line_id][$sub_company_id][$rowDates] as $record) {
            $produceMinw+= $record['producedMin'];
            $totalPresentsw+= $record['total_present']; 
            ...
            @endif
             ...
             @if($countsW!=0)
            <td class="amount text-right  line{{ $rowFetchD->line_id }}-eff" style="text-align:right">{{ indian_number_format_wd(round($produceMinw / ($totalPresentsw * 480))); }}</td> 
            @else
        ...
        <td class="amount text-right">{{ date('Y-m-d', strtotime($list[$x])) }} -{{ date('Y-m-d',strtotime('+7 day',strtotime($list[$x]))) }}</td> 
          @php
          
         $periodD = App\Http\Controllers\DailyProductionEntry::getBetweenDates(date('Y-m-d',strtotime($list[$x])),date('Y-m-d',strtotime('+7 day',strtotime($list[$x])))); 
          
          
         foreach($deptlist as  $indexd => $rowFetchD)
         { 
         
         $produceMin=0;$totalPresents=0; 
           
            foreach($periodD as $rowDatesD)
            {
 
                        if (isset($dataMap[$rowFetchD->line_id][$sub_company_id][$rowDatesD])) {
        foreach($dataMap[$rowFetchD->line_id][$sub_company_id][$rowDatesD] as $recordD) {
            $produceMin+= $recordD['producedMin'];
            $totalPresents+= $recordD['total_present']; 
         ...
         @endphp
           ...
           <td class="amount text-right present{{ $rowFetchD->line_id }}-emp" style="text-align:right;display:none">{{ $totalPresents; }}</td>  
        @php } @endphp
        ...
        $(this).find('td.PROEFF').text((((produceMinHorizontal) /(totalPresentHorizontal * 480) * (100))).toFixed(2));
         ...
         $('#rptsum tfoot td.PROMINGrand').text(totalProducedGrand.toLocaleString('en-IN'));
          $('#rptsum tfoot td.PROEFFGrand').text((((totalProducedGrand / (totalPresentGrand * 480)) * (100))).toFixed(2)); 
              ...
              $('#rptsum tfoot td.PROMINGrand').text('-');
               $('#rptsum tfoot td.PROEFFGrand').text('-'); 
         }
