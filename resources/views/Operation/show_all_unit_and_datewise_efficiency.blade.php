   ...
   <table border="1" id="rptsum">
    <thead>
        ...
        <tr>
            ...
            <th colspan="2" nowrap>{{ $rowFetch->dept_name }}</th>
             @endforeach
     ...
     @foreach($subCompanyList as $rowSubcompany)
      @foreach($deptlist as  $rowFetch)
   <th style="background-color:#91d14f;font-weight: 700;color:#000" nowrap>Pro. Mins.</th>
   ...
   <th style="background-color:#91d14f;font-weight: 700;color:#000" nowrap>Eff.</th>  
    ...
    <tbody>
     ...
     @php $totalProMin=0;$totalEFF=0;$totalPresent=0;  $totals = array_fill(0, count($deptlist), ['totalProMin' => 0, 'totalEFF' => 0]); @endphp
        ...
        ->select('daily_pr_date','dept_id','sub_company_id','daily_pr_date', 
        DB::raw("
        ...
        "))
        ->whereBetween('daily_pr_date',[$fromDate,$toDate])
        ->whereIn('sub_company_id',array_unique($sub_company_array))
    ...
    $dataMap[$record->dept_id][$record->sub_company_id][$record->daily_pr_date][] = [
        ...
        'total_present' =>$record->total_present ?? 0
          ...
          $period = App\Http\Controllers\DailyProductionEntry::getBetweenDates($fromDate,$toDate);
    @endphp
        ...
        <tr>
            <td style="font-weight:bold;" nowrap>{{ date('d-m-Y',strtotime($rowDates)) }}</td>
              @foreach($subCompanyList as $rowSubcompany)
