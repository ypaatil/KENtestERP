...
<div class="row">
    <div class="left-buttons">
      ...
      <div class="centerDiv">
        <span>Unitwise Efficiency Report Yearly</span>
    </div>
   ...
   <th style="background-color:#91d14f;font-weight: 700;">Efficiency</th>  
    ...
    @endphp
       ...
       @foreach($MonthList as $month)
        ...
        <tr> 
          <td class="amount text-right"><a href="/show_line_wise_efficiency_weekly_new/{{ $month->monthId }} / {{ $year }}/{{ $sub_company_id }}">{{ $month->MonthName }}</a></td>    
          @foreach($deptlist as  $index => $rowFetch)
            ...
            if (isset($dataMap[$rowFetch->line_id][$sub_company_id][$month->monthId])) {
            foreach($dataMap[$rowFetch->line_id][$sub_company_id][$month->monthId] as $record) {
            ...
            @if($producedMin!=0)
            <td class="amount text-right lineAmt{{ $rowFetch->line_id }}" lineAttr="{{ $rowFetch->line_id  }}" style="text-align:right">{{ indian_number_format_wd(round(($producedMin ?? 0)))  }}</td> 
            @else
            ...
            <td class="amount text-right line{{ $rowFetch->line_id }}-eff" style="text-align:right">{{ number_format((float)((round(($producedMin ?? 0)) / ($total_present * 480)) * (100)), 2, '.', ''); }}</td>  
           ...
           <td class="amount text-right present{{ $rowFetch->line_id }}-emp" style="text-align:right;display:none">{{ $total_present; }}</td>  
          ...
          @endforeach
            ...
            @if($totalProMin!=0)
            <td class="amount text-right TOTALPROMIN" style="text-align:right">{{ indian_number_format_wd($totalProMin); }}</td> 
            @else
          ...
          @if($totalProMin!=0 && $totalPresent!=0)
            ...
            <td class="amount text-right TOTALGRANDEFF" style="text-align:right">{{   number_format((float)(($totalProMin / ($totalPresent * 480)) * 100), 2, '.', '') }}</td>  
        ...
        @php  $totalProMin=0;$totalEFF=0;$totalPresent=0; @endphp
       ...
       @endphp
          ...
          <td class="amount text-right GRANDPROMIN" style="text-align:right;font-weight:bold"></td>  
          <td class="amount text-right GRANDPROEFF" style="text-align:right;font-weight:bold"></td> 
...
$(document).ready(function () {
    ...
    $('#rptsum tbody tr').each(function () {
        ...
        $(this).find('td').each(function () {
         ...
         if(!isNaN(parseFloat($(this).closest('tr').find('td.TOTALPROMIN').text().replace(/,/g, '')))) {
            GrandPro += parseFloat($(this).closest('tr').find('td.TOTALPROMIN').text().replace(/,/g, ''));
             }
         
    });
    
    $('#rptsum tfoot td.GRANDPROMIN').text(GrandPro.toLocaleString('en-IN'));
    $('#rptsum tfoot td.GRANDPROEFF').text(((GrandPro / (grandPresent * 480)) * 100).toFixed(2));
