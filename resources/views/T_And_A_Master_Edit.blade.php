    ...
    <div class="col-xl-12">
        <div class="card">
            <div class="card-body">
                ...
                <form action="{{route('T_And_A_Master.update',$T_And_A_MasterList)}}" method="POST" id="frmData">
                    @csrf 
                    @method('put')
                    <div class="row">
                        <div class="col-md-4">
                            <div class="mb-4">
                                ...
                                <select name="tr_code" class="form-select" id="tr_code" onchange="getDetails(this.value);">
                                    <option value="">--Select Sales Order No--</option>
                                    @foreach($SalesOrderList as  $row)
                                    {
                                        <option value="{{ $row->tr_code }}" {{ $row->tr_code== $T_And_A_MasterList->tr_code  ? 'selected="selected"' : '' }}>{{ $row->tr_code }}</option>
                                    }
                                    @endforeach
                                </select>
                            </div>
                        ...
                        <div class="col-md-4">
                            <div class="mb-4">
                                ...
                                <select name="Ac_code" class="form-select" id="Ac_code">
                                    ...
                                    {
                                        <option value="{{ $row->ac_code }}" {{ $row->ac_code== $T_And_A_MasterList->Ac_code  ? 'selected="selected"' : '' }}>{{ $row->ac_name }}</option>
                                    }
                                    @endforeach
                                </select>
                            </div>
                        ...
                        <div class="col-md-4">
                            <div class="mb-4">
                                ...
                                <input type="date" name="order_received_date" class="form-control" id="order_received_date" value="{{$T_And_A_MasterList->order_received_date }}">
                            </div>
                        </div>
                    </div>
                    <div class="row">
                        <div class="col-md-2">
                            <div class="mb-2">
                                ...
                                <select name="mainstyle_id" class="form-select" id="mainstyle_id">
                                    ...
                                    {
                                        <option value="{{ $row->mainstyle_id }}" {{ $row->mainstyle_id== $T_And_A_MasterList->mainstyle_id  ? 'selected="selected"' : '' }}>{{ $row->mainstyle_name }}</option>
                                    }
                                    @endforeach
                                </select>
                            </div>
                        ...
                        <div class="col-md-2">
                            <div class="mb-2">
                                ...
                                <select name="substyle_id" class="form-select" id="substyle_id">
                                    ...
                                    {
                                        <option value="{{ $row->substyle_id }}" {{ $row->substyle_id== $T_And_A_MasterList->substyle_id  ? 'selected="selected"' : '' }}>{{ $row->substyle_name }}</option>
                                    }
                                    @endforeach
                                </select>
                            </div>
                        ...
                        <div class="col-md-2">
                            <div class="mb-2">
                                ...
                                <select name="fg_id" class="form-select" id="fg_id">
                                    ...
                                    {
                                        <option value="{{ $row->fg_id }}" {{ $row->fg_id== $T_And_A_MasterList->fg_id  ? 'selected="selected"' : '' }}>{{ $row->fg_name }}</option>
                                    }
                                    @endforeach
                                </select>
                            </div>
                        ...
                        <div class="col-md-2">
                            <div class="mb-2">
                                ...
                                <select name="style_no" class="form-select" id="style_no">
                                    ...
                                    {
                                        <option value="{{ $row->style_no }}" {{ $row->style_no== $T_And_A_MasterList->style_no  ? 'selected="selected"' : '' }}>{{ $row->style_no }}</option>
                                    }
                                    @endforeach
                                </select>
                            </div>
                        ...
                        <div class="col-md-2">
                            <div class="mb-2">
                                ...
                                <input type="text" name="style_description" class="form-control" id="style_description" value="{{$T_And_A_MasterList->style_description}}" >
                            </div>
                        </div>
                        <div class="col-md-2">
                            <div class="mb-2">
                                <label for="formrow-email-input" class="form-label">Shipment Date</label>
                                <input type="date" name="shipment_date" class="form-control" id="shipment_date" value="{{$T_And_A_MasterList->shipment_date}}">
                                <input type="hidden" name="userId" value="{{ Session::get('userId') }}" class="form-control" id="formrow-email-input">
                                <input type="hidden" name="t_and_a_id" value="{{ $T_And_A_MasterList->t_and_a_id }}" class="form-control" id="formrow-email-input">
                            </div>
                        </div>
                    </div>
                    <div class="row">
                        <div class="table-wrap">
                            <div class="table-responsive">
                                <table id="footable_3" class="table  table-bordered table-striped m-b-0  footable_3">
                                    ...
                                    <tbody id="PlanData">
                                        @php  if($T_And_A_Detailfetch->isEmpty()) { @endphp   
                                        ...
                                        @php } else { @endphp
                                        @php $no=1; @endphp
                                        @foreach($T_And_A_Detailfetch as $row)
                                        <tr>
                                            <td><input type="text" name="id[]" value="{{ $no }}" id="id" style="width:50px;"/></td>
                                            <td> <select name="act_id[]" class="item" id="act_id" style="width:200px; height:30px;" required>
                                                <option value="">--Activity Name--</option>
                                                @foreach($ActList as  $rowact)
                                                {
                                                    <option value="{{ $rowact->act_id }}" {{ $row->act_id== $rowact->act_id  ? 'selected="selected"' : '' }}>{{ $rowact->act_name }}</option>
                                                }
                                                @endforeach
                                            </select></td>

                                             
                                            <td><input type="date" name="target_date[]" id="target_date" style="width:110px;" value="{{$row->target_date}}" @if(Session::get('user_type')!=1) readOnly @endif  /></td>
                                            <td><input type="date" name="actual_date[]" id="actual_date" style="width:110px;" value="{{$row->actual_date}}" /></td>
                                            <!--<td>-->
                                            <!--    <input type="button" onclick="insertcone(); " class="btn btn-warning pull-left" value="+">-->
                                            <!--    <input type="button" class="btn btn-danger pull-left" onclick="deleteRowcone(this);" value="X" >-->
                                            <!--</td>-->
