...
<div class="row">
   <div class="col-12">
      <div class="page-title-box d-sm-flex align-items-center justify-content-between">
         <h4 class="mb-sm-0 font-size-18">Trims Outward-Cutting Department Edit</h4>
         <div class="page-title-right">
            <ol class="breadcrumb m-0">
               <li class="breadcrumb-item"><a href="javascript: void(0);">Transaction</a></li>
               <li class="breadcrumb-item active">Trims Outward - Cutting Department Edit</li>
            </ol>
         </div>
      </div>
   </div>
   ...
   <div class="col-xl-12">
      <div class="card">
         <div class="card-body">
            ...
            @endif
             @if(isset($TrimsOutwardCuttingMasterList))
            <form action="{{ route('TrimsOutwardCuttingDepartment.update',$TrimsOutwardCuttingMasterList) }}" method="POST" enctype="multipart/form-data" id="frmData">
               @method('put')
               @csrf 
               <div class="row">
                  <div class="col-md-2">
                     <div class="mb-3">
                        ...
                        <input type="date" name="tocd_date" class="form-control" id="tocd_date" value="{{$TrimsOutwardCuttingMasterList->tocd_date}}" required>
                        <input type="hidden" name="tocd_code" class="form-control" id="tocd_code" value="{{$TrimsOutwardCuttingMasterList->tocd_code}}"> 
                  ...
                  <div class="col-md-2">
                     <div class="mb-3">
                        ...
                        <select name="dc_no" class="form-select select2" id="dc_no" onchange="GetTrimsOutwardData();" disabled>
                           <option value="">-- Select --</option>
                           @foreach($foutList as $rows)
                                <option value="{{$rows->trimOutCode}}" {{ $rows->trimOutCode == $TrimsOutwardCuttingMasterList->dc_no ? 'selected="selected"' : '' }}>{{$rows->trimOutCode}}({{$rows->vpo_code}})</option>
                           @endforeach
                        </select>
                     </div>
                  </div>
                  ...
                  <div class="col-md-2">
                     <div class="mb-3">
                        ...
                        <input type="date" name="outward_date" class="form-control" id="outward_date" value="{{$TrimsOutwardCuttingMasterList->outward_date}}"  readonly>
                        </div>
                  </div>
                     ...
                     <div class="mb-3">
                        ...
                        <select name="vendorId" class="form-select select2" id="vendorId" disabled>
                           ...
                           @foreach($ledger_master as $rowss)
                                <option value="{{$rowss->ac_code}}" {{ $rowss->ac_code == $TrimsOutwardCuttingMasterList->vendorId ? 'selected="selected"' : '' }}>{{$rowss->ac_short_name}}</option>
                           @endforeach
                        </select>
                     </div>
                  </div>
                   <div class="col-md-2">
                     <div class="mb-3">
                        ...
                        <input type="text" name="cutting_po_no" class="form-control" id="cutting_po_no" value="{{$TrimsOutwardCuttingMasterList->cutting_po_no}}" readonly>
                     </div>
                  </div>
                   <div class="col-md-2">
                     <div class="mb-3">
                        ...
                        <select name="mainstyle_id" class="form-select select2" id="mainstyle_id" disabled>
                           ...
                           @foreach($main_style_master as $rows)
                                <option value="{{$rows->mainstyle_id}}" {{ $rows->mainstyle_id == $TrimsOutwardCuttingMasterList->mainstyle_id ? 'selected="selected"' : '' }}>{{$rows->mainstyle_name}}</option>
                           @endforeach
                        </select>
                     </div>
                  </div>
                     ...
                     <div class="mb-3">
                        ...
                        <select name="substyle_id" class="form-select select2" id="substyle_id" disabled>
                           ...
                           @foreach($sub_style_master as $rows)
                                <option value="{{$rows->substyle_id}}" {{ $rows->substyle_id == $TrimsOutwardCuttingMasterList->substyle_id ? 'selected="selected"' : '' }}>{{$rows->substyle_name}}</option>
                           @endforeach
                        </select>
                     </div>
                  </div>
                     ...
                     <div class="mb-3">
                        ...
                        <select name="fg_id" class="form-select select2" id="fg_id" disabled>
                           ...
                           @foreach($fg_master as $rows)
                                <option value="{{$rows->fg_id}}" {{ $rows->fg_id == $TrimsOutwardCuttingMasterList->fg_id ? 'selected="selected"' : '' }}>{{$rows->fg_name}}</option>
                           @endforeach
                        </select>
                     </div>
                  </div>
                   <div class="col-md-2">
                     <div class="mb-3">
                        ...
                        <input type="text" name="style_no" class="form-control" id="style_no" value="{{$TrimsOutwardCuttingMasterList->style_no}}" readonly>
                     </div>
                  </div>
                  <div class="col-md-2">
                     <div class="mb-3">
                        <label for="style_description" class="form-label">Style Description</label>
                        <input type="text" name="style_description" class="form-control" id="style_description" value="{{$TrimsOutwardCuttingMasterList->style_description}}" readonly>
                     </div>
                  </div>
                        ...
                        <table id="footable_1" class="table table-bordered table-striped m-b-0  footable_1">
                           ...
                           <tbody>
                               @php
                                    $sr_no = 1;
                               @endphp
                               @foreach($TrimsInwardCuttingDetailList as $row)
                                <tr>
                                   ...
                                   <td><input type="number" step="any" name="outward_qty[]" value="{{$row->outward_qty}}" id="outward_qty" onchange="calTotals();" style="width:80px;" readonly></td>  
                                </tr>
                                @endforeach
                           </tbody> 
                        </table>
               ...
               <div class="row">
                  <div class="col-md-2">
                     <div class="mb-3">
                        ...
                        <input type="number" step="any" name="total_outward_meter" class="form-control" id="total_outward_meter" value="{{$TrimsOutwardCuttingMasterList->total_outward_meter}}" readonly>
                     </div>
                  </div>
                  <div class="col-md-2">
                     <div class="mb-3">
                        <label for="total_received_meter" class="form-label">Total Outward Qty.</label>
                        <input type="number" step="any"  name="total_received_meter" class="form-control" id="total_received_meter" value="{{$TrimsOutwardCuttingMasterList->total_received_meter}}" readonly>
                     </div>
                  </div> 
                  <div class="col-sm-4">
                     <div class="mb-3">
                        <label for="remark" class="form-label">Remark</label>
                        <input type="text" name="remark" class="form-control" id="remark" value="{{$TrimsOutwardCuttingMasterList->remark}}" />
                     </div>
                  </div>
                  <div class="col-sm-6">
...
<script>
    ...
    function calTotals() 
    {
        ...
        {
            total_qty +=parseFloat($(this).find("td input[name='item_qty[]']").val()) || 0;
            total_outward += parseFloat($(this).find("td input[name='outward_qty[]']").val()) || 0; 
        });
         

        $('#total_outward_meter').val(outward_total);
        $("#total_received_meter").val(total_challan); 
    }
