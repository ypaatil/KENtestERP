...
<div class="row">
   <div class="col-xl-12">
      <div class="card">
         <div class="card-body">
            <h4 class="card-title mb-4">FG Transfer To Location Inward</h4>
            @if ($errors->any())
            <div class="col-md-6">
            ...
            @endif
            <form action="{{route('FGTransferToLocationInward.store')}}" method="POST" enctype="multipart/form-data" id="frmData">
               @csrf 
               <div class="row">
                  <div class="col-md-2">
                     <div class="mb-3">
                        <label for="fgti_date" class="form-label">Entry Date</label>
                        <input type="date" name="fgti_date" class="form-control" id="fgti_date" value="{{date('Y-m-d')}}" required>
                  ...
                  <div class="col-md-2">
                     <div class="mb-3">
                        ...
                        <select name="fgt_code" class="form-control select2" id="fgt_code" required onchange="GetFGTransferLocationData(this.value);" >
                           <option value="">--Select--</option>
                           @foreach($FGTCodeList as  $row)
                                <option value="{{ $row->fgt_code }}">{{ $row->fgt_code }}</option>
                           @endforeach
                        </select>
                     </div>
                  </div>
                  ...
                  <div class="col-md-3">
                     ...
                     <select name="from_loc_id" class="form-control select2" id="from_loc_id" required>
                  ...
                  <div class="col-md-3">
                     ...
                     <select name="to_loc_id" class="form-control select2" id="to_loc_id" required>
                        <option value="">--Location--</option>
                        @foreach($LocationList as  $row)
                        <option value="{{ $row->loc_id }}">{{ $row->location }}</option>
                        @endforeach
                  ...
                  <div class="col-md-6">
                     <div class="mb-5">
                        ...
                        <select name="sales_order_no[]" class="form-control select2" id="sales_order_no" onchange="getPackingInhouseDetails();" multiple disabled>
                  ...
                  <input type="number" value="1" name="cntrr1" id="cntrr1" hidden="true"/>
               </div>
               <div class="panel-group" id="accordion">
                  <div class="panel panel-default">
                     <div class="panel-heading">
                        <h4 class="panel-title">
                           <a data-toggle="collapse" data-parent="#accordion" href="#collapse1">FG Transfer To Location Inward Entry</a>
                        </h4>
                     </div>
                        ...
                        <div class="panel-body">
                           <div class="row">
                              <div class="table-wrap">
                                 <div class="table-responsive" id="tbls"> 
                                 </div>
                              </div>
                           </div>
                        </div>
...
<script>
    ...
    function GetFGTransferLocationData(fgt_code)
    {
        $.ajax({
           dataType: "json",
           url: "{{ route('GetFGTransferLocationData') }}",
           data: { 'fgt_code': fgt_code },
           success: function(data)
           {
                var salesOrders = data.sales_order_no ? data.sales_order_no.split(',') : [];
    
                if (data.sales_order_options) {
                    var optionsHtml = '';
                    data.sales_order_options.forEach(function(item) {
                        optionsHtml += '<option value="' + item.id + '">' + item.text + '</option>';
                    });
                    $('#sales_order_no').html(optionsHtml);
                }
    
                $('#sales_order_no').val(salesOrders);
                $('#sales_order_no').select2();
                $('#sales_order_no').trigger('change');
                $('#Ac_code').val(data.Ac_code).trigger('change');
                $('#from_loc_id').val(data.from_loc_id).trigger('change');
                $('#to_loc_id').val(data.to_loc_id).trigger('change');
                $('#driver_name').val(data.driver_name);
                $('#vehical_no').val(data.vehical_no);
