...
<div class="row">
   <div class="col-xl-12">
      <div class="card">
         <div class="card-body">
            <h4 class="card-title mb-4">QC Stitching Inhouse</h4>
            @if ($errors->any())
            <div class="col-md-6">
            ...
            @endif
            @if(isset($QCStitchingInhouseMasterList))
            <form action="{{ route('QCStitchingInhouse.update',$QCStitchingInhouseMasterList->qcsti_code) }}" method="POST" enctype="multipart/form-data" id="frmData">
               @method('put')
               @csrf
               <div class="row">
                  <div class="col-md-2">
                     <div class="mb-3">
                        <label for="qcsti_date" class="form-label">Entry Date</label>
                        <input type="date" name="qcsti_date" class="form-control" id="qcsti_date" value="{{$QCStitchingInhouseMasterList->qcsti_date}}" required  >
                        <input type="hidden" name="c_code" class="form-control" id="c_code" value="{{ $QCStitchingInhouseMasterList->c_code }}">
                        <input type="hidden" name="qcsti_code" class="form-control" id="qcsti_code" value="{{$QCStitchingInhouseMasterList->qcsti_code}}" required readOnly>
                        <input type="hidden" name="userId" value="{{ Session::get('userId') }}" class="form-control" id="formrow-email-input">
                     </div>
                  </div>
                  <div class="col-md-2">
                     <div class="mb-3">
                        <label for="po_date" class="form-label">Vendor Work Order</label>
                        <select name="vw_code" class="form-control select2" id="vw_code" required  onChange="getStitchingInhouseDetails(this.value);" disabled>
                           <option value="">--Vendor Work Order No--</option>
                           @foreach($VendorWorkOrderList as  $row)
                           {
                           <option value="{{ $row->vw_code }}"
                           {{ $row->vw_code == $QCStitchingInhouseMasterList->vw_code ? 'selected="selected"' : '' }} 
                  ...
                  <div class="col-md-2">
                     <div class="mb-3">
                        ...
                        <input type="text" name="sales_order_no" class="form-control" id="sales_order_no" value="{{$QCStitchingInhouseMasterList->sales_order_no}}" required readOnly>
                     </div>
                  </div>
                  <div class="col-md-4">
                     <div class="mb-3">
                        ...
                        <select name="Ac_code" class="form-control  " id="Ac_code" required  disabled>
                           ...
                           {{ $row->ac_code == $QCStitchingInhouseMasterList->Ac_code ? 'selected="selected"' : '' }} 
                           >{{ $row->ac_name }}</option>
                           }
                           @endforeach
                        </select>
                  ...
                  <div class="col-md-2">
                     <div class="mb-3">
                        ...
                        <input type="text" name="vendor_rate" class="form-control" id="vendor_rate" value="{{$QCStitchingInhouseMasterList->vendor_rate}}" required readOnly>
                     </div>
                  </div>
                  <div class="col-md-2">
                     <div class="mb-3">
                        ...
                        <select name="mainstyle_id" class="form-control" id="mainstyle_id"  onchange="getSubStyle(this.value)" required disabled>
                           ...
                           {{ $row->mainstyle_id == $QCStitchingInhouseMasterList->mainstyle_id ? 'selected="selected"' : '' }}  
                  ...
                  <div class="col-md-2">
                     <div class="mb-3">
                        ...
                        <select name="substyle_id" class="form-control" id="substyle_id" onchange="getStyle(this.value)" required disabled>
                           ...
                           {{ $row->substyle_id == $QCStitchingInhouseMasterList->substyle_id ? 'selected="selected"' : '' }}
                  ...
                  <div class="col-md-2">
                     <div class="mb-3">
                        ...
                        <select name="fg_id" class="form-control" id="fg_id" required disabled>
                           ...
                           {{ $row->fg_id == $QCStitchingInhouseMasterList->fg_id ? 'selected="selected"' : '' }} 
                           >{{ $row->fg_name }}</option>
                           }
                           @endforeach
                        </select>
                  ...
                  <div class="col-md-2">
                     <div class="mb-3">
                        ...
                        <input type="text" name="style_no" class="form-control" id="style_no" value="{{$QCStitchingInhouseMasterList->style_no}}" required readOnly>
                     </div>
                  </div>
                  <div class="col-md-4">
                     <div class="mb-3">
                        <label for="style_description" class="form-label">Style Description</label>
                        <input type="text" name="style_description" class="form-control" id="style_description" value="{{$QCStitchingInhouseMasterList->style_description}}" required readOnly>
                     </div>
                  </div>
                  <div class="col-md-4">
                     <div class="mb-3">
                        ...
                        <select name="vendorId" class="form-control" id="vendorId" required disabled >
                           ...
                           {{  $rowvendor->ac_code == $QCStitchingInhouseMasterList->vendorId ? 'selected="selected"' : '' }}
                  ...
                  <div class="col-md-4">
                    <div class="mb-3">
                        ...
                        <select name="line_id" class="form-control" id="line_id" required  >
                           <option value="">--Line--</option>
                           @foreach($LineList as  $lines) 
                           <option value="{{ $lines->line_id }}" {{  $lines->line_id == $QCStitchingInhouseMasterList->line_id ? 'selected="selected"' : '' }}>{{ $lines->line_name }}</option> 
                           @endforeach
                        </select>
                    </div>
                 </div>
                        ...
                        <div class="panel panel-default">
                           <div class="panel-heading">
                              <h4 class="panel-title">
                                 <a data-toggle="collapse" data-parent="#accordion" href="#collapse1">Stitching Entry</a>
                              </h4>
                           </div>
                                    ...
                                    <div class="table-wrap">
                                       <div class="table-responsive">
                                          <table id="footable_2" class="table  table-bordered table-striped m-b-0  footable_2">
                                             ...
                                             <tbody>
                                                @if(count($QCStitchingInhouseDetailList)>0)
                                                @php $no=1; $nox=0;   @endphp
                                                @foreach($QCStitchingInhouseDetailList as $List) 
                                                <tr>
                                                   ...
                                                   $SizeQtyList2=explode(',', $QCStitchingInhouseRejectDetailList[$nox]->size_qty_array);
                                                   $nz=0;  
                                                   @endphp
                                                   @foreach($SizeQtyList  as $szQty)
                                                   @php  
                                                     //DB::enableQueryLog();
                                                        $QcData = DB::SELECT("SELECT ((SELECT ifnull(SUM(size_qty_total),0) FROM qcstitching_inhouse_detail WHERE sales_order_no = '".$QCStitchingInhouseMasterList->sales_order_no."' AND color_id = ".$List->color_id.") - (SELECT ifnull(SUM(size_qty_total),0) FROM stitching_inhouse_detail WHERE sales_order_no = '".$QCStitchingInhouseMasterList->sales_order_no."' AND color_id = ".$List->color_id.")) as size_qty_total");
                                                     //dd(DB::getQueryLog());  
                                                       $QcRejectData = DB::SELECT("SELECT ((SELECT ifnull(SUM(size_qty_total),0) FROM qcstitching_inhouse_reject_detail WHERE sales_order_no = '".$QCStitchingInhouseMasterList->sales_order_no."' AND color_id = ".$List->color_id.") - (SELECT ifnull(SUM(size_qty_total),0) FROM stitching_inhouse_detail WHERE sales_order_no = '".$QCStitchingInhouseMasterList->sales_order_no."' AND color_id = ".$List->color_id.")) as size_qty_total");
                                                   
                                                        $qc_qty = isset($QcData[0]->size_qty_total) ? $QcData[0]->size_qty_total : 0; 
                                                        $qc_reject_qty = isset($QcRejectData[0]->size_qty_total) ? $QcRejectData[0]->size_qty_total : 0; 
                                                        
                                                        $min = $qc_qty ? $qc_qty : $szQty;
                                                        $szQty1 = isset($SizeQtyList2[$nz]) ? $SizeQtyList2[$nz] : 0;
                                                        $min1 = $qc_reject_qty ? $qc_reject_qty : $szQty1;
                                                      

                                                   @endphp
                                                   <td><input style="width:80px; float:left;"  name="s@php echo $n; @endphp[]" class="size_id" type="number" id="s@php echo $n; @endphp"  onchange="validateSize(this);" min="0" max="{{ (int)($szQty1 ?? 0) + (int)($szQty ?? 0) }}"  value="{{$szQty}}" required />
                                                      <input style="width:80px; float:left;background-color:#FAACAA;" class="size_id2"  name="s_@php echo $n; @endphp[]"  type="number" id="s_@php echo $n; @endphp"   onchange="validateSize(this);"  min="0"  value="{{$szQty1}}" required />
                                                   </td>
                                                   @php $n=$n+1; $nz=$nz+1; @endphp
                                                   @endforeach
                                                   <td><input type="number" name="size_qty_total[]" class="size_qty_total" value="{{$List->size_qty_total}}" id="size_qty_total" style="width:80px; height:30px; float:left;"  readOnly required />
                                                      <input type="number" name="size_qty_total2[]" class="size_qty_total2" value="{{$QCStitchingInhouseRejectDetailList[$nox]->size_qty_total}}" id="size_qty_total2" style="width:80px; height:30px; float:left; background-color:#FAACAA;" readOnly required />
                                                      <input type="hidden" name="size_qty_array[]"  value="{{$List->size_qty_array}}" id="size_qty_array" style="width:80px; float:left;"  />
                                                      <input type="hidden" name="size_qty_array2[]"  value="{{$QCStitchingInhouseRejectDetailList[$nox]->size_qty_array}}" id="size_qty_array2" style="width:80px; float:left;"  />
                                                      <input type="hidden" name="size_array[]"  value="{{$List->size_array}}" id="size_array" style="width:80px;  float:left;"  />
                                                   </td>
                                                </tr>
                                                @php $no=$no+1; $nox=$nox+1; @endphp
                                                @endforeach
                                                @endif
                                             </tbody>
                                          </table>
               ...
               <div class="row">
                  <div class="col-md-2">
                     <div class="mb-3">
                        ...
                        <input type="text" name="total_qty" class="form-control" id="total_qty" value="{{$QCStitchingInhouseMasterList->total_qty}}" required readOnly>
                     </div>
                  </div>
                  <div class="col-md-2">
                     <div class="mb-3">
                        <label for="vendor_amount" class="form-label">Total Amount</label>
                        <input type="text" name="vendor_amount" class="form-control" id="vendor_amount" value="{{$QCStitchingInhouseMasterList->vendor_amount}}" required readOnly>
                     </div>
                  </div>
                  <div class="col-sm-8">
                     <label for="formrow-inputState" class="form-label">Narration</label>
                     <div class="mb-3">
                        <input type="text" name="narration" class="form-control" id="narration"  value="{{$QCStitchingInhouseMasterList->narration}}" />
                     </div>
                  </div>
               </div>
               <div class="col-sm-6">
                  ...
                  <div class="form-group">
                     ...
                     <a href="{{ Route('QCStitchingInhouse.index') }}" class="btn btn-warning w-md">Cancel</a>
                  </div>
               </div>
         </div>
         </form>
...
<script>
    ...
    function validateSize(row) 
    {
        ...
        var maxQty = $(row).parent().find('.size_id').attr('max');
        var enteredQty = parseInt($(row).val());
        
        var sz1 = $(row).parent().find('.size_id').val();
        var sz2 = $(row).parent().find('.size_id2').val();
        
        var total_size = parseFloat(sz1) + parseFloat(sz2);
    ...
    $(document).on("change", 'input[class^="size_id"]', function (event) 
   {
    ...
    $(document).on("change", 'input[class^="size_id2"]', function (event) 
    {
       var no=1;
       var sales_order_no = $('#sales_order_no').val();
       var sizes= $(this).closest("tr").find('input[name="size_array[]"]').val();
       ...
       var values = [];
       $("#footable_2 tr td  input[class='size_id2']").each(function() 
       {
             values.push($(this).val());
             if(values.length==size_array.length)
             {
                 
               $(this).closest("tr").find('input[name="size_qty_array2[]"]').val(values);
               var sum = values.reduce(function( a,  b)
               {
                   return parseInt(a) + parseInt(b);
               }, 0);
               $(this).closest("tr").find('input[name="size_qty_total2[]"]').val(sum);
               values = [];
             }
           mycalc();   
       });
   ...
   function getStitchingInhouseDetails(vw_code)
   {
...
//   }
   ...
   {   
      ...
      var sum1 = 0;
      var sum2 = 0;
      var amounts = $('.size_qty_total');
      var amounts1 = $('.size_qty_total2'); 
       
      $(amounts).each(function( index ) 
      { 
            sum1 = parseInt(sum1) + parseInt($(this).val()); 
      });
      
      $(amounts1).each(function( index ) 
      {
            sum2 = parseInt(sum2) + parseInt($(this).val());
      });
          
      
      $("#total_qty").val(parseFloat(sum1) + parseFloat(sum2)); 
