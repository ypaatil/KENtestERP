   ...
   <div class="col-xl-12">
      <div class="card">
         <div class="card-body">
            ...
            @endif  
            <form action="{{route('search')}}" method="POST" enctype="multipart/form-data" id="frmData">
                @csrf 
                ...
                <div class="row">
                    ...
                    <div class="col-md-2" style="margin-bottom: 20px;">
                        <select name="searchVendorId"  id="searchVendorId" class="select2" onchange="GetLineData(this.value);" style="width:250px;">
                           ...
                           @endphp
                                <option value="{{ $rowvendor->ac_code }}" {{$selected}} >{{ $rowvendor->ac_short_name }}</option>
                           @endforeach
                        </select>
                    </div>
                    <div class="col-md-2" style="margin-bottom: 20px;">
                        <select name="searchLineId"  id="searchLineId" class="select2" style="width:150px;">
                           <option value="">--Line No.--</option>
                           @foreach($LineList as  $rowline)
                           @php 
                                if($rowline->line_id == $searchLineId)
                                {
                                    $selected1  = 'selected';
                                }
                                else
                           ...
                           @endphp
                                <option value="{{ $rowline->line_id }}" {{$selected1}} >{{ $rowline->line_name }}</option>
                           @endforeach
                        </select>
                    </div>
                     <div class="col-md-2" style="margin-bottom: 20px;">
                         <button type="submit" class="btn btn-warning">Search</button>
                         <a href="{{ route('PPCMaster.index', ['vendorId' => $searchVendorId]) }}" class="btn btn-info">Clear</a>
                     </div>
                </div>
            </form>
                        ...
                        <table id="footable_3" class="table  table-bordered table-striped m-b-0  footable_3">
                           <thead>
                              <tr>
                                 ...
                                 <th nowrap>Sales Order</th>
                                 <th nowrap>Color</th>
                                 <th nowrap>Color Order Qty</th>
                                 <th nowrap>Line No</th>
                                 <th nowrap>No. of M/C</th>
                                 <th nowrap>Avail Mins</th>
                                 <th nowrap>Line Eff.</th>
                                 <th nowrap>SAM</th>
                                 <th nowrap>Prod. Capacity</th>
                                 <th nowrap>Target</th>
                                 <th nowrap>Start Date</th>
                                 <th nowrap>End Date</th>
                                 <th nowrap>Add/Remove </th>   
                              </tr>
                           </thead>
                           <tbody id="endData">
                              @php 
                              ...
                              <tr>
                                 ...
                                 <td>
                                    <select name="vendorId[]"  id="vendorId" class="select2" style="width:250px!important;" disabled  onchange="GetLineList(this);">
                                       <option value="">--Vendors--</option>
                                       @foreach($Ledger1 as  $rowvendor)
                                       {
                                 ...
                                 <td>
                                    <select name="sales_order_no[]"  id="sales_order_no" class="select2" style="width:150px!important;" required onchange="GetColorList(this);" >
                                 ...
                                 <td>
                                    <select name="color_id[]"  id="color_id" class="select2" style="width:250px;" required  onchange="GetColorQty(this);"  >
                                 ...
                                 <td><input type="number" step="any" name="color_order_qty[]" value="{{$ppc->color_order_qty}}" id="color_order_qty" style="width:80px;" readonly /></td>
                                 <td>
                                    <select name="line_id[]"  id="line_id" class="select2" style="width:150px!important;" disabled  >
                                       <option value="">--Line No.--</option>
                                       @foreach($LineList as  $rowline)
                                       {
                                 ...
                                 <td><input type="number" step="any"    name="machine_count[]" value="{{$ppc->machine_count}}" id="machine_count" style="width:80px;" required />
                                    <input type="hidden" step="any"    name="sr_no[]" value="{{$ppc->sr_no}}" id="sr_no" style="width:80px;" />
                                 </td>
                                 <td><input type="number"  step="any"   name="available_mins[]" value="{{$ppc->available_mins}}" id="available_mins" style="width:80px;" readonly /></td>
                                 ...
                                 <td><input type="number"  step="any"   name="sam[]" value="{{$ppc->sam}}" id="sam" style="width:80px;" readonly /></td>
                                 <td><input type="number"  step="any"   name="production_capacity[]" value="{{$ppc->production_capacity}}" id="production_capacity" style="width:80px;" readonly /></td>
                              ...
                              <tr>
                                 ...
                                 <td>
                                    <select name="vendorId[]"  id="vendorId" style="width:250px!important;" class="select2" disabled onchange="GetLineList(this);" >
                                       <option value="">--Vendors--</option>
                                       @foreach($Ledger1 as  $rowvendor)
                                       {
                                       <option value="{{ $rowvendor->ac_code }}"
                                          {{ $rowvendor->ac_code == $searchVendorId ? 'selected="selected"' : '' }} >{{ $rowvendor->ac_short_name }}</option>
                                       }
                                       @endforeach
                                    </select>
                                 </td>
                                 ...
                                 <td><input type="number" step="any" name="color_order_qty[]" value="0" id="color_order_qty" style="width:80px;" readonly /></td>
                                 <td>
                                    <select name="line_id[]"  id="line_id" style="width:150px!important;" class="select2" disabled  >
                                       <option value="">--Line No.--</option>
                                       @foreach($LineList as  $rowline)
                                       {
                                       <option value="{{ $rowline->line_id }}"
                                        {{ $rowline->line_id == $searchLineId ? 'selected="selected"' : '' }}  >{{ $rowline->line_name }}</option>
                                       }
                                       @endforeach
                                    </select>
                                 </td>
                                 <td><input type="text" name="machine_count[]" value="0" id="machine_count" style="width:80px;" required /></td>
                                  <input type="hidden" step="any"    name="sr_no[]" value="0" id="sr_no" style="width:80px;" />
                                 <td><input type="text" name="available_mins[]" value="480" id="available_mins" style="width:80px;" readonly /></td>
                                 <td><input type="text" name="line_efficiency[]" value="0" id="line_efficiency" style="width:80px;" required /></td>
                                 <td><input type="text" name="sam[]" value="0" id="sam" style="width:80px;" readonly /></td>
                                 <td><input type="text" name="production_capacity[]" value="0" id="production_capacity" style="width:80px;" readonly /></td>
...
<script>
    ...
    function GetColorQty(row)
    {
        var tr_code = $(row).parent().parent('tr').find('td select[name="sales_order_no[]"]').val();
        var color_id = $(row).val();
        $.ajax({ 
            url: "{{ route('GetColorWiseQty') }}",
            data:{'tr_code':tr_code,'color_id':color_id},
            success: function(data)
            {
                $(row).parent().parent('tr').find('td input[name="color_order_qty[]"]').val(data.qty);
                $(row).parent().parent('tr').find('td input[name="sam[]"]').val(data.sam);
                CalculateQtyRowPro($(row).parent().parent('tr'));
            }
        });
    }
    ...
    $(document).on("change", 'input[class^="startDate"]', function (event) 
    {
        ...
        var production_capacity = $(this).closest("tr").find('td input[name="production_capacity[]"]').val();
        var target = $(this).closest("tr").find('td input[name="target[]"]').val();
        var endDateObj = $(this).closest("tr").find('td input[name="end_date[]"]');
        var endDate = new Date($(this).closest("tr").find('td input[name="end_date[]"]').val());
        ...
        $(".startDate").each(function( index ) 
        {
            ...
            var p_capacity = $(this).closest("tr").find('td input[name="production_capacity[]"]').val();
            var p_target = $(this).closest("tr").find('td input[name="target[]"]').val();
            var p_endDateObj = $(this).closest("tr").find('td input[name="end_date[]"]');
            var p_endDate = new Date($(this).closest("tr").find('td input[name="end_date[]"]').val());
   ...
   {
       var available_mins = $(obj).closest("tr").find('td input[name="available_mins[]"]').val();
       ...
       t5.type="number";
       t5.step="any"; 
       t5.id = "color_order_qty"+indexcone;
       t5.name="color_order_qty[]";
       t5.value="0";
       t5.readonly = true;
       ...
       t10.value="0";
       t10.setAttribute("readonly", true);
