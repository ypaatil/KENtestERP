...
<div class="row">
         ...
         <div class="card-body">
            ...
            <form action="{{route('PackingInhouse.store')}}" method="POST" enctype="multipart/form-data" id="frmData">
               ...
               <div class="row">
                  ...
                  @endif
                  <div class="col-md-2 hide" id="sz_jump">
                     <div class="form-check form-check-primary mb-5">
                        <input class="form-check-input" type="checkbox" id="is_size_jump" name="is_size_jump"  onchange="SetSurplus();" style="font-size: 30px;margin-top: 30px;margin-left: 0px;" >
                        <label class="form-check-label" for="is_size_jump" style="margin-top: 30px;position: absolute;margin-left: 20px;font-size: 20px;">
                        Size Jump
                        </label>
                     </div>
                  </div>
   ...
   <input type="text" name="vendor_amount" class="form-control" id="vendor_amount" value="" readOnly>
   </div>
   </div>
   <div class="col-sm-6">
...
<script>
    ...
    function SetSurplus() 
    {
        if ($("#is_size_jump").is(':checked')) {
            $('.sj_surplus').each(function(index) {
                var val = parseInt($(this).html(), 10); // get number
    
                var $input = $('.sj_pack_surplus input').eq(index);
    
                // store original max if not stored already
                if (!$input.data('orig-max')) {
                    $input.data('orig-max', $input.attr('max') || '');
                }
            ...
            // restore previous max values
            $('.sj_pack_surplus input').each(function() {
                var orig = $(this).data('orig-max');
                if (orig !== undefined) {
                    if (orig === '') {
                        $(this).removeAttr('max'); // if no max originally
                    } else {
                        $(this).attr('max', orig);
                    }
                }
   ...
   //   });
    ...
    $(document).on("change", 'input[class^="size_id"]', function () {
        var $input = $(this);
        var max = parseFloat($input.attr('max')) || 0;
        var sz_qty = parseFloat($input.val()) || 0;
        var packingType = $("#packing_type_id").val();
    
        // Function: recalc row totals
        function recalcRow($row, main) {
            var sizes = $row.find('input[name="size_array[]"]').val() || "";
            var size_array = sizes.split(',');
            var values = [];
    
            $row.find("td input[class^='size_id']").each(function () {
                values.push(parseInt($(this).val()) || 0);
            });
    
            if (values.length === size_array.length) {
                $row.find('input[name="size_qty_array[]"]').val(values.join(','));
                var sum = values.reduce((a, b) => a + b, 0);
                $row.find('input[name="size_qty_total[]"]').val(sum);
    
                // if (typeof qtyCheck === "function") {
                //     qtyCheck($row, main);
                // }
            }
        }
        ...
        // Reset invalid qty
        if (sz_qty > max && packingType != 3) {
            alert("Quantity should be less than " + max);
            $input.val(0);
            sz_qty = 0;
        }
    
        // Recalculate row totals after change
        var $row = $input.closest("tr");
        recalcRow($row, $input);
    
        // Recalculate overall
        if (typeof mycalc === "function") {
            mycalc();
        }
    });
...
//   }
   ...
   function getFinishingInhouseDetails(vpo_code)
   {
     ...
     $.ajax({
       ...
       $.ajax({
           dataType: "json",
           url: "{{ route('FNSI_GetOrderQtyPackingDemo') }}",
           data:{'vpo_code':vpo_code},
           success: function(data)
           {
                $("#footable_2").html(data.html);
                ...
                if($("#packing_type_id").val() == 4)
                {
                   $("#sz_jump").removeClass("hide"); 
                }
                else
                {
                   $("#sz_jump").addClass("hide"); 
                }
   
                // After replacing #footable_2 content
                setTimeout(function() {
                $("#footable_6 tbody tr").each(function() {
                    let $srcRow = $(this); 
                    let color = $srcRow.find("td:eq(1)").text().trim(); // color name
                    let $srcCells = $srcRow.find("td"); 
                
                    // Find target row in footable_2 by color
                    $("#footable_2 tbody tr").each(function() {
                        let $targetRow = $(this);
                        let selectedColor = $targetRow.find("select[name='color_id[]'] option:selected").text().trim();
                
                        if (selectedColor === color) {
                            // Loop through all size columns except SrNo(0), Color(1), Total(last)
                            $srcCells.each(function(i) {
                                if (i > 1 && i < $srcCells.length - 1) {
                                    let qty = parseInt($(this).text().trim()) || 0;
                
                                    // Find the corresponding input in footable_2
                                    let $td = $targetRow.find("td").eq(i); 
                                    let $input = $td.find("input[type='number']");
                                    if ($input.length) {
                                        
                                        var qty1 = parseInt($td.contents().first()[0].textContent.trim()) || 0;
                                        var qty2 = parseInt(qty) || 0;
                                        
                                        var minQty = Math.min(qty1, qty2);

                                        $input.attr("max", minQty).val(0); // keep default 0, but restrict
                                        // $input.attr("td",$td.contents().first()[0].textContent); // keep default 0, but restrict
                                        
                                        
                                        // $td.contents().first()[0].textContent = qty + " "; // show qty text before input
                                    }
                                }
                            });
                
                            // Update total column
                            // let total = parseInt($srcCells.last().text().trim()) || 0;
                            // let $totalTd = $targetRow.find("td").last();
                            // let $totalInput = $totalTd.find("input[name='size_qty_total[]']");
                            // let $overall = $totalTd.find("input[name='overall_size_qty']");
                
                            // $overall.val(total);
                            // $totalInput.attr("max", total).val(""); 
                            // $totalTd.contents().first()[0].textContent = total + " "; 
                        }
                    });
                });
            }, 200); // delay until DOM updates
