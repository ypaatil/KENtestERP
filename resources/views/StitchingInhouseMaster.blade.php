...
@endif
<form action="{{route('StitchingInhouse.store')}}" method="POST" enctype="multipart/form-data" id="frmData">
@csrf 
<div class="row">
...
<div class="col-md-2">
    <div class="mb-3">
        ...
        <input type="date" name="sti_date" class="form-control" id="sti_date" @if(Session::get('user_type') != 1) min="{{ date('Y-m-d') }}" max="{{ date('Y-m-d') }}" @endif value="{{ date('Y-m-d') }}" required
        {{ Session::get('user_type') == 1 ? '' : 'onclick="validateTodays();" onchange="validateTodays();"' }}>
        ...
        @endforeach
            ...
            <input type="hidden" name="user_type" value="{{ Session::get('user_type') }}" class="form-control" id="user_type">
    </div>
</div>
...
<input type="hidden" name="vendor_rate" class="form-control" id="vendor_rate" value="0"   readOnly>
    ...
    <div class="panel panel-default">
      <div class="panel-heading">
        <h4 class="panel-title">
          <a data-toggle="collapse" data-parent="#accordion1" href="#collapse2">Cut Panel Issue Entry</a>
        </h4>
      </div>
...
<input type="number" step="any" name="total_qty" class="form-control" id="total_qty"   readonly value="0">
</div>
</div>
...
<input type="text" name="total_helpers" class="form-control" id="total_helpers" value="0" required  >
</div>
</div>
...
<input type="text" name="total_workers" class="form-control" id="total_workers" value="0" required  >
</div>
</div>
 
<input type="hidden" name="vendor_amount" class="form-control" id="vendor_amount" value="0"   readOnly>
...
<script>
    ...
    function validateTodays() 
    {
        var sti_date = $("#sti_date").val(); 
        if (!sti_date) return;
    
        var now = new Date();
        var selectedDate = new Date(sti_date);
        ...
        if (selectedDateOnly.getTime() !== nowDateOnly.getTime()) {
            ...
            $("#sti_date").val("").trigger('change');
            return;
        }
    
        // After 10:00 PM, even today is not allowed
        var tenPM = new Date(now.getFullYear(), now.getMonth(), now.getDate(), 22, 0, 0); // 22 = 10 PM
        if (now > tenPM) {
            alert("The previous day's date cannot be considered for production. Please, you can make entries until 10:00 PM on the same day.");
            $("#sti_date").val("").trigger('change');
            return;
        }
    }
...
function getVendorWorkOrderDetails(vw_code)
{
      ...
      $.ajax({
        ...
        $.ajax({
        dataType: "json",
        url: "{{ route('VW_CutPanelGetOrderQty') }}",
        data:{'vw_code':vw_code},
        success: function(data){
