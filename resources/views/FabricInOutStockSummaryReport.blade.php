      ...
      <div class="container-fluid invoice-container">
         ...
         <div class="invoice" id="invoice">
            ...
            <main>
               ...
               <div class="">
                  <h4 class="text-4">
                  <h4 class="text-center mt-2" style="color: #000;font-size:20px;font-weight:bold;">Fabric Inward and Outward Stock Summary Report   {{ date('d-m-Y',strtotime($fdate)) }} and {{ date('d-m-Y',strtotime($tdate)) }}</h4>
               </div>
               <style>
                  .table{
                  display: table;
                     ...
                     <tbody>
                        ...
                        {  
                            ...
                            $FabricInwardDetails =DB::select("SELECT dump_fabric_stock_data.*, (SELECT sum(grn_qty) FROM dump_fabric_stock_data AS df WHERE df.track_name= dump_fabric_stock_data.track_name 
                                AND df.in_date = dump_fabric_stock_data.in_date AND df.in_date = '".$dt."' ) as gq, (SELECT sum(qc_qty) FROM dump_fabric_stock_data AS df WHERE df.track_name= dump_fabric_stock_data.track_name 
                                AND df.in_date = dump_fabric_stock_data.in_date AND df.in_date = '".$dt."' ) as qc
                            ...
                            { 
                                ...
                                if($qc_qty > 0)
                                {
                                    $total_inward_qty += $qc_qty;
                                    $total_inward_value += $qc_qty * $row->rate;
                                }
                                else
                                {
                            ...
                            foreach($FabricInwardDetails1 as $row1)
                            {
                                ...
                                $qc_qty1 = isset($row1->qc) ? $row1->qc : 0; 
