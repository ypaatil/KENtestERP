                         ...
                         <div class="mb-3">
                            ...
                            <select name="cat_id" class="form-control select2" id="cat_id" onchange="GetClassifictionData()" >
                               ...
                               @foreach($Categorylist as $categ)
                                 <option value="{{$categ->cat_id}}"   {{ $categ->cat_id == $cat_id ? 'selected="selected"' : '' }}  >{{$categ->cat_name }}</option>
                               @endforeach
                            </select>
                         </div>
                      </div>
...
<script type="text/javascript">
  ...
  function allocatedStock(row)
  {
      ...
      $.ajax({
            dataType: "json",
            url: "{{ route('GetAllocatedStockData') }}",
            data:{'po_code':po_code,'item_code':item_code,'bom_code':bom_code,'tr_code':tr_code,'tr_date':tr_date,'cat_id':cat_id},
            success: function(data){
              // console.log(data);
  ...
  $(function () {
    ...
    //     ajax: {
    //       url:"{{ route('StockAssociation.index') }}",
    //  	   data:  {'bom_code': ""},
    //  	   dataSrc: 'data'
    //     },
    //     dom: 'lBfrtip',
       ...
       var table = $('#tbl').DataTable({
        ...
        buttons: [
         ...
         columns: [
          ...
          {data: 'item_name', name: 'item_master.item_name'},
          {data: 'item_description', name: 'item_master.item_description'},
          {data: 'avaliable_Stock', name: 'avaliable_Stock', className: 'text-right'},
          {data: 'action', name: 'action'},
         ]
        });
    }
    ...
    function GetClassifictionData()
    {
        ...
        $.ajax({
            dataType: "json",
            url: "{{ route('GetClassifictionTrimsData') }}",
            data:{'cat_id':cat_id,'bom_code':bom_code},
            success: function(data)
            {
    ...
    function GetItemData()
    {
        ...
        $.ajax({
            dataType: "json",
            url: "{{ route('GetItemDataFromDetail') }}",
