...
<div class="card-body">
   ...
   @endif   
    <form action="{{ route('OCR.update',$OCRList[0]->sales_order_no) }}" method="POST" enctype="multipart/form-data">
      ...
      <div class="row">
            <div class="col-md-2">
             <div class="mb-3">
                    ...
                    <input type="text" name="sales_order_no" class="form-control" id="sales_order_no" value="{{$OCRList[0]->sales_order_no}}" readonly />
                </div>
            </div> 
       </div>
       <table class="table" id="ocrmaster">
           ...
           <tbody> 
            @php
                $srno = 1;
            @endphp
            @foreach($OCRList as $row)
               <tr>
                    <td>{{$srno++}}</td>
                    <td><input type="date" name="ocr_date[]" class="form-control" id="ocr_date" value="{{$row->ocr_date}}" /></td>
                    <td><input type="text" name="transport_qty[]" class="form-control" id="transport_qty" value="{{$row->transport_qty}}"  onkeyup="calTransQty()" /></td>
                    <td><input type="file" name="transport_image[]" class="form-control" id="transport_image" value="{{$row->transport_image}}" /></td>
                    <td><input type="text" name="testing_qty[]" class="form-control" id="testing_qty" value="{{$row->testing_qty}}" onkeyup="calTransQty()" /></td>
                    <td><input type="file" name="testing_image[]" class="form-control" id="testing_image" value="{{$row->testing_image}}" /></td>
                    <td nowrap>
