...
<div class="row">
   <div class="col-xl-12">
      <div class="card">
         <div class="card-body">
            <h4 class="card-title mb-4">Transfer Finished Goods Stock  </h4>
            @if ($errors->any())
            <div class="col-md-6">
            ...
            @endif
            <form action="{{route('TransferPackingInhouse.store')}}" method="POST" enctype="multipart/form-data" id="frmData">
               @csrf 
               <div class="row">
                  <div class="col-md-2">
                     <div class="mb-3">
                        <label for="vpo_date" class="form-label">Entry Date</label>
                        <input type="date" name="tpki_date" class="form-control" id="tpki_date" value="{{date('Y-m-d')}}" required readOnly>
                  ...
                  <div class="col-md-2">
                     <div class="mb-3">
                        <label for="main_sales_order_no" class="form-label">Main Sales Order No</label>
                  ...
                  <div class="col-md-2">
                     <div class="mb-3">
                        ...
                        <input type="text" name="order_rate" class="form-control" id="order_rate" value="" required readOnly>
                     </div>
                  </div>
                  ...
                  <div class="panel panel-default">
                     <div class="panel-heading">
                        <h4 class="panel-title">
                           <a data-toggle="collapse" data-parent="#accordion" href="#collapse1">KDPL FG Stock</a>
                        </h4>
                     </div>
   ...
   <input type="text" name="order_amount" class="form-control" id="order_amount" value="" required readOnly>
   </div>
   </div>
   <div class="col-sm-8">
...
<script>
   ...
   function checkDifferentSizeGroup(row)
   {
      ...
      $.ajax({
          dataType: "json",
          url: "{{ route('checkTransferDifferentSizeGroup') }}",
          data:{'sales_order_no':sales_order_no,'Ac_code':Ac_code,'main_sales_order_no':main_sales_order_no},
          ...
          {    
              ...
              { 
                   Swal.fire({
                     icon: 'error',
                     title: 'Oops...',
                     text: data.mismatched+' Size Group Not Matched...!'
                   });
   ...
   $.ajax({
       ...
       $('select[name="sales_order_nos[]"]').select2();
   ...
   function getPackingInhouseDetails()
   {
       ...
       $.ajax({
           dataType: "json",
           url: "{{ route('CheckInvoiceStatus') }}",
           data:{'sales_order_no':sales_order_no},
           success: function(data)
           {
                if(data.cartons != '')
                {
                    alert("The invoices that are pending must be completed for "+data.cartons);
                    $('#main_sales_order_no').select2("destroy");
                    $('#main_sales_order_no').val("");
                    $('#main_sales_order_no').select2();
                }
                else
                {
                       $.ajax({
   ...
   {   
   var main_sales_order_no=$('#main_sales_order_no').val();
   var sales_order_no=$(row).parent().parent('tr').find('select[name^="sales_order_nos[]"]').val();
   ...
   $.ajax({
       ...
       success: function(data){
        $(row).parent().parent('tr').find('select[name^="color_id[]"]').html(data.html);
       }
   });
   ...
   {   
        var main_sales_order_no = $('#main_sales_order_no').val();
        var sales_order_no = $(row).closest('tr').find('select[name^="sales_order_nos[]"]').val();
        var color_id = $(row).closest('tr').find('select[name^="color_id[]"]').val();
        var color_name = $(row).closest('tr').find('select[name^="color_id[]"] option:selected').text().replace("/", "");
        var main_row = $(row).closest('tr');
    
        // Get header names for mapping to size columns
        var headers = [];
        $("#footable_2 thead th").each(function (index) {
            headers[index] = $(this).text().trim().replace(/\s+/g, '_').replace(/[^a-zA-Z0-9_]/g, '');
        });
    
        // Loop through the cells in the current row
        main_row.find("td").each(function (index) {
            var input = $(this).find('input.size_id');
    
            if (input.length > 0) {
                // Build the class name: ColorName_SizeName (e.g., Red_M)
                var cleanHeader = headers[index] || '';
                var cleanColor = color_name.trim().replace(/\s+/g, '').replace(/[^a-zA-Z0-9_]/g, '');
                var cls = cleanColor + '_' + cleanHeader;
    
                // Find FG stock element with this class
                var fg_stock_elem = $("#footable_3").find("." + cls);
    
                if (fg_stock_elem.length > 0) {
                    var fg_stock = fg_stock_elem.html();
                    input.attr('max', fg_stock);
                } else {
                    console.warn("FG Stock not found for class: " + cls);
                }
            }
        });
    
        // Disable sales order dropdown to prevent changes after setting qty
        main_row.find('select[name="sales_order_nos[]"]').attr('disabled', true);
    }
