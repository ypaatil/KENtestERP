                      ...
                      <table class="table" style="height:10vh; ">
                         <thead>
                            <tr>
                                 ...
                                 <th nowrap class="text-center"> Unit - 1 </th>
                                 <th colspan="{{count($lineUnit1Data) + count($lineUnit2Data) + count($lineUnit3Data)}}"></th>
                                 <th nowrap class="text-center"> Unit 1 + Unit 2 + ANSH</th>
                                 <th nowrap class="text-center"> Unit 1 + Unit 2 + ANSH </th>
                                 <th nowrap class="text-center"> Unit 1 + Unit 2 + ANSH </th>
                                 <th nowrap class="text-center"> Unit 1 + Unit 2 + ANSH </th>
                            </tr>  
                            <tr style="background-color:#eee;">
                                   ...
                                   <th class="text-center" nowrap colspan="{{count($lineUnit1Data) + count($lineUnit2Data) + count($lineUnit3Data)}}">SEWING</th>
                            ...
                            <tr>
                                  ...
                                  <th class="text-center" nowrap colspan="{{count($lineUnit3Data)}}">ANSH</th> 
                            </tr> 
                            <tr>
                                ...
                                @endforeach 
                                @foreach($lineUnit3Data as $lines3)
                                     <th nowrap class="text-center">{{$lines3->line_name}}</th>
                                @endforeach 
                            </tr>
                         </thead>
                         <tbody>   
                            ...
                            <tr>
                               ...
                               <td nowrap class="text-right">{{money_format("%!.0n",(isset($stitchingData2[0]->size_qty) ? $stitchingData2[0]->size_qty : 0))}}</td>
                                ...
                                @endphp
                               @endforeach 
                               @foreach($lineUnit3Data as $line3)
                               @php
                                   $stitchingData3 = DB::select("SELECT ifnull(sum(size_qty),0) as size_qty  
                                                    from stitching_inhouse_size_detail2 where sti_date ='".$row."' 
                                                    AND vendorId=69 AND line_id='".$line3->line_id."'");
                               @endphp
                               <td nowrap class="text-right">{{money_format("%!.0n",(isset($stitchingData3[0]->size_qty) ? $stitchingData3[0]->size_qty : 0))}}</td>
                                @php
                                    $TotalSewingQty += $stitchingData3[0]->size_qty;
                                @endphp
                               @endforeach 
  ...
  $('#printPage').click(function(){
     ...
     $(document).ready(function()
     {
            ...
            $(result).each(function(){
              ...
              $('table tr').last().append('<td class="text-right last_total"><strong>'+res1+'</strong></td>');
              $('table tr').last().find('td:eq(2)').html('-');
              var Dynamic_Row = $('#Dynamic_Row').val();
              var CS = parseInt(Dynamic_Row) + parseInt(5);
              var CF = parseInt(Dynamic_Row) + parseInt(7);
              $('table tr').last().find('td:eq('+CS+')').html('-');
              $('table tr').last().find('td:eq('+CF+')').html('-');
            });
            
            // $('.last_total strong').each(function(i)
            // {
            //     if($(this).html() == 0)
            //     {  
            //         $(this).parent().parent('tr').parent('tbody').siblings('thead').each(function(i)
            //         {
            //             console.log($(this).find('tr:last > th:eq('+(i)+')').html("ms"));
            //             //$(this).find('tr:last > th:eq('+(i)+')').html("ms");
            //             //$(this).find('tr td:nth-child('+i+')').html("ms");
            //         });
