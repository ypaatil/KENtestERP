...
<form action="{{route('InwardForPacking.store')}}" method="POST" enctype="multipart/form-data" id="frmData">
@csrf 
<div class="row">
...
<div class="col-md-2">
    <div class="mb-3">
        ...
        <input type="date" name="ifp_date" class="form-control" id="ifp_date" value="{{date('Y-m-d')}}" required  >
...
<div class="col-md-2">
    <div class="mb-3">
        <label for="po_date" class="form-label">Outward For Packing</label>
       ...
       <select name="ofp_code" class="form-control select2" id="ofp_code" required  onChange="getOutwardForPackingDetails(this.value);">
<option value="">--Select--</option>
@foreach($OutwardForPackingList as  $row)
{
    <option value="{{ $row->ofp_code }}">{{ $row->ofp_code }} ({{$row->sales_order_no}})</option>
}
@endforeach
</select>
    </div>
   ...
   <div class="panel-group" id="accordion">
      ...
      <div class="panel-heading">
        <h4 class="panel-title">
          <a data-toggle="collapse" data-parent="#accordion" href="#collapse2">Outward For Packing</a>
        </h4>
      </div>
 ...
 function qtyCheck(row,main)
{
    ...
    function getOutwardForPackingDetails(ofp_code)
    {
        
        $.ajax({
            type: "GET",
            dataType:"json",
            url: "{{ route('getOutwardForPackingDetails') }}",
            data:{'ofp_code':ofp_code},
            success: function(data)
            {
                ...
                $.ajax({
                    dataType: "json",
                    url: "{{ route('GetOutwardForPackingPOQty') }}",
                    data:{'ofp_code':ofp_code, 'sales_order_no':data[0]['sales_order_no']},
                    success: function(data)
                    {
                ...
                $.ajax({
                    dataType: "json",
                    url: "{{ route('GetInwardForPackingPOQty') }}",
                    data:{'ofp_code':ofp_code, 'sales_order_no':data[0]['sales_order_no']},
                    success: function(data)
                    {
