...
<div class="col-xl-12">
   <div class="card">
      <div class="card-body">
         ...
         @endif
            @if(isset($FabricGateEntryMasterList))
            <form action="{{ route('FabricGateEntry.update',$FabricGateEntryMasterList) }}" method="POST" enctype="multipart/form-data" id="frmData">
            @method('put')
            @csrf
            <div class="row">
               <div class="col-md-2">
                  <div class="mb-3">
                     ...
                     <input type="date" name="fge_date" class="form-control" id="fge_date" value="{{$FabricGateEntryMasterList->fge_date}}" required>
                     @foreach($counter_number as  $row)
                     <input type="hidden" name="fge_code" class="form-control" id="fge_code" value="{{ $FabricGateEntryMasterList->fge_code }}"> 
                     @endforeach
               ...
               <div class="col-md-2" style="margin-top: 27px;">
                  <div class="mb-3">
                     <input type="checkbox" value="{{ $FabricGateEntryMasterList->is_manual }}" name="is_manual" id="is_manual" style="width: 56px;height: 25px;" onchange="SetManualItemCode(this);"  {{ $FabricGateEntryMasterList->is_manual == 1 ? 'checked=true' : '' }} disabled />
               ...
               <div class="col-md-2">
                  <div class="mb-3">
                     ...
                     <input type="text" name="po_code2" class="form-control" id="po_code2" value="{{ $FabricGateEntryMasterList->po_code2 }}" onchange="SetPOCode();"   {{ $FabricGateEntryMasterList->is_manual == 1 ? 'required' : 'readonly' }}  >
                  </div>
               </div>
               <div class="col-md-2">
                  <div class="mb-3">
                     <label for="po_code" class="form-label">PO No</label>   
                     <select name="po_code" class="form-select select2" id="po_code1" onchange="SetPOCode();getDetails(this.value);" disabled>
                        <option value="">--Select--</option>
                        @foreach($POList as  $rowpol) 
                        <option value="{{ $rowpol->pur_code  }}"  {{ $rowpol->pur_code == $FabricGateEntryMasterList->po_code ? 'selected="selected"' : '' }} >{{ $rowpol->pur_code }}</option> 
                        @endforeach
                     </select>
                  </div>
               </div>
               ...
               <div class="col-md-2">
                  <div class="mb-3">
                     ...
                     <input type="text" name="dc_no" id="dc_no" class="form-control" id="dc_no" value="{{$FabricGateEntryMasterList->dc_no}}">
                  </div>
               </div>
               <!-- This is for DC date -->
               <div class="col-md-2">
                  <div class="mb-3">
                     ...
                     <input type="date" name="dc_date" id="invoice_date" class="form-control" id="dc_date" value="{{$FabricGateEntryMasterList->dc_date}}">
                  </div>
               </div>
               ...
               <div class="col-md-2">
                  <div class="mb-3">
                     ...
                     <input type="text" name="invoice_no" id="invoice_no" class="form-control" id="invoice_no" value="{{$FabricGateEntryMasterList->invoice_no}}">
                  </div>
               </div>
               <div class="col-md-2">
                  <div class="mb-3">
                     <label for="invoice_date" class="form-label">Invoice Date</label>
                     <input type="date" name="invoice_date" id="invoice_date" class="form-control" id="invoice_date" value="{{$FabricGateEntryMasterList->invoice_date}}" >
                  </div>
               </div>
               <div class="col-md-3">
                  <div class="mb-3">
                     ...
                     <select name="Ac_code" class="form-select" id="Ac_code">
                        ...
                        @foreach($Ledger as  $row) 
                            <option value="{{ $row->ac_code }}" {{ $row->ac_code == $FabricGateEntryMasterList->Ac_code ? 'selected="selected"' : '' }} >{{ $row->ac_name }}</option> 
                        @endforeach
                     </select>
                  </div>
               </div>  
                  ...
                  <div class="mb-3">
                     ...
                     <select name="bill_to" class="form-select" id="bill_to" disabled>
                        @foreach($BillToList as  $row) 
                            <option value="{{ $row->sr_no }}" {{ $row->sr_no == $FabricGateEntryMasterList->bill_to ? 'selected="selected"' : '' }} >{{ $row->trade_name }}({{$row->site_code}})</option> 
                        @endforeach
                     </select>
                  </div>
               </div> 
               <div class="col-md-3">
                  ...
                  <select name="location_id" class="form-select select2" id="location_id" >
                     ...
                     <option value="{{ $row->loc_id }}" {{ $row->loc_id == $FabricGateEntryMasterList->location_id ? 'selected="selected"' : '' }} >{{ $row->location }}</option>
                     @endforeach
                  </select>
               </div> 
               <div class="col-md-2">
                  <div class="mb-3">
                     ...
                     <input type="text" name="lr_no" id="lr_no" class="form-control" id="lr_no" value="{{$FabricGateEntryMasterList->lr_no}}">
                  </div>
               </div> 
               <div class="col-md-2">
                  <div class="mb-3">
                     <label for="transport_name" class="form-label">Transport Name</label>
                     <input type="text" name="transport_name" id="transport_name" class="form-control" value="{{$FabricGateEntryMasterList->transport_name}}" id="transport_name" >
                  </div>
               </div> 
               <div class="col-md-2">
                  <div class="mb-3">
                     <label for="vehicle_no" class="form-label">Vehicle No</label>
                     <input type="text" name="vehicle_no" id="vehicle_no" class="form-control" value="{{$FabricGateEntryMasterList->vehicle_no}}" id="vehicle_no">
                  </div>
               </div>
                     ...
                     <table id="footable_2" class="table  table-bordered table-striped m-b-0  footable_2">
                        ...
                        <tbody>
                            @php
                                $sr_no = 1;
                            @endphp
                           @foreach($FabricGateEntryDetails as $List) 
                           <tr> 
                              ...
                              <td><input type="number" step="any" name="no_of_roll[]" onchange="calc();" value="{{$List->no_of_roll}}" id="no_of_roll" style="width:80px;"/></td> 
                              <td><input type="number" step="any" name="challan_qty[]"  onchange="calAmt(this);calc();DuplicateQty(this);" onkeyup="DuplicateQty(this);" value="{{$List->challan_qty}}" id="challan_qty" style="width:80px;"/></td> 
               ...
               <div class="row">
                  ...
                  <div class="col-md-2">
                     <div class="mb-3">
                        ...
                        <input type="number" readonly step="any" name="total_roll" class="form-control" id="total_roll" value="{{$FabricGateEntryMasterList->total_roll}}">
                     </div>
                  </div>
                  <div class="col-md-2">
                     <div class="mb-3">
                        <label for="total_meter" class="form-label">Total Challan Qty</label>
                        <input type="number" readonly step="any" name="total_meter" class="form-control" id="total_meter" value="{{$FabricGateEntryMasterList->total_meter}}">
                     </div>
                  </div>
                  <div class="col-md-2">
                     <div class="mb-3">
                        <label for="total_received_meter" class="form-label">Total Received Qty</label>
                        <input type="number" readonly step="any" name="total_received_meter" class="form-control" id="total_received_meter" value="{{$FabricGateEntryMasterList->total_received_meter}}">
                     </div>
                  </div>
                  <div class="col-md-2">
                     <div class="mb-3">
                        <label for="total_amount" class="form-label">Total Amount</label>
                        <input type="number" step="any" name="total_amount" readonly class="form-control" id="total_amount" value="{{$FabricGateEntryMasterList->total_amount}}" >
                     </div>
                  </div>
                  </br>
                  <div class="col-md-2">
                     <div class="mb-3">
                        <div class="form-check form-check-primary mb-5">
                            ...
                            <input type="text" name="remark" class="form-control" id="remark" value="{{$FabricGateEntryMasterList->remark}}">
                        </div>
                     </div>
                  </div>
               </div>
