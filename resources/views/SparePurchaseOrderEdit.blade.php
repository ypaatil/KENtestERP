   ...
   <div class="col-xl-12">
      <div class="card">
         <div class="card-body">
            ...
            @endif
                <form action="{{route('SparePurchaseOrder.update',$SpareMasterData)}}" id="form1" method="POST" >
                <input type="hidden" name="type" id="type" class="form-control" value="<?php echo  'SparePurchaseOrder' ?>" /> 
                @method('put')
                @csrf      
               <div class="row">
                  <div class="col-md-2">
                     <div class="mb-3">
                        ...
                        <select name="firm_id" class="form-select" id="firm_id" required>
                           ...
                           <option value="{{ $row->firm_id }}" {{ $row->firm_id == $SpareMasterData->firm_id ? 'selected="selected"' : '' }} >{{ $row->firm_name }}</option>
                           @endforeach
                        </select>
                     </div>
                  </div>
                  <div class="col-md-2">
                     <div class="mb-3">
                        <label for="pur_code" class="form-label">PO Date</label>
                        <input type="hidden" name="pur_code" class="form-control" id="pur_code" value="{{$SpareMasterData->pur_code}}">
                        <input type="date" name="pur_date" class="form-control" id="pur_date" value="{{$SpareMasterData->pur_date}}" required>
                  ...
                  <div class="col-md-3">
                     <div class="mb-3">
                        ...
                        <select name="Ac_code" class="form-select select2" id="Ac_code" onchange="getPartyDetails();" required>
                           ...
                           {
                           <option value="{{ $rowledger->ac_code  }}"  {{ $rowledger->ac_code == $SpareMasterData->Ac_code ? 'selected="selected"' : '' }}  >{{ $rowledger->ac_name }} ({{$rowledger->Bt_name}})</option>
                           }
                           @endforeach
                        </select>
                     </div>
                  ...
                  <div class="col-md-2">
                     <div class="mb-3">
                        ...
                        <select name="tax_type_id" class="form-select" id="tax_type_id"  disabled required>
                           ...
                           {
                           <option value="{{ $rowgst->tax_type_id  }}"  {{ $rowgst->tax_type_id == $SpareMasterData->tax_type_id ? 'selected="selected"' : '' }}  >{{ $rowgst->tax_type_name }}</option>
                           }
                           @endforeach
                        </select>
                     </div>
                  ...
                  <div class="col-md-3">
                     <div class="mb-3">
                        <label for="cat_id" class="form-label">Category Spares</label>
                        <select name="cat_id" class="form-select select2" id="cat_id" onchange="GetClassesList();"> 
                           @foreach($CategroyList as  $row) 
                           <option value="{{ $row->cat_id  }}"  {{ $row->cat_id == $SpareMasterData->cat_id ? 'selected="selected"' : '' }}  >{{ $row->cat_name }}</option> 
                           @endforeach
                        </select>
                     </div>
                  </div>
                  <div class="col-md-4">
                     <div class="mb-3">
                       ...
                       <select name="class_id[]" class="form-select select2" id="class_id"  multiple  disabled>
                            <option value="0">--Select--</option>    
                            
                            @php  $class_ids = explode(',', $SpareMasterData->class_id);   @endphp
                     ...
                     <table id="footable_2" class="table  table-bordered table-striped m-b-0 footable_2 stripe row-border order-column" cellspacing="0" width="100%">
                        ...
                        <tbody id="bomdis">
                          ...
                          @foreach($detailpurchase as $row)
                           <tr>
                              <td><input type="text" name="id[]" value="{{$sr_no++}}" id="id" style="width:50px; height:30px;"/></td>
                              ...
                              <td>
                                 <select name="spare_item_codes[]" class="item" id="spare_item_codes" style="width:250px; height:30px;"  onchange="GetSpareItemMasterData(this);" >
                                    <option value="">--Select Item--</option>
                                    @foreach($itemlist as  $row1)
                                    <option value="{{$row1->spare_item_code}}" {{ $row1->spare_item_code == $row->spare_item_code ? 'selected="selected"' : '' }}> ({{$row1->spare_item_code}}) {{$row1->item_name}}</option>
                                    @endforeach
                                 </select>
                              </td>                               
                              <td nowrap>{{$row->mc_model_name}}</td>
                              <td><input type="text"  name="hsn_code[]" value="{{$row->hsn_code}}" id="hsn_code" style="width:80px;"  readOnly/> </td>
                              <td>
                                 <select name="unit_id[]"  id="unit_id" style="width:100px;  height:30px;"   >
                                    <option value="">--Select Unit--</option>
                                    @foreach($unitlist as  $rowunit)
                                    <option value="{{$rowunit->unit_id}}" {{ $rowunit->unit_id == $row->unit_id ? 'selected="selected"' : '' }}>{{$rowunit->unit_name}}</option>
                                    @endforeach
                                 </select>
                              </td>
                              <td><input type="number" step="any" class="ITEMQTY"   name="item_qtys[]" value="{{$row->item_qty}}" id="item_qty" style="width:80px;  height:30px;" /></td>
                              <td><input type="number" step="any" name="item_rates[]"   value="{{$row->item_rate}}" class="RATE"  id="item_rate" style="width:80px; height:30px;"  required/></td>
                              <td><input type="number" step="any" name="disc_pers[]"  value="{{$row->disc_per}}" class=""  id="disc_per" style="width:80px; height:30px;" required/></td>
                              <td><input type="number" step="any" name="disc_amounts[]"  value="{{$row->disc_amount}}" class=""  id="disc_amount" style="width:80px; height:30px;" required/></td>
                              <td><input type="number" step="any" name="pur_cgsts[]" readOnly value="{{$row->cgst_per}}" class="pur_cgsts"  id="pur_cgst" style="width:80px; height:30px;" required/></td>
                              <td><input type="number" step="any" name="camts[]" readOnly value="{{$row->camt}}" class="GSTAMT"  id="camt" style="width:80px; height:30px;" required/></td>
                              <td><input type="number" step="any" name="pur_sgsts[]" readOnly value="{{$row->sgst_per}}" class=""  id="pur_sgst" style="width:80px; height:30px;" required/></td>
                              <td><input type="number" step="any" name="samts[]" readOnly  value="{{$row->samt}}" class="GSTAMT"  id="samt" style="width:80px; height:30px;" required/></td>
                              <td><input type="number" step="any" name="pur_igsts[]" readOnly value="{{$row->igst_per}}" class=""  id="pur_igst" style="width:80px; height:30px;" required/></td>
                              <td><input type="number" step="any" name="iamts[]" readOnly value="{{$row->iamt}}" class="GSTAMT"  id="iamt" style="width:80px; height:30px;" required/></td>
                              <td><input type="number" step="any" name="amounts[]" readOnly value="{{$row->amount}}" class="GROSS"  id="amount" style="width:80px; height:30px;" required/></td>
                              <td><input type="number" step="any" name="freight_hsn[]" readOnly class="" id="freight_hsn" value="{{$row->freight_hsn}}" style="width:80px; height:30px;"></td>
                              <td><input type="number" step="any" name="freight_amt[]" class="FREIGHT" id="freight_amt" value="{{$row->freight_amt}}" onkeyup="calFreightAmt(this);" style="width:80px; height:30px;"></td>
                              <td><input type="number" step="any" name="total_amounts[]" readOnly class="TOTAMT" value="{{$row->total_amount}}"  id="total_amount" style="width:80px; height:30px;" required/></td>
                           </tr>
                           @endforeach
                        </tbody>
                        <tfoot>
               ...
               <div class="row">
                  <div class="col-md-2">
                     <div class="mb-3">
                        ...
                        <input type="text" readOnly value="{{$SpareMasterData->total_qty}}" name="total_qty" class="form-control" id="total_qty">
                     </div>
                  </div>
                  <div class="col-md-2">
                     <div class="mb-3">
                        <label for="formrow-email-input" class="form-label">Gross Amount</label>
                        <input type="text" name="Gross_amount" class="form-control" id="Gross_amount" onChange="mycalc();" value="{{$SpareMasterData->Gross_amount}}"  readOnly>
                     </div>
                  </div>
                  <div class="col-md-2">
                     <div class="mb-3">
                        <label for="formrow-email-input" class="form-label">GST Amount</label>
                        <input type="text" name="Gst_amount" class="form-control" value="{{$SpareMasterData->Gst_amount}}" id="Gst_amount"  readOnly>
                     </div>
                  </div>
                  <div class="col-md-2">
                     <div class="mb-3">
                        <label for="formrow-email-input" class="form-label">Freight Amount</label>
                        <input type="text" name="totFreightAmt"  class="form-control" id="totFreightAmt" value="{{$SpareMasterData->totFreightAmt}}" readOnly>
                     </div>
                  </div>
                  <div class="col-md-2">
                     <div class="mb-3">
                        <label for="formrow-email-input" class="form-label">Net Amount</label>
                        <input type="text" name="Net_amount" class="form-control" id="Net_amount" value="{{$SpareMasterData->Net_amount}}" readOnly>
                     </div>
                  </div>
               </div>
               <div class="row">
                  <div class="col-md-4">
                     <div class="mb-3">
                        ...
                        <input type="text" name="deliveryAddress" class="form-control" id="deliveryAddress" value="{{$SpareMasterData->deliveryAddress}}">
                     </div>
                  </div>
                  <div class="col-md-2">
                     <div class="mb-3">
                        <label for="delivery_date" class="form-label">Delivery Date</label>
                        <input type="date" name="delivery_date" class="form-control" id="delivery_date" value="{{$SpareMasterData->delivery_date}}" required>
                     </div>
                  </div>
                  <div class="col-md-4">
                     <div class="mb-3">
                        <label for="narration" class="form-label">Narration / Remark</label>
                        <input type="text" name="narration" class="form-control" id="narration" value="{{$SpareMasterData->narration}}">
                     </div>
                  </div>
               </div>
               <div class="row">
                  <div class="col-md-12">
                     <div class="mb-3">
                        ...
                        <textarea name="terms_and_conditions" class="form-control" id="editor1" >
                           {{$SpareMasterData->terms_and_conditions}}
                        </textarea>
                     </div>
                  </div>
               </div>
               <div>
                  <button type="submit" id="Submit" class="btn btn-success w-md" onclick="EnableFields();">Update</button>
