...
<div class="row">
   <div class="col-xl-12">
      <div class="card">
         <div class="card-body">
            <h4 class="card-title mb-4">Rejected PCS Deilvery Challan</h4>
            @if ($errors->any())
            <div class="col-md-6">
            ...
            @endif
            @if(isset($RejectedList))
            <form action="{{ route('RejectedPcsDeliveryChallan.update',$RejectedList) }}" method="POST" enctype="multipart/form-data" id="frmData">
            @method('put')
               @csrf 
               <div class="row">
                  <div class="col-md-2">
                     <div class="mb-3">
                        ...
                        <input type="date" name="rpdc_date" class="form-control" id="rpdc_date" value="{{date('Y-m-d')}}" required readOnly>
                        <input type="hidden" name="userId" value="{{ Session::get('userId') }}" class="form-control" id="formrow-email-input">
                        <input type="hidden" name="rpdc_code" class="form-control" id="rpdc_code" value="{{$RejectedList->rpdc_code}}" required readOnly>
                     </div>
                  </div>
                  <div class="col-md-4">
                     <div class="mb-3">
                        ...
                        <select name="firm_id" class="form-control" id="firm_id" required   >
                           ...
                           {
                           <option value="{{ $row->firm_id }}"  {{ $row->firm_id == $RejectedList->firm_id ? 'selected="selected"' : '' }} >{{ $row->firm_name }}</option>
                           }
                           @endforeach
                        </select>
                     </div>
                  ...
                  <div class="col-md-2">
                     <div class="mb-3">
                        ...
                        <select name="sales_order_no[]" class="form-control select2" id="sales_order_no" required  onChange="getPackingInhouseDetails();" multiple>
                           <option value="">--Sales Order No--</option>
                            @php $sales_orders = explode(',', $RejectedList->sales_order_no);   @endphp
                  ...
                  <div class="col-md-4">
                     <div class="mb-3">
                        ...
                        <select name="vendorId" class="form-control" id="vendorId" required  onChange="getSalesOrderList(this.value);" >
                           <option value="">--Select Vendor--</option>
                           @foreach($Ledger as  $row1)
                           {
                           <option value="{{ $row1->ac_code }}"  {{ $row1->ac_code == $RejectedList->vendorId ? 'selected="selected"' : '' }} >{{ $row1->ac_name }}</option>
                           }
                           @endforeach
                        </select>
                     </div>
                  ...
                  <div class="panel panel-default">
                     <div class="panel-heading">
                        <h4 class="panel-title">
                           <a data-toggle="collapse" data-parent="#accordion" href="#collapse1">Rejecteed Pcs.</a>
                        </h4>
                     </div>
                                    ...
                                    <table id="footable_2" class="table  table-bordered table-striped m-b-0  footable_2">
                                        <thead>
                                         ...
                                         <tbody id="CartonData">
                                                ...
                                                @if(count($RejectedDetailList)>0)
                                                
                                                @php $no=1;$n=1; @endphp
                                                @foreach($RejectedDetailList as $List) 
                                                 
                                                <tr>
                                                    <td>
                                                        <input type="text" name="id" value="@php echo $no; @endphp" id="id" style="width:50px;height:30px;"/>
                                                    </td>
                                                    <td> 
                                                    ...
                                                    <td>
                                                        <input style="width:80px; float:left;" max="{{$szQty}}" min="0" name="s@php echo $n; @endphp[]" class="size_id" type="number" id="s@php echo $n; @endphp" value="{{$szQty}}" required />  
                                                    </td>
                                                    @php $n=$n+1;  @endphp
                                                    @endforeach
                                                    ...
                                                    <td>
                                                        <button type="button" onclick="addNewRow();" id="AButton" class="AButton btn btn-warning pull-left">+</button> <input type="button" class="btn btn-danger pull-left" onclick="deleteRowcone1(this);" value="X" >
                                                    </td>
   ...
   <input type="text" name="total_qty" class="form-control" id="total_qty" value="{{$RejectedList->total_qty}}" required>
   </div>
   </div>
   <div class="col-sm-8">
   <label for="formrow-inputState" class="form-label">Narration</label>
   <div class="mb-3">
   <input type="text" name="narration" class="form-control" id="narration"  value="{{$RejectedList->narration}}" />
   </div>
   </div>
   </div>
   <div class="col-sm-6">
   ...
   <a href="{{ Route('RejectedPcsDeliveryChallan.index') }}" class="btn btn-warning w-md">Cancel</a>
   </div>
   </div>
</div>
</form>
...
//   });
   ...
   function addNewRow()
   {
       ...
       var lastTr = $('#footable_2 tr:last')[0];
       var ls = $(lastTr).find('td:nth-child(1) input').val();
       
        var sales_order_no=$('#sales_order_no').val().join(",");
     $.ajax({
           dataType: "json",
           url: "{{ route('Reject_GetOrdarQtyByRow') }}",
           data:{'sales_order_no':sales_order_no,'srno':ls},
           success: function(data){
   ...
   function getPackingInhouseDetails()
   {
    ...
    //alert(sales_order_no);
       ...
       //     });
           ...
           $.ajax({
           dataType: "json",
           url: "{{ route('Reject_PC_GetOrderQty') }}",
           data:{'sales_order_no':sales_order_no},
           success: function(data){
