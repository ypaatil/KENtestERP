...
<div class="row">
   <div class="col-xl-12">
      <div class="card">
         <div class="card-body">
            <h4 class="card-title mb-4">Packing Rejection Master Edit</h4>
            @if ($errors->any())
            <div class="col-md-6">
            ...
            @endif
            @if(isset($PackingRejectionMasterList))
            <form action="{{ route('PackingRejection.update',$PackingRejectionMasterList) }}" method="POST" enctype="multipart/form-data" id="frmData">
               @method('put')
               @csrf
               ...
               <div class="row">
                  <div class="col-md-2">
                     <div class="mb-3">
                        <label for="qcp_date" class="form-label">Entry Date</label>
                        <input type="date" name="qcp_date" class="form-control" id="qcp_date" value="{{$PackingRejectionMasterList->qcp_date}}" readonly  > 
                        <input type="hidden" name="qcp_code" class="form-control" id="qcp_code" value="{{ $PackingRejectionMasterList->qcp_code }}"> 
                        <input type="hidden" name="userId" value="{{ Session::get('userId') }}" class="form-control" id="formrow-email-input">
                     </div>
                  </div>
                  <div class="col-md-2">
                     <div class="mb-3">
                        <label for="vpo_code" class="form-label">Packing Order</label>
                        <select name="vpo_code" class="form-control select2" id="vpo_code" disabled onChange="getPackingOrderDetails(this.value);">
                           <option value="">--Select--</option>
                           @foreach($packingOrderList as  $row)
                           {
                           <option value="{{ $row->vpo_code }}" {{ $row->vpo_code == $PackingRejectionMasterList->vpo_code ? 'selected="selected"' : '' }} >{{ $row->vpo_code }} ({{$row->sales_order_no}})</option>
                           }
                           @endforeach
                        </select>
                     </div>
                  ...
                  <div class="col-md-2">
                     <div class="mb-3">
                        ...
                        <input type="text" name="sales_order_no" class="form-control" id="sales_order_no" value="{{ $PackingRejectionMasterList->sales_order_no }}" readOnly>
                     </div>
                  </div>
                  <input type="hidden" name="vendor_rate" class="form-control" id="vendor_rate" value="{{ $PackingRejectionMasterList->vendor_rate }}"   readOnly>
                  <div class="col-md-4">
                     <div class="mb-3">
                        ...
                        <select name="Ac_code" class="form-control select2" id="Ac_code" disabled>
                           ...
                           {
                           <option value="{{ $row->ac_code }}" {{ $row->ac_code == $PackingRejectionMasterList->Ac_code ? 'selected="selected"' : '' }} >{{ $row->ac_name }}</option>
                           }
                           @endforeach
                        </select>
                     </div>
                  ...
                  <div class="col-md-2">
                     <div class="mb-3">
                        ...
                        <select name="mainstyle_id" class="form-control " id="mainstyle_id"  onchange="getSubStyle(this.value)" disabled>
                           ...
                           {
                           <option value="{{ $row->mainstyle_id }}"  {{ $row->mainstyle_id == $PackingRejectionMasterList->mainstyle_id ? 'selected="selected"' : '' }} >{{ $row->mainstyle_name }}</option>
                           }
                           @endforeach
                        </select>
                     </div>
                  ...
                  <div class="col-md-2">
                     <div class="mb-3">
                        ...
                        <select name="substyle_id" class="form-control" id="substyle_id" onchange="getStyle(this.value)" disabled>
                           ...
                           {
                           <option value="{{ $row->substyle_id }}" {{ $row->substyle_id == $PackingRejectionMasterList->substyle_id ? 'selected="selected"' : '' }} >{{ $row->substyle_name }}</option>
                           }
                           @endforeach
                        </select>
                     </div>
                  ...
                  <div class="col-md-2">
                     <div class="mb-3">
                        ...
                        <select name="fg_id" class="form-control" id="fg_id" disabled>
                           ...
                           {
                                <option value="{{ $row->fg_id }}" {{ $row->fg_id == $PackingRejectionMasterList->fg_id ? 'selected="selected"' : '' }}>{{ $row->fg_name }}</option>
                           }
                           @endforeach
                        </select>
                     </div>
                  ...
                  <div class="col-md-2">
                     <div class="mb-3">
                        ...
                        <input type="text" name="style_no" class="form-control" id="style_no" value="{{ $PackingRejectionMasterList->style_no }}" readOnly>
                     </div>
                  </div>
                  <div class="col-md-3">
                     <div class="mb-3">
                        <label for="style_description" class="form-label">Style Description</label>
                        <input type="text" name="style_description" class="form-control" id="style_description" value="{{ $PackingRejectionMasterList->style_description }}" readOnly>
                     </div>
                  </div>
                  <div class="col-md-3">
                     <div class="mb-3">
                        ...
                        <select name="vendorId" class="form-control" id="vendorId" disabled>
                           ...
                           {
                           <option value="{{ $rowvendor->ac_code }}"  {{ $rowvendor->ac_code == $PackingRejectionMasterList->vendorId ? 'selected="selected"' : '' }}>{{ $rowvendor->ac_name }}</option>
                           }
                           @endforeach
                        </select>
                     </div>
                        ...
                        <div class="panel panel-default">
                           <div class="panel-heading">
                              <h4 class="panel-title">
                                 <a data-toggle="collapse" data-parent="#accordion" href="#collapse1">Packing Rejection Entry</a>
                              </h4>
                           </div>
                                    ...
                                    <div class="table-wrap">
                                       <div class="table-responsive">
                                          <table id="footable_2" class="table  table-bordered table-striped m-b-0  footable_2">
                                             ...
                                             <tbody>
                                                @if(count($PackingRejectionDetailList)>0)
                                                @php $no=1;$n=1; @endphp
                                                @foreach($PackingRejectionDetailList as $List) 
                                                <tr>
                                                   ...
                                                   <td><input style="width:80px; float:left;"  name="s@php echo $n; @endphp[]" class="size_id" type="number" id="s@php echo $n; @endphp" original="{{$szQty}}" value="{{$szQty}}" max="{{$List->size_qty_total}}" required />  </td>
                                                   @php $n=$n+1;  @endphp
                                                   @endforeach
                                                   <td>
               ...
               <!-- end row -->
                  <div class="row">
                       ...
                       <input type="number" step="any" name="total_qty" class="form-control" id="total_qty" readonly value="{{ $PackingRejectionMasterList->total_qty }}">
                       </div>
                       </div> 
                       <input type="hidden" name="vendor_amount" class="form-control" id="vendor_amount" value="{{ $PackingRejectionMasterList->vendor_amount }}" readOnly>
                       <div class="col-sm-8">
                       <label for="formrow-inputState" class="form-label">Narration</label>
                       <div class="mb-3">
                       <input type="text" name="narration" class="form-control" id="narration" value="{{ $PackingRejectionMasterList->narration }}" />
                       </div>
                       </div>
                </div>
               <div class="col-sm-6">
                  ...
                  <div class="form-group">
                     ...
                     <a href="{{ Route('PackingRejection.index') }}" class="btn btn-warning w-md">Cancel</a>
                  </div>
               </div>
         </div>
         </form>
...
<script>
   ...
   function getPackingOrderDetails(vpo_code)
   { 
       $.ajax({
           type: "GET",
           dataType:"json",
           url: "{{ route('GetPackingOrderDetails') }}",
           ...
           success: function(data)
           {
               ...
               $.ajax({
                   dataType: "json",
                   url: "{{ route('Packing_GetOrderQty') }}",
                   data:{'vpo_code':vpo_code, 'tr_code': data[0]['sales_order_no']},
                   success: function(data)
                   {
               ...
               $.ajax({
                   dataType: "json",
                   url: "{{ route('PackingRejectionOrderQty') }}",
                   data:{'vpo_code':vpo_code, 'tr_code': data[0]['sales_order_no']},
                   success: function(data)
                   {
   ...
   var original =  $(row).find("td input[class='size_id']").attr('original');
