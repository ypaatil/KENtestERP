                               ...
                               <td class="text-center" nowrap id="total_pass">0.00</td>
                                 @php
                                    ...
                                    { 
                                        ...
                                        if(isset($passData[0]->pass_qty) > 0)
                                        {
                                            $pass =  money_format('%!.0n',round($passData[0]->pass_qty));
                                    ...
                                    @endphp
                                    <td class="text-center pass_{{$i}}" style="{{$strong}}" nowrap>{{$pass}} </td>
                                    @php 
                                    }
                                   @endphp
                            </tr>
                               ...
                               <td class="text-center" nowrap id="total_rejected">0.00</td>
                                 @php
                                    ...
                                    { 
                                        ...
                                        if($rejectData[0]->reject_qty != 0)
                                        {
                                            $reje =  money_format('%!.0n',round($rejectData[0]->reject_qty));
                                            $strong2 = 'font-size:16px;font-weight:800';
                                        }
                                        else
                                        {
                                            $reje = "";
                                    ...
                                    @endphp
                                    <td class="text-center reject_{{$i}}" style="{{$strong2}}" nowrap>{{$reje}}</td>
                                    @php 
                                    }
                                   @endphp
                            </tr>
                                   ...
                                   @php
                                        ...
                                        { 
                                              ...
                                              $defectQtyData = DB::select("select ifnull(sum(dhu_details.defect_qty),0) as defect_qty,dhu_details.dhu_sdt_Id,$defect->dhu_so_Id FROM dhu_details 
                                               ...
                                               //dd(DB::getQueryLog());
                                            if(isset($defectQtyData[0]->defect_qty) > 0)
                                            {
                                                $deft =  money_format('%!.0n',round($defectQtyData[0]->defect_qty));
                                                $strong1 = 'font-size:16px;font-weight:800';
                                            }
                                            else
                                            {
                                                $deft = "";
                                        ...
                                        @endphp
                                        <td class="text-center d_{{$defect->dhu_sdt_Id}} v_{{$i}}" style="{{$strong1}}" nowrap>{{$deft}}</td>
                                        @php 
                                        }
                                    @endphp
     ...
     $(document).ready(function()
     {
            ...
            $.each(result, function (i)             
            {
                ...
                if(totalQty > 0)
                {
                    ...
                    $('.total_'+result[i]).css('font-weight','800');
                } 
                else
                {
                    $('.total_'+result[i]).html("");
                }
                ...
                for(var x=1; x<=totalDays; x++)
                {
                   ...
                   if(jQuery.inArray(x, sunDays) === -1)
                   {
                        ...
                        if(vertical > 0)
                        {
                            ...
                            $('.noOfDefect_'+x).css('font-weight','800');
                        } 
                        else
                        {
                            $('.noOfDefect_'+x).html(vertical.toFixed());
                        ...
                        if(rejected_dhu > 0)
                        {
                            ...
                            $('.dhu_rejected_'+x).css('font-weight','800');
                        } 
                        else
                        {
                            $('.dhu_rejected_'+x).html("");
                        }
                ...
                $('#total_defects').html(de_horizontal.toFixed()).css('font-weight','800');
                $('#total_rejected').html(reject_horizontal.toFixed()).css('font-weight','800');
                $('#total_pass').html(pass_horizontal.toFixed()).css('font-weight','800');
                $('#total_dhu_rejected').html(dhu_reject_horizontal.toFixed(2)+'%').css('font-weight','800');
                $('#total_dhu').html(dhu_horizontal.toFixed(2)+'%').css('font-weight','800');
            });
      });
