         ...
         <div class="card-body">
            <table id="datatable-buttons" class="table table-bordered dt-responsive nowrap w-100">
               ...
               <tbody>
                  ...
                  @php
                    $riskPath = 'https://kenerp.com/uploads/FGImages/'.$row->risk_assessment;
                    $stylePath = 'https://kenerp.com/uploads/FGImages/'.$row->style_feasibility;
                    $skillPath = 'https://kenerp.com/uploads/FGImages/'.$row->skill_mapping;
                  ...
                  <tr>
                     ...
                     <td>{{ $row->username }}</td>
                     <td nowrap>
                         <a class="btn btn-{{$color}} btn-sm" href="javascript:void(0);" fg_id="{{$row->fg_id}}" status="{{$row->status}}" onclick="ChangeStatus(this);" title="status">
                            {{$status}}
                        </a>
                    </td>
   ...
   {  
          ...
          Swal.fire({
            ...
            if (result.isConfirmed) 
            {
                 $.ajax({
                 type: "GET", 
                 url: "{{ route('changeFGCategoryStatus') }}",
                 data:{'fg_id':fg_id,'status':status},
                 success: function(data)
                 {
                     Swal.fire(
                        'Closed!',
                        'Your Style status has been changed.',
                        'success'
                      )
