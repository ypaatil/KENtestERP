   ...
   <div class="col-xl-12">
      <div class="card">
         <div class="card-body">
            ...
            @endif
            <form action="{{route('QCStitchingInhouse.store')}}" method="POST" enctype="multipart/form-data" id="frmData">
               @csrf 
               <div class="row">
                  <div class="col-md-2">
                     <div class="mb-3">
                        <label for="qcsti_date" class="form-label">Entry Date</label>
                        <input type="date" name="qcsti_date" class="form-control" id="qcsti_date" value="{{date('Y-m-d')}}" required  >
                  ...
                  <div class="col-md-2">
                     <div class="mb-3">
                        ...
                        <select name="vw_code" class="form-control select2" id="vw_code" required  onChange="getStitchingInhouseDetails(this.value);">
                  ...
                  <div class="panel panel-default">
                     <div class="panel-heading">
                        <h4 class="panel-title">
                           <a data-toggle="collapse" data-parent="#accordion" href="#collapse1">QC Stitching Entry</a>
                        </h4>
                     </div>
...
<script>
           ...
           var size_array = sizes.split(',');
             ...
             if(values.length==size_array.length)
             {
               ...
               $(this).closest("tr").find('input[name="size_qty_total[]"]').val(sum);
                 qtyCheck($(this).closest("tr"),1,main_size_id);
                   values = [];
             }
   ...
   $(document).on("keyup", 'input[class^="size_id2"]', function (event) 
   {
      var no=1;
       var main_size_id = $(this); 
       var sales_order_no = $('#sales_order_no').val();
       ...
       var size_array = sizes.split(',');
         ...
         if(values.length==size_array.length)
         {
           ...
           qtyCheck($(this).closest("tr"),2,main_size_id);
               values = [];
         }
     ...
     function qtyCheck(row,ele,main)
     {
       var totalQty = $(row).find("td input[name='overall_size_qty']").val();
       var size_qty_total = $(row).find("td input[name='size_qty_total[]']").val();
       var size_qty_total2 = $(row).find("td input[name='size_qty_total2[]']").val();
       var size_row =  $(row).find("td input[class='size_id']");
       var size_row2 =  $(row).find("td input[class='size_id2']");
       var total_row = parseFloat(size_qty_total) + parseFloat(size_qty_total2);

       if(parseFloat(total_row) > parseFloat(totalQty))
       {
           ...
           else
           {
               ...
               var sizes= $(row).find('input[name="size_array[]"]').val();
               var size_array = sizes.split(',');
               $(size_row2).each(function() 
               {
                 values1.push($(this).val());
                 if(values1.length==size_array.length)
                 {
                       $(row).find('input[name="size_qty_array2[]"]').val(values1);
                       var sum = values1.reduce(function( a,  b)
                       {
                           return parseInt(a) + parseInt(b);
                       }, 0);
                       $(row).find('input[name="size_qty_total2[]"]').val(sum);
                       values = [];
                 }
               });
           }
