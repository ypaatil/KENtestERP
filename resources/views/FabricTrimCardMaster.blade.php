...
<div class="card">
<div class="card-body">
<h4 class="card-title mb-4">Fabric Trim Card</h4>
@if ($errors->any())
...
<form action="{{route('FabricTrimCard.store')}}" method="POST" enctype="multipart/form-data">
@csrf 
<div class="row">
<div class="col-md-2">
    <div class="mb-3">
    <label for="ftc_date" class="form-label"> Date</label>
    <input type="date" name="ftc_date" class="form-control" id="ftc_date" value="{{date('Y-m-d')}}" required>
    @foreach($counter_number as  $row)
    <input type="hidden" name="ftc_code" class="form-control" id="ftc_code" value="{{ 'FTC'.'-'.$row->tr_no }}">
    <input type="hidden" name="c_code" class="form-control" id="c_code" value="{{ $row->c_code }}">
    @endforeach
    <input type="hidden" name="userId" value="{{ Session::get('userId') }}" class="form-control" id="formrow-email-input">
    </div>
@php
 session()->put('FabricTrimCard','1');
@endphp
</div>
...
<!--        <input type="text" name="job_code" class="form-control" id="job_code" value="" onfocusout="" required>-->
<!--    </div>-->
<!--</div>-->
...
<!--<input type="text" name="job_code" class="form-control" id="job_code" value="" required onfocusout="getJobCardDetails(this.value);" >-->
 
<select name="sales_order_no" class="form-select select2" id="sales_order_no" onchange="getJobCardDetails(1);" required>
 ...
 function getJobCardDetails(code)
{
   ...
   else
   {
    ...
    $.ajax({
            ...
            data:{'sales_order_no':sales_order_no, 'style_no':style_no},
            ...
            $("#sales_order_no").val(data[0]["po_code"]);
            var q=data[0]["po_code"];
            $('#select2-sales_order_no-container').html(q);
            ...
            data:{'sales_order_no':data[0]["po_code"]},
    ...
    $.ajax({
            ...
            dataType:"json",
            url: "{{ route('Average') }}",
            data:{'sales_order_no':sales_order_no,'color_id':color_id,'average':average,'part_id':part_id},
            success: function(data){
...
$("table.footable_2").on('change', 'input[name^="averages"]', function (event)   
    ...
    $.ajax({
            ...
            data:{'sales_order_no':sales_order_no,'body_color_id':body_color_id,'trim_color_id':trim_color_id,'average':average,'part_id':part_id},
            success: function(data){
...
function CalculateRow(row)
	{ 
        ...
        $.ajax({
            ...
            dataType:"json",
            url: "{{ route('JobCardDetail') }}",
            data:{'sales_order_no':sales_order_no, 'color_id':color_id,'part_id':part_id,},
            success: function(data){
