...
<div class="card">
<div class="card-body">
<h4 class="card-title mb-4">Finishing Inhouse</h4>
@if ($errors->any())
...
@if(isset($FinishingInhouseMasterList))
<form action="{{ route('FinishingInhouse.update',$FinishingInhouseMasterList) }}" method="POST" enctype="multipart/form-data" id="frmData">
@method('put')
@csrf
...
<div class="col-md-2">
    <div class="mb-3">
        <label for="fns_date" class="form-label">Entry Date</label>
        <input type="date" name="fns_date" class="form-control" id="fns_date" value="{{$FinishingInhouseMasterList->fns_date}}" required  >
        <input type="hidden" name="c_code" class="form-control" id="c_code" value="{{ $FinishingInhouseMasterList->c_code }}">
        <input type="hidden" name="fns_code" class="form-control" id="fns_code" value="{{$FinishingInhouseMasterList->fns_code}}" required readOnly>
...
@foreach($ProcessList as  $row)
{
    ...
    {{ $row->process_id == $FinishingInhouseMasterList->process_id ? 'selected="selected"' : '' }} 
...
@foreach($VendorPurchaseOrderList as  $row)
{
    <option value="{{ $row->vpo_code }}"
      ...
      {{ $row->vpo_code == $FinishingInhouseMasterList->vpo_code ? 'selected="selected"' : '' }} 
...
<input type="text" name="sales_order_no" class="form-control" id="sales_order_no" value="{{$FinishingInhouseMasterList->sales_order_no}}" required readOnly>
</div>
</div>
...
@foreach($BuyerList as  $row)
{
    <option value="{{ $row->ac_code }}"
     {{ $row->ac_code == $FinishingInhouseMasterList->Ac_code ? 'selected="selected"' : '' }} 
    >{{ $row->ac_name }}</option>
}
@endforeach
</select>
...
<input type="text" name="vendor_rate" class="form-control" id="vendor_rate" value="{{$FinishingInhouseMasterList->vendor_rate}}" required readOnly>
</div>
</div>   
...
@foreach($MainStyleList as  $row)
{
    ...
    {{ $row->mainstyle_id == $FinishingInhouseMasterList->mainstyle_id ? 'selected="selected"' : '' }}  
...
@foreach($SubStyleList as  $row)
{
    <option value="{{ $row->substyle_id }}"
     {{ $row->substyle_id == $FinishingInhouseMasterList->substyle_id ? 'selected="selected"' : '' }}
...
@foreach($FGList as  $row)
{
    ...
    {{ $row->fg_id == $FinishingInhouseMasterList->fg_id ? 'selected="selected"' : '' }} 
    >{{ $row->fg_name }}</option>
}
@endforeach
</select>
...
<input type="text" name="style_no" class="form-control" id="style_no" value="{{$FinishingInhouseMasterList->style_no}}" required readOnly>
</div>
</div>
...
<input type="text" name="style_description" class="form-control" id="style_description" value="{{$FinishingInhouseMasterList->style_description}}" required readOnly>
</div>
</div>
...
@foreach($Ledger as  $rowvendor)
{
    ...
    {{  $rowvendor->ac_code == $FinishingInhouseMasterList->vendorId ? 'selected="selected"' : '' }}
...
@if(count($FinishingInhouseDetailList)>0)

@php $no=1;$n=1; @endphp
@foreach($FinishingInhouseDetailList as $List) 
...
<input type="text" name="total_qty" class="form-control" id="total_qty" value="{{$FinishingInhouseMasterList->total_qty}}" required readOnly>
</div>
</div>
...
<input type="text" name="vendor_amount" class="form-control" id="vendor_amount" value="{{$FinishingInhouseMasterList->vendor_amount}}" required readOnly>
...
<div class="mb-3">
<input type="text" name="narration" class="form-control" id="narration"  value="{{$FinishingInhouseMasterList->narration}}" />
</div>
</div>
...
<a href="{{ Route('FinishingInhouse.index') }}" class="btn btn-warning w-md">Cancel</a>
</div>
</div>
...
function getVendorProcessOrderDetails(vpo_code)
{
      ...
      $.ajax({
        ...
        $.ajax({
        dataType: "json",
        url: "{{ route('vpo_GetFinishedPOQty') }}",
        data:{'vpo_code':vpo_code},
        success: function(data){
        $("#footable_2").html(data.html);
        }
