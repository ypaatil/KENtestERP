                  ...
                  <table class="table table-bordered text-1 table-sm" style="height:10vh; ">
                     <thead>
                        ...
                        <tr style="text-align:center; border: 1px  solid;">
                           ...
                           <th rowspan="2">SAM</th>
                           <th rowspan="2">Total Helpers</th>
                           <th rowspan="2">Total Operators</th>
                     ...
                     <tbody>
                        ...
                        sum(total_qty) as  qty, stitching_inhouse_master.sales_order_no,line_master.line_name, sum(total_workers)  as total_workers,sum(total_helpers)  as total_helpers,
                        ...
                        (select AVG(total_workers) from stitching_inhouse_detail 
                        INNER JOIN stitching_inhouse_master on stitching_inhouse_master.sti_code=stitching_inhouse_detail.sti_code
                        inner join line_master on line_master.line_id=stitching_inhouse_master.line_id
                        where stitching_inhouse_master.vendorId='".$vendorId."' ".$lineData." and 
                    ...
                    //DB::enableQueryLog();
                       ...
                       // dd(DB::getQueryLog());
                        ...
                        if($totalWorkers > 0)
                        {
                            $TotalOperator = money_format('%!.0n',round((($totalPMin)/(($line->total_helpers+$line->total_workers) * 480)),2) * 100);
                         
                            $TotalOperator12 = $totalWorkers;
                       
                            $EffMonth = money_format('%!.0n',round((($QtyData[0]->qty * sprintf('%.2f', $SAM[0]->sam))/($totalWorkers * 480)),2)*100);
                            $totalLEff = (round((($line->qty * sprintf('%.2f', $SAM[0]->sam))/($totalWorkers * 480)),2)*100);
                            $totalEMonth = round((($QtyData[0]->qty * sprintf('%.2f', $SAM[0]->sam))/($totalWorkers * 480)),2)*100;
                        }
                        else
                        {
                            $TotalOperator = 0;
                        ...
                        if($TodaysPcs * $SAM[0]->sam > 0 && ($line->Todaysworkers * 480) > 0)
                        {
                            $total_avaliable_min = round((($TodaysPcs * sprintf('%.2f', $SAM[0]->sam))/($line->Todaysworkers * 480)),2)*100;
                        }
                        else
                        {
                             $total_avaliable_min = 0;
                        }
                        
                           
                        if(($line->qty * $SAM[0]->sam) > 0 && ($totalWorkers * 480) > 0)
                        {
                            $lineEff = round((($line->qty * sprintf('%.2f', $SAM[0]->sam))/($totalWorkers * 480)),2)*100;
                        }
                        else
                        {
                            $lineEff = 0;
                        ...
                        <tr style="  border: 1px  solid;">
                           ...
                           <td style="text-align:right;">{{ sprintf('%.2f', $avgSAM) }}</td>
                           <td style="text-align:right;">{{$line->total_helpers ? $line->total_helpers : 0}}</td>
                           <td style="text-align:right;">{{round($line->total_workers,2)}}</td>
                           <td style="text-align:right;">{{$line->total_helpers+$line->total_workers}}</td>
                           <td style="text-align:right;">{{money_format('%!.0n',$totalPMin)}}</td>
                           <td style="text-align:right;">{{money_format('%!.0n',($line->total_helpers+$line->total_workers) * 480)}}</td>
                     ...
                     <tfoot>  
                        <tr>    
                            ...
                            @php
                                ...
                                {
                                    $totalOp = number_format(($totalProducedMin/$totalQty),2); 
                                }
                                else
                                {
                                    $totalOp = 0;
                            ...
                            <th class="text-right">{{$allTotalHelpers}}</th>
                            <th class="text-right">{{$allTotalWorkers}}</th>
                            <th class="text-right">{{$allTotalHelpers+$allTotalWorkers}}</th>
