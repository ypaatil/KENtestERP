...
<div class="row">
   <div class="col-xl-12">
      <div class="card">
         <div class="card-body">
            <h4 class="card-title mb-4">Cut Panel GRN</h4>
            @if ($errors->any())
            <div class="col-md-6">
            ...
            @endif
            @if(isset($CutPanelGRNMasterList))
            <form action="{{ route('CutPanelGRN.update',$CutPanelGRNMasterList) }}" method="POST" enctype="multipart/form-data" id="frmData">
               @method('put')
               @csrf
               <div class="row">
                  <div class="col-md-2">
                     <div class="mb-3">
                        <label for="cpg_date" class="form-label">Entry Date</label>
                        <input type="date" name="cpg_date" class="form-control" id="cpg_date" value="{{$CutPanelGRNMasterList->cpg_date}}" required  >
                        <input type="hidden" name="c_code" class="form-control" id="c_code" value="{{ $CutPanelGRNMasterList->c_code }}">
                        <input type="hidden" name="cpg_code" class="form-control" id="cpg_code" value="{{$CutPanelGRNMasterList->cpg_code}}" readOnly>
                  ...
                  <div class="col-md-2">
                     <div class="mb-3">
                        ...
                        <select name="vpo_code" class="form-control select2" id="vpo_code" required  onChange="getVendorProcessOrderDetails(this.value);" disabled>
                           <option value="">--Vendor Work Order No--</option>
                           @foreach($VendorProcessOrderList as  $row)
                           {
                           <option value="{{ $row->vpo_code }}"
                           {{ $row->vpo_code == $CutPanelGRNMasterList->vpo_code ? 'selected="selected"' : '' }} 
                  ...
                  <div class="col-md-2">
                     <div class="mb-3">
                        ...
                        <input type="text" name="sales_order_no" class="form-control" id="sales_order_no" value="{{$CutPanelGRNMasterList->sales_order_no}}" readOnly>
                     </div>
                  </div>
                  <div class="col-md-4">
                     <div class="mb-3">
                        ...
                        <select name="Ac_code" class="form-control select2" id="Ac_code"  disabled>
                           ...
                           {{ $row->ac_code == $CutPanelGRNMasterList->Ac_code ? 'selected="selected"' : '' }} 
                           >{{ $row->ac_name }}</option>
                           }
                           @endforeach
                        </select>
                  ...
                  <div class="col-md-2">
                     <div class="mb-3">
                        ...
                        <input type="text" name="vendor_rate" class="form-control" id="vendor_rate" value="{{$CutPanelGRNMasterList->vendor_rate}}" readOnly>
                     </div>
                  </div>
                  <div class="col-md-2">
                     <div class="mb-3">
                        ...
                        <select name="mainstyle_id" class="form-control" id="mainstyle_id"  onchange="getSubStyle(this.value)" disabled>
                           ...
                           {{ $row->mainstyle_id == $CutPanelGRNMasterList->mainstyle_id ? 'selected="selected"' : '' }}  
                  ...
                  <div class="col-md-2">
                     <div class="mb-3">
                        ...
                        <select name="substyle_id" class="form-control" id="substyle_id" onchange="getStyle(this.value)"  disabled>
                           ...
                           {{ $row->substyle_id == $CutPanelGRNMasterList->substyle_id ? 'selected="selected"' : '' }}
                  ...
                  <div class="col-md-2">
                     <div class="mb-3">
                        ...
                        <select name="fg_id" class="form-control" id="fg_id"   disabled>
                           ...
                           {{ $row->fg_id == $CutPanelGRNMasterList->fg_id ? 'selected="selected"' : '' }} 
                           >{{ $row->fg_name }}</option>
                           }
                           @endforeach
                        </select>
                  ...
                  <div class="col-md-2">
                     <div class="mb-3">
                        ...
                        <input type="text" name="style_no" class="form-control" id="style_no" value="{{$CutPanelGRNMasterList->style_no}}"   readOnly>
                     </div>
                  </div>
                  <div class="col-md-4">
                     <div class="mb-3">
                        <label for="style_description" class="form-label">Style Description</label>
                        <input type="text" name="style_description" class="form-control" id="style_description" value="{{$CutPanelGRNMasterList->style_description}}"   readOnly>
                     </div>
                  </div>
                  <div class="col-md-4">
                     <div class="mb-3">
                        ...
                        <select name="vendorId" class="form-control" id="vendorId" required disabled >
                           ...
                           {{  $rowvendor->ac_code == $CutPanelGRNMasterList->vendorId ? 'selected="selected"' : '' }}
                  ...
                  <div class="col-md-2">
                    <div class="mb-3">
                        <label for="cu_code" class="form-label">Fabric Cutting GRN</label>
                        <select name="cu_code" class="form-control select2" id="cu_code" >
                        <option value="">--Select--</option>
                        @foreach($cuttingList as  $cutting)
                            <option value="{{ $cutting->cu_code }}" {{  $cutting->cu_code == $CutPanelGRNMasterList->cu_code ? 'selected="selected"' : '' }} >{{ $cutting->cu_code }}</option>
                        @endforeach
                        </select>
                    </div>
                  </div>
                        ...
                        <div class="panel panel-default">
                           <div class="panel-heading">
                              <h4 class="panel-title">
                                 <a data-toggle="collapse" data-parent="#accordion" href="#collapse1">Cut Piece GRN</a>
                              </h4>
                           </div>
                                    ...
                                    <div class="table-wrap">
                                       <div class="table-responsive">
                                          <table id="footable_2" class="table  table-bordered table-striped m-b-0  footable_2">
                                             ...
                                             <tbody>
                                                @if(count($CutPanelGRNDetailList)>0)
                                                @php $no=1;$n=1; @endphp
                                                @foreach($CutPanelGRNDetailList as $List) 
                                                <tr>
                                                   ...
                                                   @php  
                                                     //DB::enableQueryLog();
                                                        $cutPanelData = DB::SELECT("SELECT ((SELECT ifnull(SUM(size_qty_total),0) FROM cut_panel_issue_detail WHERE sales_order_no = '".$CutPanelGRNMasterList->sales_order_no."' AND color_id = ".$List->color_id.") - (SELECT ifnull(SUM(size_qty_total),0) FROM cut_panel_grn_size_detail WHERE sales_order_no = '".$CutPanelGRNMasterList->sales_order_no."' AND color_id = ".$List->color_id.")) as size_qty_total");
                                                     //dd(DB::getQueryLog());  
                                                    
                                                        $cut_panel_qty = isset($cutPanelData[0]->size_qty_total) ? $cutPanelData[0]->size_qty_total : 0; 
                                                        
                                                        $min = $cut_panel_qty ? $cut_panel_qty : $szQty;
                                                      

                                                   @endphp
                                                   <td ><input style="width:80px; float:left;"  name="s@php echo $n; @endphp[]" onchange="validateSize(this);" min="{{ max($min, 0) }}" max="{{$szQty}}" class="size_id" type="number" id="s@php echo $n; @endphp" value="{{$szQty}}" required />  </td>
                                                   @php $n=$n+1;  @endphp
                                                   @endforeach
               ...
               <div class="row">
                  <div class="col-md-2">
                     <div class="mb-3">
                        ...
                        <input type="text" name="total_qty" class="form-control" id="total_qty" value="{{$CutPanelGRNMasterList->total_qty}}"  readOnly>
                     </div>
                  </div>
     
                  <div class="col-md-2">
                        <div class="mb-3">
                            <label for="cons" class="form-label">Cons. (Mtr)</label>
                            <input type="number" step="any" name="cons" class="form-control" id="cons" value="{{$CutPanelGRNMasterList->cons}}" onkeyup="CalFabricUsed();">
                        </div>
                  </div>
                         
                  <div class="col-md-2">
                        <div class="mb-3">
                            <label for="fabric_used" class="form-label">Fabric Used</label>
                            <input type="number" step="any" name="fabric_used" class="form-control" id="fabric_used" value="{{$CutPanelGRNMasterList->fabric_used}}" readOnly>
                        </div>
                  </div> 
                  ...
                  <div class="col-md-2">
                     <div class="mb-3">
                        ...
                        <input type="text" name="vendor_amount" class="form-control" id="vendor_amount" value="{{$CutPanelGRNMasterList->vendor_amount}}" readOnly>
                     </div>
                  </div>
                  <div class="col-sm-8">
                     <label for="formrow-inputState" class="form-label">Narration</label>
                     <div class="mb-3">
                        <input type="text" name="narration" class="form-control" id="narration"  value="{{$CutPanelGRNMasterList->narration}}" />
                     </div>
                  </div>
               </div>
               <div class="col-sm-6">
                  ...
                  <div class="form-group">
                     ...
                     <a href="{{ Route('CutPanelGRN.index') }}" class="btn btn-warning w-md">Cancel</a>
                  </div>
               </div>
         </div>
         </form>
...
<script>
    ...
    function CalFabricUsed()
    {
        var cons = $("#cons").val();
        var total_qty = $("#total_qty").val();
        $("#fabric_used").val(total_qty*cons);
    }
