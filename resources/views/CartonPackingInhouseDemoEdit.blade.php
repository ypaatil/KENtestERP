   ...
   <div class="col-xl-12">
      <div class="card">
         <div class="card-body">
            ...
            @endif
            @if(isset($CartonPackingInhouseMasterList))
            <form action="{{ route('CartonPackingInhouse.update',$CartonPackingInhouseMasterList) }}" method="POST" enctype="multipart/form-data" id="frmData">
               @method('put')
               @csrf
               <div class="row">
                  <div class="col-md-2">
                     <div class="mb-3">
                        ...
                        <input type="date" name="cpki_date" class="form-control" id="cpki_date" value="{{$CartonPackingInhouseMasterList->cpki_date}}" required>
                        <input type="hidden" name="c_code" class="form-control" id="c_code" value="{{ $CartonPackingInhouseMasterList->c_code }}">
                        <input type="hidden" name="endflag" class="form-control" id="endflag" value="{{ $CartonPackingInhouseMasterList->endflag }}">
                        <input type="hidden" name="cpki_code" class="form-control" id="cpki_code" value="{{$CartonPackingInhouseMasterList->cpki_code}}" required readOnly>
                  ...
                  <div class="col-md-1 hide">
                     <div class="mb-3">
                        ...
                        <select name="firm_id" class="form-control" id="firm_id">
                           ...
                           {{ $row->firm_id == $CartonPackingInhouseMasterList->firm_id ? 'selected="selected"' : '' }}  
                  ...
                  <div class="col-md-3">
                     <div class="mb-3">
                        ...
                        <select name="Ac_code" class="form-control" id="Ac_code" required  disabled>
                           ...
                           {{ $row->ac_code == $CartonPackingInhouseMasterList->Ac_code ? 'selected="selected"' : '' }} 
                           >{{ $row->ac_name }}</option>
                           }
                           @endforeach
                        </select>
                  ...
                  <div class="col-md-1">
                     <div class="mt-5"> 
                        <input type="checkbox" name="isRTV" id="isRTV" style="font-size:15px;" {{ $CartonPackingInhouseMasterList->isRTV == 1 ? 'checked="checked"' : '' }}  />
                  ...
                  <div class="col-md-4">
                     <div class="mb-5">
                        ...
                        <select name="sales_order_no[]" class="form-control select2" id="sales_order_no" required onChange="checkDifferentSizeGroup(this);"  multiple style="width:100%;height:50%;">
                           <option value="">--Sales Order No--</option>
                           @php $sales_orders = explode(',', $CartonPackingInhouseMasterList->sales_order_no);   @endphp
                  ...
                  <input type="hidden" name="mainstyle_id" class="form-control" id="mainstyle_id" value="{{$CartonPackingInhouseMasterList->mainstyle_id}}">
                  <input type="hidden" name="substyle_id" class="form-control" id="substyle_id" value="{{$CartonPackingInhouseMasterList->substyle_id}}"> 
                  <div class="col-md-2 hide">
                     <div class="mb-3">
                        ...
                        <select name="fg_id" class="form-control" id="fg_id">
                           ...
                           {{ $row->fg_id == $CartonPackingInhouseMasterList->fg_id ? 'selected="selected"' : '' }} 
                           >{{ $row->fg_name }}</option>
                           }
                           @endforeach
                        </select>
                  ...
                  <div class="col-md-2 hide">
                     <div class="mb-3">
                        ...
                        <input type="text" name="style_no" class="form-control" id="style_no" value="{{$CartonPackingInhouseMasterList->style_no}}">
                     </div>
                  </div>
                  <div class="col-md-4 hide">
                     <div class="mb-3">
                        <label for="style_description" class="form-label">Style Description</label>
                        <input type="text" name="style_description" class="form-control" id="style_description" value="{{$CartonPackingInhouseMasterList->style_description}}">
                     </div>
                  </div>
                  <div class="col-md-2 hide">
                     <div class="mb-3">
                        <label for="order_rate" class="form-label">Order Rate</label>
                        <input type="text" name="order_rate" class="form-control" id="order_rate" value="{{$CartonPackingInhouseMasterList->order_rate}}">
                     </div>
                  </div>
                  <div class="col-md-6">
                     <div class="mb-3">
                        <label for="buyer_location_id" class="form-label">Buyer Location</label>
                        <select name="buyer_location_id" class="form-control" id="buyer_location_id" >
                           <option value="">--Select Location--</option>
                           @foreach($LedgerDetail as  $row)
                           {
                           <option value="{{ $row->sr_no }}"
                           {{ $row->sr_no == $CartonPackingInhouseMasterList->buyer_location_id ? 'selected="selected"' : '' }} 
                           >{{ $row->consignee_address }}</option>
                           }
                           @endforeach
                        </select>
                     </div>
                        ...
                        <div class="panel panel-default">
                           ...
                           <div id="collapse1" class="panel-collapse collapse in" style="width:100%;">
                              <div class="panel-body">
                                 <div class="row">
                                    <input type="number" value="{{count($CartonPackingInhouseDetailList)}}" name="cntrr1" id="cntrr1" readonly="" hidden="true"/>
                                    <div class="table-wrap">
                                       <div class="table-responsive">
                                            @if(count($CartonPackingInhouseDetailList)>0)
                                            @php $no=1;$n=1; @endphp
                                            @foreach($CartonPackingInhouseDetailList as $List) 
                                            @php
                                                
                                                $BuyerPurchaseList = DB::table("buyer_purchse_order_master")->select('sz_code')->where('tr_code',$List->sales_order_no)->get();
                                            
                                                $sz_codes = isset($BuyerPurchaseList[0]->sz_code) ? $BuyerPurchaseList[0]->sz_code : 0;
                                                 
                                                $SizeDetailList = DB::table("size_detail")->where('size_detail.sz_code','=', $sz_codes)->get();
                                            @endphp
                                             ...
                                             <tbody id="CartonData">
                                                <tr>
                                                   ...
                                                   <td>
                                                      ...
                                                      <select name="color_id[]"   id="color_id" style="width:200px; height:30px;"  class="select2" onchange="ResetAllValues(this);CalculateQtyRowProColor(this);" >
                                                   ...
                                                   <td> 
                                                      <select name="style_no_id[]" id="style_no_id" style="width:200px; height:30px;"  class="">
                                                         <option value="">--Select--</option>
                                                          @foreach($styleNoList as $styles)
                                                         <option value="{{ $styles->style_no_id }}"  {{ $styles->style_no_id == $List->style_no_id ? 'selected="selected"' : '' }} >{{ $styles->style_no }}</option> 
                                                         @endforeach
                                                      </select>
                                                   </td>
                                                   @php 
                                                   ...
                                                   <td><button type="button" onclick="addNewRow(this);" id="AButton" class="AButton btn btn-warning pull-left">+</button> <input type="button" class="btn btn-danger pull-left" onclick="deleteRowcone1(this);" value="X" style="margin-left:10px;"></td>
                                                </tr>
                                             </tbody>
                                          </table>
                                            @php $no=$no+1;  @endphp
               ...
               <div class="row">
                  <div class="col-md-2">
                     <div class="mb-3">
                        ...
                        <input type="text" name="total_qty" class="form-control" id="total_qty" value="{{$CartonPackingInhouseMasterList->total_qty}}" required readOnly>
                     </div>
                  </div> 
                   <input type="hidden" name="order_amount" class="form-control" id="order_amount" value="{{$CartonPackingInhouseMasterList->order_amount}}" > 
                  <div class="col-sm-8">
                     <label for="formrow-inputState" class="form-label">Narration</label>
                     <div class="mb-3">
                        <input type="text" name="narration" class="form-control" id="narration"  value="{{$CartonPackingInhouseMasterList->narration}}" />
                     </div>
                  </div>
               </div>
               <div class="col-sm-6">
...
//   }
   ...
   {  
       $(this).attr('class','form-control select2'); 
   });
   ...
   {   
        ...
        $.ajax({
            ...
            success: function(dataa) {
                let select = row.find('select[name^="style_no_id[]"]');
                select.html(dataa.html); // Load the new options
        
                // If only 1 non-empty option is available, select it automatically
                let options = select.find('option');
                if (options.length === 1) {
                    select.val(options.first().val()).trigger('change.select2');
                }
            }
        });
