   ...
   <div class="col-xl-12">
      <div class="card">
         <div class="card-body">
            ...
            @endif
            <form action="{{route('PackingInhouse.store')}}" method="POST" enctype="multipart/form-data" id="frmData">
               @csrf 
               <div class="row">
                  <div class="col-md-2">
                     <div class="mb-3">
                  ...
                  <div class="col-md-2">
                     <div class="mb-3">
                        <label for="packing_type_id" class="form-label">Packing Type</label>
                        <select name="packing_type_id" class="form-control select2" id="packing_type_id" onchange="SetPackingType(this);">
                           @php
                           $packingTypeData = DB::SELECT("SELECT packing_type_master.* FROM packing_type_master 
                           INNER JOIN packing_auth ON packing_auth.packing_type_auth_id = packing_type_master.packing_type_id
                           WHERE isRead=1 AND isWrite=1 AND username = '".Session::get('username')."' GROUP BY packing_auth.packing_type_auth_id");
                           @endphp
                           <option value="">--Select--</option>
                           @foreach($packingTypeData as $packingTypes)
                           <option value="{{$packingTypes->packing_type_id}}">{{$packingTypes->packing_type_name}}</option>
                           @endforeach
                        </select>
                     </div>
                  </div>
                  <div class="col-md-2">
                     <div class="mb-3">
                        ...
                        <select name="vpo_code" class="form-control select2" id="vpo_code" onChange="getFinishingInhouseDetails(this.value);" disabled>
                  ...
                  <div class="col-md-2">
                     <div class="mb-3">
                        ...
                        <select name="sales_order_no" class="form-control select2" id="sales_order_no" required  onChange="getSalesOrderTableOpening(this.value);" disabled>
                  ...
                  <div class="panel panel-default">
                     <div class="panel-heading">
                        <h4 class="panel-title">
                           <a data-toggle="collapse" data-parent="#accordion0" href="#collapse0">Stitching Entry</a>
                        </h4>
                     </div>
                     <div id="collapse0" class="panel-collapse collapse in" style="width:100%;">
...
<!-- end row -->
<script>
   function SetPackingType(row)
   {
       if($("#packing_type_id").val() == 4)
       {
           $("#vpo_code").attr('disabled', false);
           $("#sales_order_no").attr('disabled', true);
       }
       else
       {
           $("#vpo_code").attr('disabled', true);
           $("#sales_order_no").attr('disabled', false);
       }
       $("#packing_type_id").attr('disabled', true);
   }
   ...
   function getSalesOrderTableOpening(sales_order_no)
   {
     ...
     $.ajax({
       ...
       data:{'tr_code':sales_order_no,'packing_type_id':packing_type_id},
       success: function(data){
   ...
   function getFinishingInhouseDetails(vpo_code)
   {
     ...
     $.ajax({
           ...
           $("#Ac_code").val(data[0]['Ac_code']).trigger('change');
           $("#vendorId").val(data[0]['vendorId']).trigger('change');
       ...
       $.ajax({
       dataType: "json",
       url: "{{ route('FNSI_GetOrderQtyPacking') }}",
       data:{'vpo_code':vpo_code},
       success: function(data){
       $("#footable_2").html(data.html);
       }
       ...
       $.ajax({
       dataType: "json",
       url: "{{ route('Stitching_GetOrderQty') }}",
       data:{'vpo_code':vpo_code},
       success: function(data){
       $("#footable_6").html(data.html);
       }
       });
   }
   ...
   <div class="col-xl-12">
      <div class="card">
         <div class="card-body">
            ...
            @endif
            @if(isset($PackingInhouseMasterList))
            <form action="{{ route('PackingInhouse.update',$PackingInhouseMasterList) }}" method="POST" enctype="multipart/form-data" id="frmData">
               @method('put')
               @csrf
               <div class="row">
                  <div class="col-md-2">
                     <div class="mb-3">
                        ...
                        <input type="date" name="pki_date" class="form-control" id="pki_date" value="{{$PackingInhouseMasterList->pki_date}}" required  >
                        <input type="hidden" name="c_code" class="form-control" id="c_code" value="{{ $PackingInhouseMasterList->c_code }}">
                        <input type="hidden" name="pki_code" class="form-control" id="pki_code" value="{{$PackingInhouseMasterList->pki_code}}" required readOnly>
                        <input type="hidden" name="userId" value="{{ Session::get('userId') }}" class="form-control" id="formrow-email-input">
                     </div>
                  </div>      
                  <div class="col-md-2">
                    <div class="mb-3">
                       <label for="packing_type_id" class="form-label">Packing Type</label>
                       <select name="packing_type_id" class="form-control select2" id="packing_type_id" onchange="SetPackingType(this);" disabled>
                            <option value="">--Select--</option> 
                             @php
                                   $packingTypeData = DB::SELECT("SELECT packing_type_master.* FROM packing_type_master 
                                                    INNER JOIN packing_auth ON packing_auth.packing_type_auth_id = packing_type_master.packing_type_id
                             ...
                             @endphp
                             @foreach($packingTypeData as $packingTypes)
                                <option value="{{$packingTypes->packing_type_id}}" {{ $packingTypes->packing_type_id == $PackingInhouseMasterList->packing_type_id ? 'selected="selected"' : '' }} >{{$packingTypes->packing_type_name}}</option>  
                             @endforeach 
                        </select>
                    </div>
                  </div> 
                  <div class="col-md-2">
                     <div class="mb-3">
                        <label for="po_date" class="form-label">Process Order No.</label>
                        <select name="vpo_code" class="form-control select2" id="vpo_code"  onChange="getFinishingInhouseDetails(this.value);" disabled>
                           <option value="">--Vendor Work Order No--</option>
                           @foreach($VendorPurchaseOrderList as  $row)
                           {
                           <option value="{{ $row->vpo_code }}"
                           {{ $row->vpo_code == $PackingInhouseMasterList->vpo_code ? 'selected="selected"' : '' }} 
                  ...
                  <div class="col-md-2">
                     <div class="mb-3">
                        <label for="formrow-email-input" class="form-label">Sales order No</label>
                        <input type="text" name="sales_order_no" class="form-control" id="sales_order_no" value="{{$PackingInhouseMasterList->sales_order_no}}" required readOnly>
                     </div>
                  </div>
                  <div class="col-md-4">
                     <div class="mb-3">
                        ...
                        <select name="Ac_code" class="form-control select2" id="Ac_code" disabled   >
                           ...
                           {{ $row->ac_code == $PackingInhouseMasterList->Ac_code ? 'selected="selected"' : '' }} 
                           >{{ $row->ac_name }}</option>
                           }
                           @endforeach
                        </select>
                  ...
                  <div class="col-md-2">
                     <div class="mb-3">
                        ...
                        <input type="text" name="vendor_rate" class="form-control" id="vendor_rate" value="{{$PackingInhouseMasterList->vendor_rate}}" required readOnly>
                     </div>
                  </div>
                  <div class="col-md-2">
                     <div class="mb-3">
                        ...
                        <select name="mainstyle_id" class="form-control" id="mainstyle_id"  onchange="getSubStyle(this.value)" required disabled>
                           ...
                           {{ $row->mainstyle_id == $PackingInhouseMasterList->mainstyle_id ? 'selected="selected"' : '' }}  
                  ...
                  <div class="col-md-2">
                     <div class="mb-3">
                        ...
                        <select name="substyle_id" class="form-control" id="substyle_id" onchange="getStyle(this.value)" required disabled>
                           ...
                           {
                           <option value="{{ $row->substyle_id }}"
                           {{ $row->substyle_id == $PackingInhouseMasterList->substyle_id ? 'selected="selected"' : '' }}
                           >{{ $row->substyle_name }}</option>
                           }
                           @endforeach
                        </select>
                  ...
                  <div class="col-md-2">
                     <div class="mb-3">
                        ...
                        <select name="fg_id" class="form-control" id="fg_id" required disabled>
                           ...
                           {
                           <option value="{{ $row->fg_id }}"
                           {{ $row->fg_id == $PackingInhouseMasterList->fg_id ? 'selected="selected"' : '' }} 
                           >{{ $row->fg_name }}</option>
                           }
                           @endforeach
                        </select>
                  ...
                  <div class="col-md-2">
                     <div class="mb-3">
                        <label for="formrow-email-input" class="form-label">Style No</label>
                        <input type="text" name="style_no" class="form-control" id="style_no" value="{{$PackingInhouseMasterList->style_no}}" required readOnly>
                     </div>
                  </div>
                  <div class="col-md-4">
                     <div class="mb-3">
                        <label for="style_description" class="form-label">Style Description</label>
                        <input type="text" name="style_description" class="form-control" id="style_description" value="{{$PackingInhouseMasterList->style_description}}" required readOnly>
                     </div>
                  </div>
                  <div class="col-md-4">
                     <div class="mb-3">
                        <label for="formrow-inputState" class="form-label">Vendor</label>
                        <select name="vendorId" class="form-control select2" id="vendorId" disabled >
                           <option value="">--Select Vendor--</option>
                           @foreach($Ledger as  $rowvendor)
                           {
                           <option value="{{ $rowvendor->ac_code }}"
                           {{  $rowvendor->ac_code == $PackingInhouseMasterList->vendorId ? 'selected="selected"' : '' }}
                 ...
                 @if(Session::get('user_type')== 1) 
                  <div class="col-md-2">
                     <div class="form-check form-check-primary mb-3">
                        <input class="form-check-input" type="checkbox" id="is_opening" name="is_opening"  
                        @if($PackingInhouseMasterList->is_opening==1)checked @endif    >
                        <label class="form-check-label" for="is_opening">
                        Opening Stock
                        </label>
                     </div>
                  ...
                  <div class="col-md-3">
                     ...
                     <select name="location_id" class="form-control select2  " id="location_id" required>
                        ...
                        {{ $row->loc_id == $PackingInhouseMasterList->location_id ? 'selected="selected"' : '' }}    
                        >{{ $row->location }}</option>
                        }
                        @endforeach
                     </select>
                                    ...
                                    <div class="table-wrap">
                                       <div class="table-responsive">
                                          <table id="footable_2" class="table  table-bordered table-striped m-b-0  footable_2">
                                             ...
                                             <tbody>
                                                @if(count($PackingInhouseDetailList)>0)
                                                @php $no=1;$n=1; @endphp
                                                @foreach($PackingInhouseDetailList as $List) 
                                                <tr>
                                                   ...
                                                   <td>
                                                      ...
                                                      <select name="color_id[]"   id="color_id" style="width:280px; height:30px;" disabled>
                                                   ...
                                                   @php  
                                                        ...
                                                        $cartonData = DB::SELECT("SELECT ((SELECT SUM($size_ids) FROM packing_inhouse_size_detail WHERE sales_order_no = '".$PackingInhouseMasterList->sales_order_no."' AND color_id = ".$List->color_id.") - (SELECT  SUM($size_ids) FROM carton_packing_inhouse_size_detail WHERE sales_order_no = '".$PackingInhouseMasterList->sales_order_no."' AND color_id = ".$List->color_id.")) as size_qty_total");
                                                        //dd(DB::getQueryLog());
                                                        $carton_qty = isset($cartonData[0]->size_qty_total) ? $cartonData[0]->size_qty_total : 0;
                                                        
                                                        //if($carton_qty > 0)
                                                        //{
                                                        //    $min = 'min='.(max($szQty - $carton_qty, 0));
                                                        //    $max = 'max='.$szQty;
                                                        //}
                                                        //else
                                                        //{
                                                        ...
                                                        $max = 'max='.$szQty;
                                                   @endphp
                                                   <td ><input style="width:80px; float:left;"  name="s@php echo $n; @endphp[]" class="size_id" type="number" {{$min}}  {{$max}} onchange="validateSize(this);" id="s@php echo $n; @endphp" value="{{$szQty}}" required />  </td>
                                                   @php $n=$n+1;  @endphp
                                                   @endforeach
                                                   <td>
                                                      <input type="hidden" name="overall_size_qty[]" value="{{$List->size_qty_total}}" class="overall_size_qty">
                                                      <input type="number" name="size_qty_total[]" class="size_qty_total" value="{{$List->size_qty_total}}" id="size_qty_total" style="width:80px; height:30px; float:left;" readonly  />
               ...
               <div class="row">
                  <div class="col-md-2">
                     <div class="mb-3">
                        <label for="total_qty" class="form-label">Total Qty</label>
                        <input type="text" name="total_qty" class="form-control" id="total_qty" value="{{$PackingInhouseMasterList->total_qty}}"  readOnly>
                     </div>
                  </div>
                  <div class="col-md-2">
                     <div class="mb-3">
                        <label for="rate" class="form-label">Rate</label>
                        <input type="number" step="any" name="rate" class="form-control" id="rate" value="{{$PackingInhouseMasterList->rate}}" required onchange="mycalc();">
                     </div>
                  </div>
                  <div class="col-md-2">
                     <div class="mb-3">
                        <label for="vendor_amount" class="form-label">Total Amount</label>
                        <input type="text" name="vendor_amount" class="form-control" id="vendor_amount" value="{{$PackingInhouseMasterList->vendor_amount}}" readOnly>
                     </div>
                  </div>
                  <div class="col-sm-6">
                     <label for="formrow-inputState" class="form-label">Narration</label>
                     <div class="mb-3">
                        <input type="text" name="narration" class="form-control" id="narration"  value="{{$PackingInhouseMasterList->narration}}" />
                     </div>
                  </div>
               </div>
               <div class="col-sm-6">
...
<script>
   ...
   $(document).on("change", 'input[class^="size_id"]', function (event) 
   {
        var no=1;
        var sz = $(this);
        var prev_value =  $(this).closest("tr").find('input[name="overall_size_qty[]"]').val();
        var sales_order_no = $('#sales_order_no').val();
        
        var sizes1= $(this).parent().parent("tr").find('td>.size_id');
        var sizeArr = "";
        sizes1.each(function() {
            sizeArr = sizeArr+$(this).val()+',';
        });
        sizes = sizeArr.replace(/,\s*$/, "");
       
       
        var size_array = sizes.split(',');
        var is_opening = $("#is_opening").is(":checked");
         
        var numericArray = size_array.map(function(number) {
          return parseInt(number.trim(), 10);
        });
 
        var sum = numericArray.reduce(function(total, num) {
          return total + num;
        }, 0);
        ...
        else if(sum < prev_value)
        {
                $(this).closest("tr").find('input[name="size_qty_total[]"]').val(sum); 
        }
        else
        {      
            var over = $(this).closest("tr").find('input[name="size_qty_total[]"]').val();
                
            $(sz).val(0);   
            var sizes2= $(this).parent().parent("tr").find('td>.size_id');
            var sizeArr1 = "";
            sizes2.each(function() {
                sizeArr1 = sizeArr1+$(this).val()+',';
            });
            sizes12 = sizeArr1.replace(/,\s*$/, "");
           
           
            var size_array12 = sizes12.split(',');
            var is_opening = $("#is_opening").is(":checked");
            
            var numericArray12 = size_array12.map(function(number) {
              return parseInt(number.trim(), 10);
            });
     
            var sum12 = numericArray12.reduce(function(total, num) {
              return total + num;
            }, 0);
            
           $(sz).val(parseFloat(prev_value) - parseFloat(sum12));   
           
            alert("Total quantity can not be greater than "+prev_value);
       
            $(this).closest("tr").find('input[name="size_qty_total[]"]').val(prev_value);
                 
        }
        
        $(this).closest("tr").find('input[name="size_qty_array[]"]').val(numericArray);
    //      var values = [];
    //      $("#footable_2 tr td  input[class='size_id']").each(function() {
    //      values.push($(this).val());
    //      if(values.length==size_array.length)
    ...
    //               console.log('sum: '+sum);
    //                 console.log('PreValues: '+prev_value);
    //       if(is_opening == true)
    //       {
    //             $(this).closest("tr").find('input[name="size_qty_total[]"]').val(sum);
    //             console.log(1);
