...
@if(isset($CityList))
<form action="{{ route('City.update',$CityList) }}" method="POST">
@method('put')
...
<select name="country_id" class="form-select" id="country_id" onChange="getState(this.value);">
<option value="">--- Select Country ---</option>
@foreach($Countrylist as  $row)
{
<option value="{{ $row->c_id }}"

{{ $row->c_id == $CityList->country_id ? 'selected="selected"' : '' }}
...
{{ $row2->state_id == $CityList->state_id ? 'selected="selected"' : '' }}
...
<div class="col-md-2">
<div class="mb-3">
<label for="formrow-inputDistrict" class="form-label">District</label>
...
{{ $row->d_id == $CityList->dist_id ? 'selected="selected"' : '' }}
...
@foreach($talukalist as  $row)
{
<option value="{{ $row->tal_id }}"
{{ $row->tal_id == $CityList->taluka_id ? 'selected="selected"' : '' }}
>{{ $row->taluka }}</option>
...
<div class="col-md-4">
<div class="mb-3">
<label for="formrow-city_name-input" class="form-label">City Name</label>
<input type="text" name="city_name" class="form-control" id="formrow-city_name-input" value="{{ $CityList->city_name }}">
...
<div class="mb-3">
<label for="formrow-city_name-input" class="form-label">City Name</label>
<input type="text" name="city_name" class="form-control" id="formrow-city_name-input">
<input type="hidden" name="userId" value="{{ Session::get('userId')}}" class="form-control" id="formrow-userId-input">
</div>
</div>
