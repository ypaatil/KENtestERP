   ...
   <div class="col-xl-12">
      <div class="card">
         <div class="card-body">
            ...
            <form action="{{ route('FGLocationTransferOutward.update',$TransferPackingInhouseMasterList) }}" method="POST" enctype="multipart/form-data" id="frmData">
               @method('put')
               @csrf
               <div class="row">
                  <div class="col-md-2">
                     <div class="mb-3">
                        ...
                        <input type="date" name="ltpki_date" class="form-control" id="ltpki_date" value="{{$TransferPackingInhouseMasterList->ltpki_date}}" required>
                        <input type="hidden" name="c_code" class="form-control" id="c_code" value="{{ $TransferPackingInhouseMasterList->c_code }}">
                        <input type="hidden" name="ltpki_code" class="form-control" id="ltpki_code" value="{{$TransferPackingInhouseMasterList->ltpki_code}}" readOnly>
                        <input type="hidden" name="userId" value="{{ Session::get('userId') }}" class="form-control" id="formrow-email-input">
                     </div>
                  </div>
                  <div class="col-md-2">
                     <div class="mb-3">
                        <label for="po_date" class="form-label">Sales Order No: {{$TransferPackingInhouseMasterList->sales_order_no}}</label>
                        <select name="main_sales_order_no" class="form-control select2" id="main_sales_order_no" required  onChange="getPackingInhouseDetails(this.value);"  >
                           ...
                           {{ $row->tr_code == $TransferPackingInhouseMasterList->sales_order_no ? 'selected="selected"' : '' }} 
                           >{{ $row->tr_code }}</option>
                           }
                           @endforeach
                        </select>
                  ...
                  <div class="col-md-2">
                     <div class="mb-3">
                        ...
                        <select name="from_loc_id" class="form-control" id="from_loc_id" required>
                           ...
                           {{ $row->loc_id == $TransferPackingInhouseMasterList->from_loc_id ? 'selected="selected"' : '' }} 
                           >{{ $row->location }}</option>
                           }
                           @endforeach
                        </select>
                  ...
                  <div class="col-md-2">
                     <div class="mb-3">
                        ...
                        <select name="to_loc_id" class="form-control" id="to_loc_id" required>
                           ...
                           {
                           <option value="{{ $row->loc_id }}"
                           {{ $row->loc_id == $TransferPackingInhouseMasterList->to_loc_id ? 'selected="selected"' : '' }} 
                           >{{ $row->location }}</option>
                           }
                           @endforeach
                        </select>
                                             ...
                                             <tbody id="CartonData">
                                                ...
                                                <tr>
                                                   ...
                                                   <td>
                                                      ...
                                                      <select name="color_id[]"   id="color_id" style="width:200px; height:30px;" onchange="setFGLimit(this);" sales_order_no="'.$List->sales_order_no.'" size_array="'.$List->size_qty_array.'" required>
                                                   ...
                                                   <td ><input style="width:80px; float:left;" max="{{$szQty}}" min="0" name="s@php echo $n; @endphp[]" class="size_id" type="number" id="s@php echo $n; @endphp" value="{{$szQty}}" required /> <span>{{$szQty}}</span></td>
                                                   @php $n=$n+1;  @endphp
                                                   @endforeach
...
<script>
   ...
   $(document).ready(function() {
       ...
       $('input,select').not('.size_id').attr('disabled', true);
   });  
