   ...
   <div class="col-xl-12">
      <div class="card">
         <div class="card-body">
            ...
            @endif
            <form action="{{route('WashingInhouse.store')}}" method="POST" enctype="multipart/form-data" id="frmData">
               @csrf 
               <div class="row">
                  <div class="col-md-2">
                     <div class="mb-3">
                        <label for="vpo_date" class="form-label">Entry Date</label>
                        <input type="date" name="wash_date" class="form-control" id="wash_date" value="{{date('Y-m-d')}}" required  >
...
<script>
   ...
   $(document).ready(function() {
     ...
     function CalSizeQtyTotal() 
     {
        $("#footable_2 tr").each(function() 
        {
            var values = [];
            $(this).find("td input[class='size_id']").each(function() {
                values.push($(this).val() ? parseInt($(this).val(), 10) : 0);
            });
            ...
            $(this).find('input[name="size_qty_total[]"]').val(sum);
    
            $(this).find('input[name="size_qty_array[]"]').val(values.join(","));
        });
    }
   ...
   $(document).on("change", 'input[class^="size_id"]', function (event) 
   {
        ...
        var current_size = $(this);
        var max_size= $(this).parent().parent('tr').find('td input[name="size_qty_total[]"]').siblings('span').html();
        var current_val = $(this).parent().parent('tr').find('td input[name="size_qty_total[]"]').val();
        if(parseInt(current_val) > parseInt(max_size))
        {
            alert("Total Qty. Exceeded..!"); 
            $(this).val(0);
            $(this).closest("tr").find('input[name="size_qty_total[]"]').val(0);
            CalSizeQtyTotal();
        } 
        else
        {
