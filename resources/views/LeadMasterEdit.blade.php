...
<div class="row">
   <div class="col-xl-12">
      <div class="card">
         <div class="card-body">
            <h4 class="card-title mb-4">Lead - LD{{$CRMMaster->crm_id}}</h4>
            @if ($errors->any()) 
            <div class="col-md-6">
            ...
            @endif
            @if(isset($CRMMaster))
            <form action="{{ route('Lead.update',$CRMMaster) }}" method="POST" enctype="multipart/form-data" id="frmData">
               @method('put')
               @csrf
               <input type="hidden" name="crm_id" class="form-control" id="crm_id" value="{{ $CRMMaster->crm_id}}">
               <div class="row"> 
                  <div class="col-md-4">
                     <div class="mb-3">
                        <label for="buyer_name" class="form-label">Buyer Name <span class="required_label">*</span></label>
                        <input type="text" name="buyer_name" class="form-control" id="buyer_name" value="{{$CRMMaster->buyer_name}}" required>
                     </div>
                  </div>
                  <div class="col-md-2">
                     <div class="mb-3">
                        <label for="buyer_brand" class="form-label">Buyer Brand <span class="required_label">*</span></label>
                        <input type="text" name="buyer_brand" class="form-control" id="buyer_brand" value="{{$CRMMaster->buyer_brand}}" required>
                     </div>
                  </div>
                  <div class="col-md-2">
                     <div class="mb-3">
                        <label for="" class="form-label">Buyer Type <span class="required_label">*</span></label> 
                        <select name="buyer_type_id" class="form-select select2" id="buyer_type_id" required>
                           <option value="">--Select--</option>
                           @foreach($BuyerTypeList as  $row) 
                                <option value="{{ $row->buyer_type_id }}" {{ $row->buyer_type_id == $CRMMaster->buyer_type_id ? 'selected="selected"' : '' }}>{{ $row->buyer_type_name }}</option> 
                           @endforeach
                        </select>
                     </div>
                  </div>
                     ...
                     <div class="mb-3">
                        ...
                        <select name="order_group_id" class="form-select" id="order_group_id" required >
                           ...
                           @foreach($OrderGroupList as  $row) 
                                <option value="{{ $row->og_id }}"  {{ $row->og_id == $CRMMaster->order_group_id ? 'selected="selected"' : '' }} >{{ $row->order_group_name }}</option> 
                           @endforeach
                        </select>
                     </div>
                  </div>
                     ...
                     <div class="mb-3"> 
                        ...
                        <select name="cur_id" class="form-select" id="cur_id" required>
                           ...
                           @foreach($CurrencyList as  $row)
                              <option value="{{ $row->cur_id }}" {{ $row->cur_id == $CRMMaster->cur_id ? 'selected="selected"' : '' }}>{{ $row->currency_name }}</option>
                           @endforeach
                        </select> 
                     </div>
                  </div>
                     ...
                     <div class="mb-3">
                        ...
                        <select name="country_id" class="form-select" id="country_id" onchange="getState(this.value);" required>
                           ...
                           @foreach($countryList as  $row)
                              <option value="{{ $row->c_id }}"  {{ $row->c_id == $CRMMaster->country_id ? 'selected="selected"' : '' }} >{{ $row->c_name }}</option>
                           @endforeach
                        </select>
                     </div>
                  </div>
                     ...
                     <div class="mb-3">
                        ...
                        <select name="state_id" class="form-select" id="state_id">
                           ...
                           @foreach($StateList as  $row)
                              <option value="{{ $row->state_id }}"  {{ $row->state_id == $CRMMaster->state_id ? 'selected="selected"' : '' }} >{{ $row->state_name }}</option>
                           @endforeach
                        </select>
                     </div>
                  </div>
                  <div class="col-md-2">
                     <div class="mb-3">
                        ...
                        <input type="text" name="city_name" class="form-control" id="city_name" value="{{$CRMMaster->city_name}}" >
                     </div>
                  </div>
                  <div class="col-md-2">
                     <div class="mb-3">
                        <label for="zip_code" class="form-label">Zip/Postal Code</label>
                        <input type="text" name="zip_code" class="form-control" id="zip_code" value="{{$CRMMaster->zip_code}}" >
                     </div>
                  </div>
                  <div class="col-md-4">
                     <div class="mb-3">
                        <label for="street_name" class="form-label">Street</label>
                        <textarea  class="form-control" name="street_name"  id="street_name" cols="12" >{{$CRMMaster->street_name}}</textarea> 
                     </div>
                  </div> 
                  <div class="col-md-2">
                     <div class="mb-3">
                        ...
                        <select name="stage_id" class="form-select" id="stage_id" required>
                           ...
                           @foreach($StatgeList as  $row)
                              <option value="{{ $row->stage_id }}" {{ $row->stage_id == $CRMMaster->stage_id ? 'selected="selected"' : '' }} >{{ $row->stage_name }}</option>
                           @endforeach
                        </select>
                     </div>
                  </div>
                  <div class="col-md-2">
                     <div class="mb-3">
                        <label for="compliant" class="form-label">Compliant/Non-compliant <span class="required_label">*</span></label>
                        <select name="compliant_status_id" class="form-select" id="compliant_status_id" required>
                           <option value="">--Select--</option>
                           @foreach($CompliantList as  $row) 
                                <option value="{{ $row->compliant_status_id }}" {{ $row->compliant_status_id == $CRMMaster->compliant_status_id ? 'selected="selected"' : '' }} >{{ $row->compliant_status_name }}</option> 
                           @endforeach
                        </select>
                     </div>
                  </div>
                     ...
                     <div class="mb-3">
                        ...
                        <select name="lead_status_id" class="form-select" id="lead_status_id" required>
                           ...
                           @foreach($LeadStatusList as  $row)
                              <option value="{{ $row->lead_status_id }}" {{ $row->lead_status_id == $CRMMaster->lead_status_id ? 'selected="selected"' : '' }}>{{ $row->lead_status_name }}</option>
                           @endforeach
                        </select>
                     </div>
                  </div>
                  <div class="col-md-2">
                     <div class="mb-3">
                        ...
                        <input type="text" name="ownership_name" class="form-control" id="ownership_name" value="{{$CRMMaster->ownership_name}}" >
                     </div>
                  </div>
               </div> 
               <div class="row"> 
                      ...
                      <table id="footable_2" class="table  table-bordered table-striped m-b-0  footable_2">
                         ...
                         <tbody> 
                            @php
                                $sr_no = 1;
                            @endphp
                            @foreach($CRMDetails as $row)
                            <tr>
                               <td><input type="text" name="sr_no[]" value="{{$sr_no++}}" class="form-control" id="id0" style="width:50px;" readonly/></td>
                               <td><input type="text" name="contactName[]" class="contactName form-control" value="{{$row->contactName}}" id="contactName" style="width:300px;" required /></td>
                               <td><input type="text" name="contactNo[]" class="contactNo form-control" value="{{$row->contactNo}}" id="contactNo" style="width:250px;" /></td>
                               <td><input type="text" name="email[]" class="email form-control" value="{{$row->email}}" id="email" style="width:300px;" /></td> 
                               <td>
