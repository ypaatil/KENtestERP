...
<div class="row">
   <div class="col-xl-12">
      <div class="card">
         <div class="card-body">
            ...
            @endif
            <form action="{{route('FGOutletOpening.store')}}" method="POST" enctype="multipart/form-data" id="frmData">
               @csrf 
               <div class="row">
                  <div class="col-md-2">
                     <div class="mb-3">
                        <label for="fgo_date" class="form-label">Date</label>
                        <input type="date" name="fgo_date" class="form-control" id="fgo_date" value="{{date('Y-m-d')}}">
   ...
   <input type="number" step="any" name="total_qty" class="form-control" id="total_qty" value="" readOnly>
   </div>
   </div> 
   <div class="col-sm-8">
...
<script>
   ...
   function GetSizeDetailList(str)
   {
    ...
    {
        var fgo_code = $("#fgo_code").val();
        $.ajax({
            dataType: "json",
            url: "{{ route('GetFGOpeningOutwardData') }}",
            data: {'fgo_code': fgo_code},
    ...
    $(document).on("change", 'input[class^="size_id"]', function (event) 
    {
      var no=1;
       ...
       var size_array = sizes.split(',');
         ...
         var values = [];
         $(this).closest("tr").find("input[class='size_id']").each(function() 
         {
             values.push($(this).val());
        ...
        var values1 = [];
        $(this).closest("tr").find("input[class='size_rate']") .each(function() {
            values1.push($(this).val());
            $(this).closest("tr").find('input[name="rate_array[]"]').val(values1); 
        });
