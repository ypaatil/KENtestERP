   ...
   <div class="col-xl-12">
      <div class="card">
         <div class="card-body">
            ...
            @endif
            <form action="{{route('VendorPurchaseOrder.store')}}" method="POST" enctype="multipart/form-data" id="frmData">
               @csrf 
               <div class="row">
                  <div class="col-md-2">
                     <div class="mb-3">
                        <label for="vpo_date" class="form-label">PO Date</label>
                        <input type="date" name="vpo_date" class="form-control" id="vpo_date" value="{{date('Y-m-d')}}" required readOnly>
                  ...
                  <div class="col-md-2">
                     <div class="mb-3">
                        ...
                        <select name="process_id" class="form-control" id="process_id" onchange="checkSalesOrder(this.value);" required>
                  ...
                  <div class="col-md-2">
                     <div class="mb-3">
                        ...
                        <select name="sales_order_no" class="form-control select2" id="sales_order_no" required onChange="checkCostingStatus(this.value);" disabled>
                           <option value="0">--Sales Order No--</option>
                  ...
                  <input type="hidden" name="season_id" class="form-control" id="season_id" value="" required readOnly>
                  <input type="hidden" name="currency_id" class="form-control" id="currency_id" value="" required readOnly>
                  <input type="hidden" name="order_rate" class="form-control" id="order_rate" value="" required readOnly>
                  <div class="col-md-2">
                     <div class="mb-3">
               ...
               <div class="row">
                  <div class="">
                     <div class="panel-group" id="accordion">
                        ...
                        <div class="panel panel-default" id="fabric_panel">
                        ...
                        <div class="panel panel-default" id="trim_fabric_panel">
                           <div class="panel-heading">
                              <h4 class="panel-title">
                        ...
                        <div class="panel panel-default" id="packing_trim_panel">
                           <div class="panel-heading">
                              <h4 class="panel-title">
...
<script>
   ...
   $(document).on('keydown', 'input[type="number"]', function(e) {
    ...
    { 
        if(row != "")
        {   
            $("#sales_order_no").removeAttr('disabled');
        }  
        else
        {
            $("#sales_order_no").val(""); 
        ...
        if(row == 1)
        { 
            $("#packing_trim_panel").remove();
        }
        
        if(row == 3)
        {
             $("#fabric_panel").remove();
             $("#trim_fabric_panel").remove();
        }
   ...
   function MainBtn()
   {
    ...
    {  
        ...
        $('#footable_1 tbody tr').each(function() {
                ...
                var itemName = row.find('td select[name="item_code[]"] option:selected').text();
                var color = row.find('td select[name="class_id[]"] option:selected').text();
                var size = row.find('td input[name="description[]"]').val();
    ...
    {   
        ...
        $('#footable_5 tbody tr').each(function() {
                var row = $(this);
                // Get values of item name, color, and size
                var itemName = row.find('td select[name="item_codesx[]"] option:selected').text();
                var color = row.find('td select[name="class_idsx[]"] option:selected').text();
                var size = row.find('td input[name="descriptionsx[]"]').val();
                // Get selected value from dropdown
                var selectedValue = row.find('select').val();
                // Create a unique key based on the combination of values
                var key = itemName + '-' + color + '-' + size + '-' + selectedValue;
   ...
   // }
    ...
    $('table.footable_4').on("change", 'input[name^="consumptionss[]"],input[name^="rate_per_unitss[]"],input[name^="bom_qtyss[]"]', function()
   {
      // alert();
   CalculateQtyRowPross($(this).closest("tr"));
