...
<form action="{{route('MultiPayment.update',$multipaymentfetch)}}" method="POST">
<input type="hidden" name="type" id="type" class="form-control" value="<?php echo  'PURCHASE' ?>" /> 
<input type="hidden" name="c_code" id="c_code" class="form-control" value="{{ $multipaymentfetch->c_code }}" /> 
...
{{ $row->firm_id == $multipaymentfetch->firm_id ? 'selected="selected"' : '' }}
...
<input type="text" id="TrNo" name="TrNo" class="form-control" placeholder="Transaction No" value="{{ $multipaymentfetch->tr_code }}" readonly />
...
<input type="date" name="tr_date" class="form-control" value="{{ $multipaymentfetch->tr_date }}"/>
</div>
</div>
<div class="col-md-4">
<div class="mb-3">
<label for="formrow-email-input" class="form-label">Payment Date</label><br>

<input type="radio" name="pay_mode" value="1"  @if($multipaymentfetch->pay_mode==1) checked="Checked" @endif>BY CASH  

<input type="radio" name="pay_mode" value="2"  @if($multipaymentfetch->pay_mode==2) checked="Checked" @endif>BY CHEQUE  
<input type="radio" name="pay_mode" value="3"  @if($multipaymentfetch->pay_mode==3) checked="Checked" @endif>BY NEFT/RTGS  
<input type="radio" name="pay_mode" value="4"  @if($multipaymentfetch->pay_mode==4) checked="Checked" @endif>OTHER
</div>
</div>
</div>
...
<select name="cr_code" class="form-select" id="CrCode">
<option value="">--Select Cash/Bank AC--</option>
@foreach($cashbank as  $rowledgercr)
{
<option value="{{ $rowledgercr->ac_code  }}"

{{ $rowledgercr->ac_code == $multipaymentfetch->cr_code ? 'selected="selected"' : '' }}
...
<select name="Ac_code" class="form-select" id="DrCode" onchange="GetUnpaidBills(this.value);">
...
<select name="tr_nos" class="form-select" id="TrNos" onchange="detail(this.value);">
...
<tbody> 
  ...
  @php $no=1; @endphp
@foreach($detailmultipayment as $row)

<tr>
<td><input type="text" readOnly style="width:50px;"   name="id[]" value="{{ $no }}"></td>

<td>
<input readOnly style="width:200px;" type="hidden" name="DrCodes[]"   value="{{ $row->Ac_code }}">
...
<input type="hidden"   name="cnt" id="cnt" value="{{ count($detailmultipayment) }}">
...
<div class="mb-3">
<label for="formrow-email-input" class="form-label">Total Paying Amount</label>
<input type="text" name="total_amount" class="form-control" id="total_amount" value="{{ $multipaymentfetch->total_amount }}">
</div>
</div>
<div class="col-md-6">
<div class="mb-3">
<label for="formrow-email-input" class="form-label">Narration</label>
<input type="text" name="narration" class="form-control" id="narration" value="{{ $multipaymentfetch->narration }}">
</div>
</div>
...
t2.value=document.getElementById("DrCode").value;
cell2.appendChild(t2);
document.getElementById("DrCodes"+index).style.display='none';

var tx2=document.createElement("input");
var tx = document.getElementById("DrCode");
...
type: "GET",
dataType:'json',
url: "{{ route('getUnpaidPaymentBills') }}",
...
$("#TrNos").html(data.html);
 ...
 function detail(TrNos){


 var Accode=document.getElementById('DrCode').value;

$.ajax({
type:"GET",
url: "{{ route('getPaymentDetail') }}",
dataType:"json",
