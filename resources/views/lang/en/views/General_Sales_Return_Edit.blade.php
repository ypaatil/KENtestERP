...
<h4 class="mb-sm-0 font-size-18">General Sales Return</h4>

<div class="page-title-right">
<ol class="breadcrumb m-0">
<li class="breadcrumb-item"><a href="javascript: void(0);">Forms</a></li>
<li class="breadcrumb-item active">General Sales Return</li>
</ol>
</div>
...
<div class="card">
<div class="card-body">
<h4 class="card-title mb-4">Sale <input type="checkbox" name="type_change" id="type_change" value=""  onkeyup="calculateGstsss();" checked /> Type Change </h6></h4> 
...
<form action="{{route('GeneralSalesReturn.update',$salesreturnfetch)}}" method="POST">
<input type="hidden" name="type" id="type" class="form-control" value="<?php echo  'SALES' ?>" /> 
@method('put')
@csrf 
<div class="row">
<div class="col-md-4">
...
{{ $row->firm_id == $salesreturnfetch->firm_id ? 'selected="selected"' : '' }}
...
<input type="text" name="bill_code" class="form-control" id="bill_code" value="{{ $salesreturnfetch->bill_code }}" readonly="readonly">
<input type="hidden" name="c_code" id="c_code" value="{{ $salesreturnfetch->c_code }}" />
...
<input type="date" name="bill_date" class="form-control" id="formrow-email-input" value="{{ $salesreturnfetch->bill_date }}">
</div>
</div>
<div class="col-md-2">
<div class="mb-3">
<label for="formrow-email-input" class="form-label">Payment Type</label>
<select name="pay_type" class="form-select" id="pay_type">
<option value="0">--Payment Type--</option>
@foreach($paytypelist as  $rowpay)
{
<option value="{{ $rowpay->pay_type  }}"

{{ $rowpay->pay_type == $salesreturnfetch->pay_type ? 'selected="selected"' : '' }}

	>{{ $rowpay->pay_name }}</option>
...
{{ $rowgst->tax_type_id == $salesreturnfetch->tax_type_id ? 'selected="selected"' : '' }}
...
{{ $rowledger->ac_code == $salesreturnfetch->Ac_code ? 'selected="selected"' : '' }}
...
@php  if($detailsalesreturn->isEmpty()) { @endphp
<tr>
...
<td><input type="text"   name="mrps[]"  value="0" class=""  id="mrp" style="width:80px;" required/></td>
...
<td><input type="button" class="btn btn-danger pull-left" onclick="deleteRowcone(this);" value="X" ></td>
  ...
  @php } else { @endphp
  @php $no=1; @endphp
@foreach($detailsalesreturn as $row)
...
<td><input style="width:80px;" type="text" class="Qty" name="item_qtys[]" id="item_qty" value="{{ $row->item_qty }}">
</td>
<td><input  style="width:80px;" type="number" step="0.01" class="" name="item_rates[]" id="item_rate" value="{{ $row->item_rate }}">
</td> 
<td><input readOnly style="width:80px;"  type="number" step="0.01" class="" name="mrps[]" id="mrp" value="{{ $row->mrp }}"></td>
<td><input readOnly style="width:80px;"  type="number" step="0.01"  class="" id="pur_cgst" name="pur_cgsts[]" value="{{ $row->pur_cgst }}"></td>
<td><input  style="width:80px;"  type="number" step="0.01" class="GSTAMT" id="camt" name="camts[]" value="{{ $row->camt }}"></td>
<td><input readOnly style="width:80px;"  type="number" step="0.01" class="" id="pur_sgst" name="pur_sgsts[]" value="{{ $row->pur_sgst }}"></td>
<td><input style="width:80px;"  type="number" step="0.01" class="GSTAMT" name="samts[]" id="samt" value="{{ $row->samt }}"></td>
<td><input readOnly style="width:80px;"  type="number" step="0.01" class="" name="pur_igsts[]" id="pur_igst" value="{{ $row->pur_igst }}"></td>
<td><input  style="width:80px;"  type="number" step="0.01" class="GSTAMT" id="iamt" name="iamts[]" value="{{ $row->iamt }}"></td>
<td><input  style="width:80px;"  type="number" step="0.01" class="GROSS" id="amount" name="amounts[]" value="{{ $row->amount }}"></td>
<td><input  style="width:80px;"  type="number" step="0.01" class="TOTAMT" id="total_amount" name="total_amounts[]" value="{{ $row->total_amount }}"></td>
<td><input type="button" class="btn btn-danger pull-left" onclick="deleteRow(this);" value="Remove" ></td>
 </tr>
 @php $no=$no+1;  @endphp
@endforeach
@php } @endphp
...
<br/>
<input type="hidden"   name="cnt" id="cnt" value="{{ count($detailsalesreturn) }}">
<div class="row">
<div class="col-md-2">
<div class="mb-3">
<label for="formrow-email-input" class="form-label">Gross Amount</label>
<input type="text" name="Gross_amount" class="form-control" id="Gross_amount" onChange="mycalc();"  value="{{ $salesreturnfetch->Gross_amount }}">
</div>
</div>
<div class="col-md-2">
<div class="mb-3">
<label for="formrow-email-input" class="form-label">Add 1</label>
<input type="text" name="add1" id="add1" class="form-control" id="add1" onkeyup="mycalc();" value="{{ $salesreturnfetch->add1 }}">
</div>
</div>
<div class="col-md-2">
<div class="mb-3">
<label for="formrow-email-input" class="form-label">Add 2</label>
<input type="text" name="add2" id="add2" class="form-control" id="add2" onkeyup="mycalc();" value="{{ $salesreturnfetch->add2 }}">
</div>
</div>
...
<input type="text" name="less1"  class="form-control" id="less1" onkeyup="mycalc();" value="{{ $salesreturnfetch->less1 }}">
</div>
</div>
<div class="col-md-2">
<div class="mb-3">
<label for="formrow-email-input" class="form-label">Less 2</label>
<input type="text" name="less2"  class="form-control" id="less2" onkeyup="mycalc();" value="{{ $salesreturnfetch->less2 }}">
</div>
</div>
...
<input type="text" name="Gst_amount" id="Gst_amount" class="form-control" id="Gst_amount" value="{{ $salesreturnfetch->Gst_amount }}" OnChange="mycalc();">
</div>
</div>
</div>
...
<input type="text" name="Net_amount" id="Net_amount" class="form-control" id="Net_amount" value="{{ $salesreturnfetch->Net_amount }}" OnChange="mycalc();">
</div>
</div>
...
<input type="text" name="narration" class="form-control" id="narration" value="{{ $salesreturnfetch->narration }}">
</div>
</div>	
</div>
...
t5.id = "mrps"+index;
t5.name= "mrps[]";
t5.value=document.getElementById("mrp").value;
...
t6.type="number";
t6.step="0.01";
t6.id = "pur_cgsts"+index;
t6.name= "pur_cgsts[]";
t6.value=document.getElementById("pur_cgst").value;
...
t7.id = "camts"+index;
t7.name= "camts[]";
t7.value=document.getElementById("camt").value;
...
t8.type="number";
t8.step="0.01";
t8.id = "pur_sgsts"+index;
t8.name= "pur_sgsts[]";
t8.value=document.getElementById("pur_sgst").value;
...
t9.id = "samts"+index;
t9.name= "samts[]";
t9.value=document.getElementById("samt").value;
...
t10.type="number";
t10.step="0.01";
t10.id = "pur_igsts"+index;
t10.name= "pur_igsts[]";
t10.value=document.getElementById("pur_igst").value;
...
t11.type="number";
t11.step="0.01";
t11.className="GSTAMT";
t11.id = "iamts"+index;
t11.name= "iamts[]";
t11.value=document.getElementById("iamt").value;
...
t12.step="0.01";
t12.className="GROSS";
t12.id = "amounts"+index;
t12.name= "amounts[]";
t12.value=document.getElementById("amount").value;
...
t13.type="number";
t13.step="0.01";
t13.className='TOTAMT';
t13.id = "total_amounts"+index;
t13.name= "total_amounts[]";
t13.value=document.getElementById("total_amount").value;
...
function mycalc()
{   
var Add=parseFloat(document.getElementById('add1').value) + parseFloat(document.getElementById('add2').value);
var Less=parseFloat(document.getElementById('less1').value) + parseFloat(document.getElementById('less2').value);
...
var ntamt=parseFloat(sum1.toFixed(0));
document.getElementById("Net_amount").value = ( parseFloat(ntamt)+ (parseFloat(Add) + parseFloat(Less)));;
...
var tax_type_id1=document.getElementById('tax_type_id').value;
//var type_change1=document.getElementById('type_change').value;

if(document.getElementById('type_change').checked==true)
{
...
else if(document.getElementById('type_change').checked==false)
{ 

if(tax_type_id1==2)
{

var Amounts =  parseFloat(amount/pur_igst/100+1);  
$('#total_amount').val(Amounts);
var iamt=  parseFloat((Amounts*(pur_igst/100))).toFixed(2);
$('#iamt').val(iamt);
...
{
var Amounts =  parseFloat(amount/pur_cgst+pur_sgst/100+1).toFixed(2);
$('#total_amount').val(parseFloat(Amounts));
var camt=  parseFloat(( Amounts*(pur_cgst/100))).toFixed(2);
$('#camt').val(camt);
var samt= parseFloat(( Amounts*(pur_sgst/100))).toFixed(2);
$('#samt').val(samt);
       ...
       data:{item_code:item_code,tax_type_id:tax_type_ids},
        success: function(data){
            ...
            if(tax_type_ids==1)
            {
                        ...
                        row.find('input[name^="mrps[]"]').val(data[0]['item_mrp']);
