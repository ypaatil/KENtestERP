...
<div class="card">
<div class="card-body">
<h4 class="card-title mb-4">Vendor Purchase Order</h4>
@if ($errors->any())
...
<form action="{{ route('VendorPurchaseOrder.update',$VendorPurchaseOrderMasterList) }}" method="POST" enctype="multipart/form-data">
@method('put')
@csrf
...
<div class="col-md-2">
    <div class="mb-3">
        ...
        <input type="date" name="vpo_date" class="form-control" id="vpo_date" value="{{$VendorPurchaseOrderMasterList->vpo_date}}" required readOnly>
        <input type="hidden" name="c_code" class="form-control" id="c_code" value="{{ $VendorPurchaseOrderMasterList->c_code }}">
        <input type="hidden" name="vpo_code" class="form-control" id="vpo_code" value="{{$VendorPurchaseOrderMasterList->vpo_code}}" required readOnly>
...
@foreach($CostTypeList as  $row)
{
    <option value="{{ $row->cost_type_id }}"
     {{ $row->cost_type_id == $VendorPurchaseOrderMasterList->cost_type_id ? 'selected="selected"' : '' }}  
...
@foreach($SeasonList as  $row)
{
    <option value="{{ $row->season_id }}"
     {{ $row->season_id == $VendorPurchaseOrderMasterList->season_id ? 'selected="selected"' : '' }}   
...
@foreach($CurrencyList as  $row)
{
    <option value="{{ $row->cur_id }}"
     {{ $row->cur_id == $VendorPurchaseOrderMasterList->currency_id ? 'selected="selected"' : '' }}
...
<div class="col-md-2">
    <div class="mb-3">
        ...
        <input type="number" name="order_rate" class="form-control" id="order_rate" readOnly value="{{$VendorPurchaseOrderMasterList->order_rate}}" required>
    </div>
</div>
...
<input type="text" name="style_no" class="form-control" id="style_no" value="{{$VendorPurchaseOrderMasterList->style_no}}" required readOnly>
</div>
</div>
...
<input type="text" name="style_description" class="form-control" id="style_description" value="{{$VendorPurchaseOrderMasterList->style_description}}" required readOnly>
...
@foreach($Ledger as  $rowvendor)
{
    ...
    {{  $rowvendor->ac_code == $VendorPurchaseOrderMasterList->vendorId ? 'selected="selected"' : '' }}
...
<td> <select name="class_id[]"   id="class_id" style="width:200px; height:30px;" required @php if($List->item_count>0){ echo 'disabled'; } @endphp>
...
<td><input type="number" step="0.01"    name="consumption[]" value="{{$List->consumption}}" readOnly id="consumption" style="width:80px; height:30px;" required /></td> 
...
<td><input type="number" step="0.01"  max="5" min="0" class="WASTAGE" name="wastage[]" value="{{$List->wastage}}" id="wastage" style="width:80px; height:30px;" required @php if($List->item_count>0){ echo 'readOnly'; } @endphp /></td> 
...
<td> <select name="item_codess[]" class="item_packing_trims" id="item_codess" style="width:200px; height:30px;" required disabled>
<option value="">--Item List--</option>
...
<td>
<input type="text"    name="descriptionss[]" value="{{$List->description}}" id="descriptionss" style="width:200px; height:30px;" required readOnly/></td> 
<td><input type="number" step="0.01"    name="consumptionss[]" value="{{$List->consumption}}" id="consumptionss" style="width:80px; height:30px;" required readOnly/></td> 
<td> <select name="unit_idss[]" class="" id="unit_idss" style="width:100px; height:30px;" required disabled>
<option value="">--Unit List--</option>
@foreach($UnitList as  $row)
{
    <option value="{{ $row->unit_id }}"
...
<td><input type="number" step="0.01" max="5" min="0" class="WASTAGE2"  name="wastagess[]" value="{{$List->wastage}}" id="wastagess" style="width:80px; height:30px;" required /></td> 
