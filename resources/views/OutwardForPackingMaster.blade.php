...
<form action="{{route('OutwardForPacking.store')}}" method="POST" enctype="multipart/form-data" id="frmData">
@csrf 
<div class="row">
...
<div class="col-md-2">
    <div class="mb-3">
        ...
        <input type="date" name="ofp_date" class="form-control" id="ofp_date" value="{{date('Y-m-d')}}" required  >
...
<div class="col-md-2">
    <div class="mb-3">
       ...
       <select name="sales_order_no" class="form-control select2" id="sales_order_no" required  onchange="getVendorWorkOrderDetails(this.value);">
...
@foreach($ToSent as  $sent) 
    <option value="{{ $sent->loc_id }}"  {{  $sent->loc_id == 6 ? 'selected="selected"' : '' }}>{{ $sent->location }}</option> 
@endforeach
</select>
</div>
</div>   
   ...
   <div class="panel-group" id="accordion">
      ...
      <div class="panel-heading">
        <h4 class="panel-title">
          <a data-toggle="collapse" data-parent="#accordion" href="#collapse2">Stitching Against Sales Order</a>
        </h4>
      </div>
      <div id="collapse2" class="panel-collapse collapse" style="width:100%;">
...
function getVendorWorkOrderDetails(sales_order_no)
{

      $.ajax({
            type: "GET",
            dataType:"json",
            url: "{{ route('getVendorWorkOrderDetailsForPacking') }}",
        ...
        $.ajax({
            dataType: "json",
            url: "{{ route('vw_GetStitichingPOQty') }}",
            data:{'sales_order_no':sales_order_no},
            success: function(data)
            {
