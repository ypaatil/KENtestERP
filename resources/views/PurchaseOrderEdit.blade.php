               ...
               <div class="row">
                  <div class="col-md-2">
                     <div class="mb-3">
                        ...
                        <select name="firm_id" class="form-select" id="firm_id" disabled>
                  ...
                  <div class="col-md-2">
                     <div class="mb-3">
                        ...
                        <select name="Ac_code" class="form-select" id="Ac_code" onchange="getPartyDetails();GetPartyDetailsSale();" disabled>
                           ...
                           >{{ $rowledger->ac_name }} ({{$rowledger->Bt_name}})</option>
                           @endforeach
                        </select>
                     </div>
                  </div>
                  <div class="col-md-2">
                     <div class="mb-3">
                        ...
                        <select name="bill_to" class="form-select select2" id="bill_to" required>
                           ...
                           @foreach($bill_to_details as  $bills)
                                @php
                                   $billed = $bills->sr_no;
                                @endphp
                            <option value="{{ $bills->sr_no  }}" {{ $billed == $purchasefetch->bill_to ? 'selected="selected"' : '' }}>{{ $bills->trade_name }} ({{$bills->site_code}})</option>
                           @endforeach
                        </select>
                     </div>
                  </div>
                  <div class="col-md-2">
                     <div class="mb-3">
                        ...
                        <select name="ship_to" class="form-select select2" id="ship_to" required>
                           <option value="">--- Select ---</option>
                           @foreach($ship_to_details as $ships)
                            @if($ships->site_code != '' && $ships->trade_name != '')
                                @php
                                   $shipped = $ships->sr_no;
                                @endphp
                                <option value="{{ $ships->sr_no  }}" {{ $shipped == $purchasefetch->ship_to ? 'selected="selected"' : '' }}>{{ $ships->trade_name }} ({{$ships->site_code}})</option>
                            @endif
                           @endforeach
                        </select>
                     </div>
                  ...
                  <div class="col-md-2">
                     <div class="mb-3">
                        ...
                        <select name="bom_type[]" class="form-select select2" id="bom_type"  onchange="GetClassesList();" multiple disabled>
                     ...
                     <table id="footable_2" class="table  table-bordered table-striped m-b-0 footable_2">
                        ...
                        <tbody id="bomdis">
                           ...
                           <tr  >
                              ...
                              <td>
                                 <select name="item_codes[]" class="item" id="item_code" style="width:250px; height:30px;" required     onchange="getItemDetails(this);" >
                              ...
                              <td>
                                 <select name="unit_id[]"  id="unit_id" style="width:100px;  height:30px;" required  >
                           ...
                           @php
                           $fabricInward = DB::SELECT("SELECT count(*) as count FROM inward_details WHERE item_code=".$row->item_code." AND po_code='".$purchasefetch->pur_code."'");
                           $trimsInward = DB::SELECT("SELECT count(*) as count FROM trimsInwardDetail WHERE item_code=".$row->item_code." AND po_code='".$purchasefetch->pur_code."'");
                           $incount = isset($fabricInward[0]->count) ? $fabricInward[0]->count: 0;
                           $trimcount = isset($trimsInward[0]->count) ? $trimsInward[0]->count: 0;
                           $isTrDisabled = '';
                           if($incount > 0 || $trimcount > 0)
                           {
                           $isTrDisabled = 'isTrDisabled';
                           }
                           @endphp
                           <tr class="{{$isTrDisabled}}">
                              <td><input type="text" name="id" value="{{ $no }}" id="id"  style="width:50px;height:30px;"/></td>
                              <td> <input type="button" class="btn btn-danger pull-left" onclick="deleteRow(this);" value="X" > <button type="button" onclick="insertRow();mycalc();"  class="btn btn-warning pull-left">+</button><button type="button" onclick="setConversion(this);" class="btn btn-success pull-left">?</button></td>
                              <td><input type="text"  name="sales_order_no[]" value="{{$row->sales_order_no}}" id="sales_order_no" style="width:80px;" readOnly/> </td>
                              <td><input type="text"  name="item_code[]" value="{{$row->item_code}}"  style="width:80px;" readOnly/> </td>
                              <td> 
                                 <input type="hidden"  name="item_codes[]" value="{{$row->item_code}}"  style="width:80px;"/> 
                                 <input type="text"  name="item_name[]" value="{{$row->item_name}}"  style="width:120px;" readonly/> 
                              </td>
                              <td> 
                                 <input type="hidden"  class="ROWCOUNT" id="ROWCOUNT"   value="1"> <input type="text"   name="hsn_code[]"   value="{{ $row->hsn_code }}" id="hsn_code" style="width:80px;height:30px;" readOnly/>
                              </td>
                              <td>
                                 <select name="unit_id[]" class="unit_id" id="unit_id" style="width:100px;height:30px;" disabled>
                                    <option value="">--- Select Unit ---</option>
                                    @foreach($unitlist as  $rowunit)
                              ...
                              <td><input style="width:80px;height:30px;" class="ITEMQTY" type="text" class="Qty" name="item_qtys[]" min="{{ $min }}" max="{{ $row->item_qty }}" value="{{ $row->item_qty }}" @php if(Session::get('user_type')!=1 && $is_approved==1){ echo 'readOnly'; } @endphp  id="item_qty"></td>
                              @php
                              $bom_code='';
               ...
               <div class="row">
                  <div class="col-md-2">
                     <div class="mb-3">
                        ...
                        <input type="text" name="total_qty" class="form-control" id="total_qty" value="{{ $purchasefetch->total_qty  }}" readOnly>
                  ...
                  <div class="col-md-2">
                     <div class="mb-3">
                        ...
                        <input type="text" name="Gross_amount" class="form-control" id="Gross_amount" onChange="mycalc();" value="{{ $purchasefetch->Gross_amount }}" readOnly>
                     </div>
                  </div>
                  <div class="col-md-2">
                     <div class="mb-3">
                        <label for="formrow-email-input" class="form-label">GST Amount</label>
                        <input type="text" name="Gst_amount" class="form-control" id="Gst_amount" value="{{ $purchasefetch->Gst_amount }}" readOnly>
                     </div>
                  </div>
                  <div class="col-md-2">
                     <div class="mb-3">
                        <label for="formrow-email-input" class="form-label">Freight Amount</label>
                        <input type="text" name="totFreightAmt"  class="form-control" id="totFreightAmt" value="{{ $purchasefetch->totFreightAmt }}" readOnly>
                     </div>
                  </div>
                  <div class="col-md-2">
                     <div class="mb-3">
               ...
               <div class="row">
                  ...
                  <div class="col-md-2">
                     <div class="mb-3">
                        ...
                        <select name="po_status" class="form-select" id="po_status" required  onchange="setCloseDate(this);">
                           <option value="">--PO Status--</option>
                           <option value="1"{{ 1 == $purchasefetch->po_status ? 'selected="selected"' : '' }}>Moving</option> 
                           <option value="2"{{ 2 == $purchasefetch->po_status ? 'selected="selected"' : '' }}>Non Moving</option> 
                        </select>
                     </div>
                  </div>
                  <div class="col-md-2">
                     <div class="mb-3">
                        ...
                        <input type="date" name="closeDate" class="form-control" id="closeDate" value="{{$purchasefetch->closeDate}}" readonly >
                     </div>
                  </div>
                  @php 
                  }
                  ...
                  @if(Session::get('gpo_approval_id') == 1)
...
<script> 
   ...
   {   
       ...
       document.getElementById("Gst_amount").value = sum2.toFixed(2);
      ...
      $.ajax({
       ...
       success: function(response){
           ...
           if(tax_type_ids==1)
           {
               ...
               row.find('img[name^="item_image[]"]').attr('src', 'https://kenerp.org/thumbnail/'+response.data[0]['item_image_path']);
   ...
   $("#footable_2 > tbody").find('tr:eq('+idx+') td:eq(9) input').removeAttr('max');
   $("#footable_2 > tbody").find('tr:eq('+idx+') td:eq(10) input').val(final_rate);
   mycalc(); 
   }
      ...
      <div class="modal-content" style="width: 900px;">
         ...
         <div class="modal-body">
               ...
               <div id="InwardData" class="table-responsive">
                  <table >
                     ...
                     <tbody>
                        ...
                        <th><input id="rateM" type="number" value="0" onkeyup="mycalc();"/></th>
            ...
            <div class="modal-footer">
               <button type="button" class="btn btn-success" data-dismiss="modal" onclick="assignValue();mycalc();closemodal();">Submit</button>
