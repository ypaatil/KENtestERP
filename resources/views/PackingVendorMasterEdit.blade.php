...
<div class="card">
<div class="card-body">
<h4 class="card-title mb-4">Packing Vendor</h4>
@if ($errors->any())
...
@if(isset($PackingVendorMasterList))
<form action="{{ route('PackingVendor.update',$PackingVendorMasterList) }}" method="POST" enctype="multipart/form-data" id="frmData">
@method('put')
@csrf
...
<div class="col-md-2">
    <div class="mb-3">
        <label for="vpki_date" class="form-label">Entry Date</label>
        <input type="date" name="vpki_date" class="form-control" id="vpki_date" value="{{$PackingVendorMasterList->vpki_date}}" required  >
        <input type="hidden" name="c_code" class="form-control" id="c_code" value="{{ $PackingVendorMasterList->c_code }}">
        <input type="hidden" name="vpki_code" class="form-control" id="vpki_code" value="{{$PackingVendorMasterList->vpki_code}}" required readOnly>
...
@foreach($VendorPurchaseOrderList as  $row)
{
    <option value="{{ $row->vpo_code }}"
      ...
      {{ $row->vpo_code == $PackingVendorMasterList->vpo_code ? 'selected="selected"' : '' }} 
...
<input type="text" name="sales_order_no" class="form-control" id="sales_order_no" value="{{$PackingVendorMasterList->sales_order_no}}" required readOnly>
</div>
</div>
...
@foreach($BuyerList as  $row)
{
    <option value="{{ $row->ac_code }}"
     {{ $row->ac_code == $PackingVendorMasterList->Ac_code ? 'selected="selected"' : '' }} 
    >{{ $row->ac_name }}</option>
}
@endforeach
</select>
...
<input type="text" name="vendor_rate" class="form-control" id="vendor_rate" value="{{$PackingVendorMasterList->vendor_rate}}" required readOnly>
</div>
</div>   
...
@foreach($MainStyleList as  $row)
{
    ...
    {{ $row->mainstyle_id == $PackingVendorMasterList->mainstyle_id ? 'selected="selected"' : '' }}  
...
@foreach($SubStyleList as  $row)
{
    <option value="{{ $row->substyle_id }}"
     {{ $row->substyle_id == $PackingVendorMasterList->substyle_id ? 'selected="selected"' : '' }}
...
@foreach($FGList as  $row)
{
    ...
    {{ $row->fg_id == $PackingVendorMasterList->fg_id ? 'selected="selected"' : '' }} 
    >{{ $row->fg_name }}</option>
}
@endforeach
</select>
...
<input type="text" name="style_no" class="form-control" id="style_no" value="{{$PackingVendorMasterList->style_no}}" required readOnly>
</div>
</div>
...
<input type="text" name="style_description" class="form-control" id="style_description" value="{{$PackingVendorMasterList->style_description}}" required readOnly>
</div>
</div>
...
@foreach($Ledger as  $rowvendor)
{
    ...
    {{  $rowvendor->ac_code == $PackingVendorMasterList->vendorId ? 'selected="selected"' : '' }}
...
<input class="form-check-input" type="checkbox" id="is_opening" name="is_opening"  
   @if($PackingVendorMasterList->is_opening==1)checked @endif    >
...
@if(count($PackingVendorDetailList)>0)

@php $no=1;$n=1; @endphp
@foreach($PackingVendorDetailList as $List) 
...
<input type="text" name="total_qty" class="form-control" id="total_qty" value="{{$PackingVendorMasterList->total_qty}}" required readOnly>
</div>
</div>
...
<input type="number" step="any" name="rate" class="form-control" id="rate" value="{{$PackingVendorMasterList->rate}}" required onchange="mycalc();">
</div>
</div>
...
<input type="text" name="vendor_amount" class="form-control" id="vendor_amount" value="{{$PackingVendorMasterList->vendor_amount}}" required readOnly>
...
<div class="mb-3">
<input type="text" name="narration" class="form-control" id="narration"  value="{{$PackingVendorMasterList->narration}}" />
</div>
</div>
...
<a href="{{ Route('PackingVendor.index') }}" class="btn btn-warning w-md">Cancel</a>
</div>
</div>
